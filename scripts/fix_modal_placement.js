...
// Remove the wrongly placed modal
const wronglyPlacedRegex = /\{\/\* COLUMN CONFIG MODAL \*\/\}\s*\{isUnlockRequestOpen && \([\s\S]*?<\/div>\s*\)\}\s*const saveStudentScore = async/;
c = c.replace(wronglyPlacedRegex, "const saveStudentScore = async");

// Inject it before the very last </div> of the component!
...
console.log('Fixed Modal Placement inside Return tree');
