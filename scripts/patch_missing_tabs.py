...
with open('src/app/admin/input-assessments/client.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
funcs_injection = """
  ...
  const uniqueAssigned=assignedIds.map(sid=>mappings.find(x=>x.subjectId===sid)).filter(bool=>bool);
...
with open('src/app/admin/input-assessments/client.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
