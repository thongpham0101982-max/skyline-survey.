...
// 2. Add the Navigation Tab
const navTarget = `<button onClick={() => setActiveTab('assignments')} className={\`flex items-center gap-2 pb-3 border-b-2 px-1 font-semibold transition-colors \${activeTab === 'assignments' ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover:text-slate-700'}\`}><UserCheck className="w-4 h-4"/>Phân công GV</button>`;
const navReplacement = navTarget + `\n<button onClick={() => setActiveTab('reports')} className={\`flex items-center gap-2 pb-3 border-b-2 px-1 font-semibold transition-colors \${activeTab === 'reports' ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover:text-slate-700'}\`}><BarChart className="w-4 h-4"/>Tổng hợp KQ</button>`;
...
// Insert it right after the assignments tab or before the last </div>
c = c.replace(/    <\/div>\n  \)\n\}\n$/, tabArea + `\n    </div>\n  )\n}\n`);
fs.writeFileSync('src/app/admin/input-assessments/client.tsx', c, 'utf8');
console.log('Added Reports tab');
