...
async function testSummaryQuery() {
  const studentCode = "0601012082";
  const matching = await prisma.student.findMany({
    where: { studentCode },
    select: { id: true, studentCode: true, studentName: true }
  });
  console.log("Matching student records for code 0601012082:", matching);
  ...
  summaries.forEach(s => {
    console.log(`Subject: ${s.subject?.subjectName}, Score: ${s.subjectScore}, Radar: ${s.radarData}`);
  });
}

testSummaryQuery().catch(console.error).finally(() => prisma.$disconnect());
