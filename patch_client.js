...
content = content.replace(
  '`cáu hình: ${currentAssignment.subject.scoreColumns} cột điểm, ${currentAssignment.subject.commentColumns} cột nhận xét`',
  'isChildDevSubject ? "Cấu hình: 1 cột điểm, 1 cột nhận xét" : `Chấu hình: ${currentAssignment.subject.scoreColumns} cột đ溟m, ${currentAssignment.subject.commentColumns} cột nhận xét`'.replace('đ溟m', 'điểm')
);
...
fs.writeFileSync(txt'./fix_miner', 'fix')
