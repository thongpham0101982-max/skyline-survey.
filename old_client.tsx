...
export function InputAssessmentsClient({ academicYears, campuses, examBoardUsers, subjects: initialSubjects, eduSystems, grades, configs: initialConfigs, teachers = [], departments = [] }: any) {
  ...
  const deleteStudent=async(id)=>{if(!confirm("X�a hoc sinh nay?"))return;await fetch("/api/input-assessment-students?id="+id,{method:"DELETE"});fetchStudents()};
  const deleteSelectedStudents=async()=>{if(selectedStudentIds.length===0)return;if(!confirm("X�a "+selectedStudentIds.length+" hoc sinh �? ch?n?"))return;await fetch("/api/input-assessment-students?ids="+selectedStudentIds.join(","),{method:"DELETE"});setSelectedStudentIds([]);fetchStudents()};
  ...
  const COLUMN_MAP = {
    ...
    "Ngay sinh": "dateOfBirth", "NgaySinh": "dateOfBirth", "dateOfBirth": "dateOfBirth",
    "Khoi KS": "grade", "Khoi": "grade", "Khối": "grade", "grade": "grade",
    "Dien khao sat": "admissionCriteria", "Dien KS": "admissionCriteria", "DienKS": "admissionCriteria", "admissionCriteria": "admissionCriteria",
    "Hinh thuc KS": "surveyFormType", "HinhThucKS": "surveyFormType", "surveyFormType": "surveyFormType",
    "Loai tuyen sinh": "targetType", "LoaiTuyenSinh": "targetType", "targetType": "targetType",
  ...
  const handleExcelImport = async (e) => {
    ...
    try {
      ...
      if (mapped.length === 0) { alert("Kh�ng t?m th?y d? li?u h?p l?. Ki?m tra tên c?t: Ma_HS_KS, Ho ten, Ngay sinh..."); setImporting(false); return; }
  ...
  const downloadTemplate = () => {
    const headers = ["Ma_HS_KS", "Ho ten", "Ngay sinh", "Dien khao sat", "Hinh thuc KS", "Loai tuyen sinh", "Hoc ky", "KQGD Tieu hoc", "KQ Hoc tap", "KQ Ren luyen"];
        ...
        <div className="space-y-5">
          ...
          {selGrades.length>0&&selEdus.length>0&&(
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              <div className="glass-card rounded-2xl overflow-hidden shadow-xl border-slate-200">
                <div className="px-5 py-4 border-b bg-indigo-50/50"><h4 className="font-bold text-indigo-800 flex items-center gap-2"><BookOpen className="w-4 h-4"/>M�n �? g�n ({uniqueAssigned.length})</h4><p className="text-xs text-indigo-500 mt-1">Bo se xoa khoi tat ca tổ hợp</p></div>
        ...
        <div className="space-y-5">
          ...
          {!studentPeriodId ? (
            <div className="text-center py-16 bg-white rounded-xl shadow-sm border text-slate-400">
              ...
              <p className="font-medium">Ch?n Kỳ khảo sát �? xem danh s�ch h?c sinh.</p>
            </div>
          ) : studentsLoading ? (
            ...
            <div className="glass-card rounded-2xl overflow-hidden shadow-xl border-slate-200">
              <div className="flex items-center justify-between px-5 py-4 border-b bg-slate-50/50">
                <h3 className="font-bold text-slate-800 flex items-center gap-2"><Users className="w-5 h-5 text-indigo-500"/>DS Học sinh KS <span className="text-sm font-normal text-slate-400">({filteredStudents.length})</span></h3>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200">
                        ...
                        <th className="px-4 py-3 text-left text-xs uppercase tracking-wider text-slate-500 font-bold">Cấu hình Kh?o s�t</th>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4 mb-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-extrabold text-slate-800 text-lg">Ph�n c�ng Gi�o vi�n Kh?o s�t</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-5 space-y-6 bg-slate-50/50 p-6 rounded-xl border border-slate-100 relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 mb-2">
                   ...
                   <h4 className="font-bold text-slate-700 text-base tracking-tight">K? Kh?o s�t & Ng�?i ph? tr�ch</h4>
                </div>
                
                <div className="space-y-5">
                  <div>
                    <label className="block text-xs uppercase tracking-wider font-bold text-slate-500 mb-2">K? Kh?o s�t <span className="text-red-500">*</span></label>
                ...
                <div className="space-y-8 flex-1">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <span className="block text-xs uppercase tracking-wider font-bold text-slate-500">M�n Kh?o s�t <span className="text-red-500">*</span></span>
          ...
          {assignPeriodId && (
            <div className="glass-card rounded-2xl overflow-hidden shadow-xl border-slate-200">
               <div className="flex items-center justify-between p-4 border-b bg-slate-50/50">
                  <h4 className="font-bold text-slate-800">Danh s�ch �? Ph�n c�ng ({assignments.length})</h4>
        ...
        <div className="space-y-5">
          <div className="glass-card p-6 rounded-2xl hover-lift border-slate-200 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3 w-full lg:w-auto">
              ...
              <div>
                <h3 className="font-bold text-slate-800 text-base">Tổng hợp K?t qu? Kh?o s�t</h3>
            ...
            <div className="flex flex-wrap items-center gap-2 lg:ml-auto w-full lg:w-auto mt-2 lg:mt-0">
                <select className="border rounded-lg px-3 py-1.5 outline-none text-sm font-medium bg-slate-50 min-w-[200px]" onChange={async (e) => {
                  ...
                  <option value="">-- Chọn Kỳ Kh?o s�t --</option>
                ...
                <button 
                    ...
                    onClick={() => {
                        const wb = XLSX.utils.book_new();
                        const wsData = [
                            ["STT", "H? T�n", "M? HS", "Ng�y sinh", "Khối", "Hệ học"], // Header row
                        ];
          ...
          {!viewResultsData ? (
            <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center">
              ...
              <p className="text-sm text-slate-400 mt-2">D? li?u s? ��?c t?ng h?p t? t?t c? c�c đợt trong k?.</p>
            </div>
            ...
            <div className="text-center py-20 bg-white rounded-2xl shadow-sm border text-slate-500 font-medium">Kỳ khảo sát n�y ch�a c� h?c sinh n�o.</div>
          ) : (
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-slate-50 text-slate-600 font-bold uppercase text-[10px] tracking-wider border-b border-slate-200 sticky top-0 z-10">
                            <tr>
                                ...
                                <th className="px-5 py-3 text-left w-64 border-r">Th�ng tin H?c sinh</th>
                                <th className="px-5 py-3 text-left border-r min-w-[500px]">Chi ti?t �i?m & Nh?n x�t c�c môn</th>
...
const assignedTeacherName = (viewResultsData.assignments || []).find((a:any) => a.subjectId === sc.subjectId && a.grade === s.grade && (a.educationSystem === s.admissionCriteria || a.educationSystem === s.surveySystem))?.user?.fullName;
                                    ...
                                    <td className="p-4 align-middle text-center">
                                        {/* Dropdown to approve/change status right here */}
                                        <div className="flex flex-col gap-2 relative">
                                            <select 
                                                value={s.admissionResult || ""}
                                                onChange={async (e) => {
                                                    const val = e.target.value;
                                                    // API call to update student admissionResult
                                                    const r = await fetch("/api/input-assessment-students", {
                                                        method: "PUT",
                                                        headers: { "Content-Type": "application/json" },
...
{/* MODALS */}
      ...
      {isSubjectOpen&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="flex justify-between items-center p-4 border-b bg-slate-50/80"><h3 className="font-bold text-slate-800 text-lg">{editingSubjectId?'Sửa M�n Kh?o s�t':'Thêm M�n Kh?o s�t m?i'}</h3><button onClick={()=>setIsSubjectOpen(false)} className="text-slate-400 hover:text-slate-600 bg-white hover:bg-slate-100 rounded-full p-1 border shadow-sm"><X className="w-5 h-5"/></button></div>
            <div className="p-5 space-y-4">
      ...
      {isStudentOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm overflow-y-auto">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[70vh] overflow-y-auto">
                ...
                <div className="space-y-4">
                    <h4 className="font-bold text-indigo-900 border-b pb-2 text-sm uppercase">Thi?t l?p đợt & Kh?o s�t</h4>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">N�m h?c Tuy?n sinh</label><select value={studentForm.hocKy} onChange={e=>setStudentForm({...studentForm, hocKy: e.target.value})} className="w-full border rounded-xl px-3 py-2 text-sm outline-none"><option value="">-- Ch?n --</option>{configsList.filter(c=>c.categoryType==='HOC_KY').map(c => <option key={c.id} value={c.code}>{c.name}</option>)}</select></div>
                        <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Loại Tuy?n sinh</label><select value={studentForm.targetType} onChange={e=>setStudentForm({...studentForm, targetType: e.target.value})} className="w-full border rounded-xl px-3 py-2 text-sm outline-none"><option value="">-- Ch?n --</option>{configsList.filter(c=>c.categoryType==='LOAI_TUYEN_SINH').map(c => <option key={c.id} value={c.code}>{c.name}</option>)}</select></div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                ...
                {studentForm.grade && parseInt(studentForm.grade) && (
                    <div className="col-span-1 md:col-span-2 space-y-4 mt-2 border-t pt-4">
                        <h4 className="font-bold text-teal-800 flex items-center gap-2 text-sm uppercase"><Award className="w-4 h-4"/> Th�nh t�ch & H? s�</h4>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">{parseInt(studentForm.grade) <= 5 ? 'H? s� T�m t?t theo CT B?' : 'Kết quả Học tập theo CT Bộ'}</label><select value={studentForm.kqgdTieuHoc} onChange={e=>setStudentForm({...studentForm, kqgdTieuHoc: e.target.value})} className="w-full border border-rose-200 bg-rose-50 rounded-xl px-3 py-2 text-sm font-bold outline-none focus:border-rose-400 focus:ring-1 focus:ring-rose-400"><option value="">-- Chon --</option>{configsList.filter(c => parseInt(studentForm.grade) <= 5 ? c.categoryType === 'KQGD_TIEU_HOC' : c.categoryType === 'KQ_HOC_TAP').map(c => (<option key={c.id} value={c.code}>{c.name}</option>))}</select></div>
