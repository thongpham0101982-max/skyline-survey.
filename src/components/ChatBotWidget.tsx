...
export function ChatBotWidget({ role = "TEACHER", chatbotCode }: ChatBotWidgetProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
        ...
        <div className="w-[360px] md:w-[410px] h-[550px] bg-white/95 backdrop-blur-md rounded-3xl border border-slate-200 shadow-2xl flex flex-col overflow-hidden mb-4 transition-all duration-300 animate-in slide-in-from-bottom-5">
          ...
          <div className="bg-[#003B3A] text-white p-4 flex items-center justify-between shadow-md shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="p-2 bg-white/10 rounded-2xl">
                <Bot className="w-5 h-5 text-[#48BFE3]" />
              </div>
              <div>
                ...
                >
                  {isBot && (
                    <div className="w-7 h-7 bg-[#003B3A] text-white rounded-xl flex items-center justify-center shrink-0 shadow-sm mt-0.5">
                      <Bot className="w-4 h-4 text-[#48BFE3]" />
                    </div>
                  )}
                  <div 
                    className={`p-3 rounded-2xl shadow-xs ${
                      isBot 
                        ...
                        : "bg-[#48BFE3] text-white rounded-tr-none"
                    }`}
                  >
              ...
              <div className="flex flex-wrap gap-1.5">
                {sampleQuestions.map((q, idx) => (
                  <button
                    ...
                    className="px-2.5 py-1 bg-white hover:bg-[#48BFE3]/10 border border-slate-200 hover:border-[#48BFE3]/20 text-[10px] md:text-xs font-semibold text-slate-600 hover:text-[#48BFE3] rounded-xl transition duration-200 text-left"
                  >
                    {q.label}
                  </button>
                ))}
          ...
          >
            <input 
              ...
              className="flex-1 text-xs md:text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white focus:border-[#48BFE3] focus:ring-1 focus:ring-[#48BFE3] transition text-xs font-semibold"
            />
            <button 
              type="submit"
              disabled={!input.trim() || isLoading}
              className="p-2.5 bg-[#48BFE3] hover:bg-[#008B84] text-white rounded-2xl shadow-md disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none transition shrink-0 active:scale-95"
            >
      ...
      >
        <span className="absolute -inset-1 rounded-full bg-[#48BFE3]/20 animate-ping opacity-75 group-hover:animate-none"></span>
        {isOpen ? (
          <ChevronDown className="w-6 h-6 text-[#48BFE3]" />
        ) : (
          <MessageSquare className="w-6 h-6 text-[#48BFE3]" />
        )}
      </button>
    </div>
  )
