...
export function Sidebar({ role, permissionModules, actualRole, taskCount = 0 }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar Content */}
      <aside className={`w-64 bg-slate-900 text-white p-6 flex flex-col shadow-xl fixed md:sticky inset-y-0 left-0 z-40 h-screen transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="flex items-center gap-3 mb-8 px-2">
          <img src="/logo.png" alt="Sky-Line Logo" className="h-8 w-auto object-contain brightness-0 invert opacity-90" />
          <div className="font-bold text-lg tracking-tight leading-none">{title}</div>
        </div>
