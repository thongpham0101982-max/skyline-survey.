...
const npsColor = (n: number) => {
  if (n <= 3) return "bg-[#00A99D] border-[#00A99D] text-white shadow-teal-100"
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        <div className="flex items-center justify-center w-10 h-10 rounded-2xl bg-[#00A99D] text-white text-sm font-black shadow-xl shadow-teal-100 flex-shrink-0">
          {idx + 1}
        </div>
        <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Câu hỏi {idx + 1}/{total}</span>
        {q.isRequired && <span className="ml-auto text-[10px] font-black text-white bg-[#00A99D] px-3 py-1 rounded-full uppercase tracking-tighter">Bắt buộc</span>}
      </div>
          ...
          {opts.choices.map((opt: string, i: number) => (
            <div
              ...
              className={`flex items-center gap-4 p-5 rounded-3xl border-2 cursor-pointer transition-all duration-300 select-none
                ${answer === opt ? "border-[#00A99D] bg-teal-50/50 shadow-xl shadow-teal-100/50 -translate-y-1" : "border-slate-100 bg-slate-50/50 hover:border-teal-300 hover:bg-white hover:shadow-lg"}`}
            >
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${answer === opt ? "border-[#00A99D] bg-[#00A99D]" : "border-slate-300"}`}>
            ...
            return (
              <div
                ...
                className={`flex items-center gap-4 p-5 rounded-3xl border-2 cursor-pointer transition-all duration-300
                  ${checked ? "border-[#00A99D] bg-teal-50/50 shadow-xl shadow-teal-100/50 -translate-y-1" : "border-slate-100 bg-slate-50/50 hover:border-teal-300 hover:bg-white"}`}
              >
                <div className={`w-6 h-6 rounded-xl border-2 flex items-center justify-center flex-shrink-0 transition-all ${checked ? "border-[#00A99D] bg-[#00A99D] shadow-md shadow-teal-100" : "border-slate-300"}`}>
      ...
      {q.questionType === "TEXT" && (
        <textarea rows={5} placeholder="Chia sẻ cảm nghĩ, mong muốn của bạn với nhà trường..."
          ...
          className="w-full p-6 text-slate-800 font-bold resize-none focus:border-[#00A99D] focus:bg-white outline-none transition-all placeholder:text-slate-300 text-base sm:text-lg shadow-inner focus:shadow-2xl focus:shadow-teal-50/50 text-xs font-semibold"
        />
      )}
            ...
            <tbody className="divide-y-8 divide-transparent">
              {opts.rows?.map((row: string, rIndex: number) => (
                <tr key={rIndex} className="group transition-all">
                  <td className="p-2 text-sm font-black text-slate-700 group-hover:text-[#00A99D] transition-colors border border-slate-200">{row}</td>
                      ...
                      <td key={cIndex} className="p-2 text-center border border-slate-200">
                        <div 
                          ...
                          className={`w-8 h-8 border-2 mx-auto cursor-pointer flex items-center justify-center transition-all duration-200
                            ...
                            ${isSelected ? "border-[#00A99D] bg-[#00A99D] shadow-xl shadow-teal-100" : "border-slate-100 bg-slate-100/50 group-hover:border-teal-300"}`}
                        >
                          {isSelected && (
                            isCheckbox 
    ...
    <div className="relative w-full max-w-2xl mx-auto font-outfit p-4 sm:p-0 pb-32">
      ...
      <div className="mb-10 animate-in fade-in slide-in-from-left duration-500">
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="bg-[#00A99D] text-white px-3 py-1 rounded-lg text-[11px] font-black shadow-lg shadow-teal-100">{progress}%</div>
        </div>
        <div className="h-3 bg-slate-100 rounded-full overflow-hidden shadow-inner">
          <div className="h-full bg-gradient-to-r from-red-600 via-[#00A99D] to-red-400 rounded-full transition-all duration-700 ease-out shadow-lg" style={{ width: `${progress}%` }} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-[3rem] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.1)] border border-slate-50 overflow-hidden animate-in fade-in slide-in-from-bottom-10 duration-700">
        <div className="h-2 bg-gradient-to-r from-[#00A99D] via-[#1E8B87] to-[#135E5B]" />
          ...
          <div className="flex items-center justify-between mt-12 pt-8 border-t border-slate-50">
            ...
            <div className="flex gap-2 items-center">
              {questions.map((_: any, i: number) => (
                <div key={i} className={`h-1.5 rounded-full transition-all duration-500 ${i === current ? "w-8 bg-[#00A99D]" : (answers[questions[i].id] ? "w-3 bg-emerald-400" : "w-1.5 bg-slate-100")}`} />
              ))}
            </div>
            {current < total - 1 ? (
              <button type="button" onClick={goNext}
                className="flex items-center justify-center w-14 h-14 sm:w-auto sm:px-10 rounded-[1.5rem] font-black text-white bg-[#00A99D] hover:bg-[#009085] active:scale-95 shadow-2xl shadow-teal-100 transition-all"
              >
