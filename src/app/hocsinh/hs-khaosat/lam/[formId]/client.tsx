...
export default function HsFormClient({ formId, periodName, studentName, className, questions }: Props) {
  ...
  return (
    <div className="min-h-screen font-outfit" style={{ background: '#f8fafc' }}>
      <div className="text-white sticky top-0 z-50 shadow-xl" style={{ background: 'linear-gradient(135deg,#BE1E2E,#8b0000)' }}>
      ...
      <div className="max-w-2xl mx-auto px-6 py-10 pb-40">
        {err && (
          <div className="mb-8 p-5 rounded-[2rem] flex items-start gap-4 text-red-600 border-2 border-red-100 bg-red-50/50 animate-in slide-in-from-top-4 shadow-sm">
              ...
              <div key={q.id} className="bg-white rounded-[2.5rem] border-2 p-8 shadow-xl shadow-slate-200/50 transition-all duration-500 hover:shadow-2xl hover:shadow-slate-200/60"
                ...
                <div className="flex items-start gap-4 mb-8">
                  ...
                  <p className="font-black text-slate-800 text-base leading-relaxed pt-1">
                    {q.questionText} {q.isRequired && <span className="text-[#BE1E2E] ml-1">*</span>}
                ...
                <div className="mt-4 transition-all duration-500">
                      ...
                      <div className="flex gap-2.5 flex-wrap justify-center sm:justify-start">
                        {Array.from({ length: (q.ratingMax || 10) - (q.ratingMin || 0) + 1 }, (_, k) => k + (q.ratingMin || 0)).map(v => (
                          <button key={v} onClick={() => ans(q.id, v)}
                            ...
                            style={{
                              background: value === v ? '#BE1E2E' : '#f8fafc',
                              color: value === v ? 'white' : '#475569',
                              borderColor: value === v ? '#BE1E2E' : '#e2e8f0',
                  ...
                  {['TEXT', 'OPEN_ENDED', 'COMMENT', 'ESSAY'].includes(type) && (
                    <textarea 
                      ...
                      className="w-full rounded-3xl px-6 py-5 text-sm font-bold text-slate-700 outline-none resize-none transition-all border-2 border-slate-50 focus:border-red-100 focus:bg-white shadow-inner"
                        ...
                        {opts.map((opt) => (
                          <button key={opt} onClick={() => ans(q.id, opt)}
                            ...
                            style={{
                              ...
                              borderColor: value === opt ? '#BE1E2E' : '#f1f5f9',
                              color: value === opt ? '#BE1E2E' : '#475569',
                              boxShadow: value === opt ? '0 4px 15px rgba(190,30,46,0.05)' : ''
                            }}>
                            {opt}
                            <div className={`w-5 h-5 rounded-full border-2 transition-all flex items-center justify-center ${value === opt ? 'border-[#BE1E2E] bg-[#BE1E2E]' : 'border-slate-200 bg-white group-hover:border-red-200'}`}>
                        ...
                        {opts.map((opt) => (
                          <button key={opt} onClick={() => toggle(opt)}
                            ...
                            style={{
                              ...
                              borderColor: currentVals.includes(opt) ? '#BE1E2E' : '#f1f5f9',
                              color: currentVals.includes(opt) ? '#BE1E2E' : '#475569'
                            }}>
                            {opt}
                            <div className={`w-6 h-6 rounded-lg border-2 transition-all flex items-center justify-center ${currentVals.includes(opt) ? 'border-[#BE1E2E] bg-[#BE1E2E]' : 'border-slate-200 bg-white group-hover:border-red-200'}`}>
                                  ...
                                  return (
                                    <td key={ci} className="p-4 text-center">
                                      <button
                                        ...
                                        className={`w-8 h-8 mx-auto flex items-center justify-center border-2 transition-all hover:scale-110 active:scale-90 ${isSelected ? 'border-[#BE1E2E] bg-[#BE1E2E] shadow-lg shadow-red-100' : 'border-slate-200 bg-white group-hover/row:border-red-200'} ${isCheckGrid ? 'rounded-xl' : 'rounded-full'}`}
                                      >
        ...
        <div className="max-w-2xl mx-auto flex gap-4">
          ...
          {step < total - 1 ? (
            ...
            <button onClick={submit} disabled={submitting} className="flex-1 flex items-center justify-center gap-3 py-4 rounded-[1.75rem] font-black text-sm text-white transition-all active:scale-95 disabled:opacity-50 shadow-2xl shadow-red-200"
              style={{ background: 'linear-gradient(135deg,#BE1E2E,#a01927)' }}>
              {submitting
