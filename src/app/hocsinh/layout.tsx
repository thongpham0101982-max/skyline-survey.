...
export default function HocSinhLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#F6F9F9] flex flex-col font-sans text-slate-800 antialiased selection:bg-[#007A72] selection:text-white">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200/90 px-2 py-2 flex items-center justify-around shadow-[0_-4px_20px_rgba(0,0,0,0.06)] select-none">
        <Link
          href="/hocsinh/portal"
        ...
        <Link
          ...
          className={`flex flex-col items-center gap-1 text-[10px] transition-colors ${
            pathname.includes("/ho-tro") ? "text-rose-600 font-black" : "text-rose-500 hover:text-rose-700 font-medium"
          }`}
        >
