...
export async function POST(req: any) {
  try {
    ...
    const result = await (prisma as any).preschoolAssessmentConfig.create({ data, include: { academicYear: true } });
...
export async function PUT(req: any) {
  try {
    ...
    const result = await (prisma as any).preschoolAssessmentConfig.update({ where: { id }, data });
    return NextResponse.json(result);
  } catch (error: any) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
