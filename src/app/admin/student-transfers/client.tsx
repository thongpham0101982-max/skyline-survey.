...
import { getTransferFormOptionsAction, getClassesByCampusAndYearAction, getStudentsByClassAction, createTransferOutAction, getTransfersAction } from "./actions"
    ...
    <div className="bg-white rounded-[32px] shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="p-8">
        <div className="flex items-center justify-between mb-6">
           ...
           {activeTab === "OUT" && (
             <button onClick={() => setShowModal(true)} className="px-6 py-3 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-indigo-700 transition-all flex items-center shadow-lg shadow-indigo-100">
        ...
        {activeTab === "OUT" && (
          loadingList ? (
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-2xl p-16 text-center">
               ...
               <p className="text-slate-500 font-medium">Bấm "Tạo phiếu lưu chuyển" để thêm mới.</p>
            </div>
          )
        )}
      ...
      {showModal && <TransferOutModal onClose={() => setShowModal(false)} onSaved={loadTransfers} />}
    </div>
  )
}
...
function TransferOutModal({ onClose, onSaved }: { onClose: () => void, onSaved: () => void }) {
  ...
  const PROVINCES = ["Hà Nội", "TP HCM", "Đà Nẵng", "Hải Phòng", "Cần Thơ", "Quảng Nam", "Huế", "Khác..."] // Mock list
