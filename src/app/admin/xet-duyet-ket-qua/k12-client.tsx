...
export function XetDuyetK12Client({ academicYears = [], campuses = [], examBoardUsers = [], subjects: initialSubjects = [], eduSystems = [], configs: initialConfigs = [], grades = [], teachers = [], departments = [], giaoVuCSUsers = [], gdcsUsers = [], currentUser = null, rolePermissions = [] }: Props) {
  ...
  const handleSendBatchApprovalEmail = async () => {
    try {
      ...
      let confirmMsg = "";
      if (isOpenDay) {
        confirmMsg = `Kỳ khảo sát Open Day sẽ tự động gửi email thông báo xét duyệt đến Giám đốc của từng Cơ sở đăng ký của học sinh trong Đợt khảo sát "${activeBatch.name}". Bạn có chắc chắn muốn gửi?`;
      } else {
        confirmMsg = `Gửi email thông báo cho GĐCS cơ sở ${campusName} để xét duyệt Đợt khảo sát "${activeBatch.name}"?\nEmail nhận: ${gdcsEmail}`;
      }
      ...
      if (res.ok && data.success) {
        if (data.groupedSent) {
          ...
          alert(`Đã gửi email thông báo xét duyệt Đợt khảo sát thành công đến các GĐCS:\n${sentDetails}`);
        } else {
...
${reportForm.directorNote}`;
  ...
  const enrollmentStats = useMemo(() => {
    let g1_passed = 0; let g1_enrolled = 0;
    let g6_passed = 0; let g6_enrolled = 0;
    let g10_passed = 0; let g10_enrolled = 0;
    let other_passed = 0; let other_enrolled = 0;
    let total_passed = 0; let total_enrolled = 0;

    filteredReportStudents.forEach(s => {
      const isPassed = s.admissionResult === "Đạt" || s.admissionResult === "Đạt cam kết";
      if (!isPassed) return;

      total_passed++;
      const isEnrolled = s.enrollmentStatus === "COMPLETED";
      if (isEnrolled) total_enrolled++;
      ...
      if (g === "1") {
        g1_passed++;
        if (isEnrolled) g1_enrolled++;
      } else if (g === "6") {
        g6_passed++;
        if (isEnrolled) g6_enrolled++;
      } else if (g === "10") {
        g10_passed++;
        if (isEnrolled) g10_enrolled++;
      } else {
        other_passed++;
        if (isEnrolled) other_enrolled++;
      }
    });

    const overallRate = total_passed > 0 ? Math.round((total_enrolled / total_passed) * 100) : 0;
    const g1Rate = g1_passed > 0 ? Math.round((g1_enrolled / g1_passed) * 100) : 0;
    const g6Rate = g6_passed > 0 ? Math.round((g6_enrolled / g6_passed) * 100) : 0;
    const g10Rate = g10_passed > 0 ? Math.round((g10_enrolled / g10_passed) * 100) : 0;
    const otherRate = other_passed > 0 ? Math.round((other_enrolled / other_passed) * 100) : 0;

    return {
      total_passed, total_enrolled, overallRate,
      g1_passed, g1_enrolled, g1Rate,
      g6_passed, g6_enrolled, g6Rate,
      g10_passed, g10_enrolled, g10Rate,
      other_passed, other_enrolled, otherRate
    };
  ...
  const safeDestinationSchools = useMemo(() => {
    return Array.isArray(destinationSchools) ? destinationSchools.filter((s: any) => s && s.name) : [];
  }, [destinationSchools]);
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
          ...
          {reportsSubTab === "stats" ? (
            <div className="space-y-8 animate-in fade-in duration-300 bg-gradient-to-br from-slate-50 via-white to-teal-50/10 text-slate-800 p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div className="relative overflow-hidden bg-white p-6 rounded-2xl border border-slate-200 border-l-4 border-l-blue-500 shadow-sm flex flex-col justify-between group hover:-translate-y-1 hover:shadow-md transition-all duration-300">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="p-2.5 bg-blue-50 text-blue-600 rounded-xl transition-all duration-300 group-hover:scale-110 group-hover:bg-blue-100">
                ...
                <div className="relative overflow-hidden bg-white p-6 rounded-2xl border border-slate-200 border-l-4 border-l-emerald-500 shadow-sm flex flex-col justify-between group hover:-translate-y-1 hover:shadow-md transition-all duration-300">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Thống kê Nhập học (Đạt/Cam kết)</span>
                  ...
                  <div className="flex items-baseline gap-1 mt-4">
                    <span className="text-4xl font-black text-slate-800 tracking-tight">{enrollmentStats.total_enrolled}</span>
                    <span className="text-xs text-slate-405 font-bold">/ {enrollmentStats.total_passed} HS nhập học ({enrollmentStats.overallRate}%)</span>
                  </div>
                </div>
              </div>
                      ...
                      >
                        ...
                        <div className="space-y-1.5 pt-2 border-t border-slate-100 text-[10px]">
                          ...
                          <div className="grid grid-cols-3 gap-1 pt-1.5 font-bold text-center">
                            ...
                            <div className="bg-purple-50 text-purple-700 rounded py-0.5">
                              <span className="block text-[8px] uppercase font-bold opacity-60">C.K</span>
                              <span className="text-[10px] font-black">{stat.committed}</span>
                            </div>
              ...
              {/* Chart & Enrollment Breakdown Row */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm p-6 overflow-hidden hover:shadow-md transition-all duration-300">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 border-b border-slate-100 pb-4 gap-3">
                    <h4 className="font-black text-slate-800 text-sm tracking-tight uppercase flex items-center gap-2.5">
                      ...
                      Biểu đồ theo dõi Học sinh Khảo sát theo Khối của các Cơ sở
                    </h4>
                    <div className="flex items-center gap-2 self-end sm:self-auto">
                      ...
                      <select
                        ...
                        className="bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-black text-slate-700 py-1.5 px-3 outline-none cursor-pointer hover:border-slate-300 transition-colors shadow-sm"
                      >
                        <option value="all">Tất cả các cơ sở</option>
                        ...
                        <Tooltip
                          content={({ active, payload, label }) => {
                            if (active && payload && payload.length) {
                              ...
                              const overallTotal = campusOverallTotal.total;
                              ...
                              if (chartCampusId === "all") {
                                return (
                                  <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-lg text-xs font-semibold text-slate-800 space-y-2">
                                    <div className="font-black text-sm border-b border-slate-100 pb-1.5 flex justify-between gap-4">
                                      ...
                                      <span className="text-indigo-650">{stat.total}/{overallTotal} HS ({gradePct}%)</span>
                                    </div>
                                    
                                    <div className="space-y-1.5 pt-1">
                                      <div className="flex justify-between gap-4 text-slate-400 text-[9px] font-black uppercase tracking-wider">
                                        <span>Cơ sở (Tổng HS Cơ sở)</span>
                                        <span>Số HS trong Khối / Tổng Cơ sở</span>
                                      </div>
                                      
                                      {[
                                        { code: "CS1", name: "Cơ sở 1", color: "bg-teal-500", total: campusOverallTotal.CS1, count: stat.CS1_total },
                                        { code: "CS2", name: "Cơ sở 2", color: "bg-pink-500", total: campusOverallTotal.CS2, count: stat.CS2_total },
                                        { code: "CS3", name: "Cơ sở 3", color: "bg-purple-500", total: campusOverallTotal.CS3, count: stat.CS3_total },
                                        { code: "CS4", name: "Cơ sở 4", color: "bg-amber-500", total: campusOverallTotal.CS4, count: stat.CS4_total },
                                        { code: "CS5", name: "Cơ sở 5", color: "bg-blue-500", total: campusOverallTotal.CS5, count: stat.CS5_total }
                                      ].map(cs => {
                                        const csPctOfCampus = cs.total > 0 ? ((cs.count / cs.total) * 100).toFixed(1) : "0";
                                        ...
                                        return (
                                          <div key={cs.code} className="flex justify-between items-center py-0.5">
                                            ...
                                            <span className="font-extrabold text-slate-850">{cs.count}/{cs.total} HS ({csPctOfCampus}%)</span>
                                          </div>
                                        );
                                      })}
                                    </div>
                                ...
                                const campusTotal = campusOverallTotal[campusCode] || 0;
                                const campusGradeTotal = stat[`${campusCode}_total`] || 0;
                                const campusGradePct = campusTotal > 0 ? ((campusGradeTotal / campusTotal) * 100).toFixed(1) : "0";
                                ...
                                return (
                                  <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-lg text-xs font-semibold text-slate-800 space-y-2">
                                    <div className="font-black text-sm border-b border-slate-100 pb-1.5 flex justify-between gap-4">
                                      ...
                                      <span className="text-teal-650">{campusGradeTotal}/{campusTotal} HS ({campusGradePct}%)</span>
                                    </div>
                                    <div className="space-y-1.5 pt-1">
                                      <div className="flex justify-between py-0.5">
                                        <span className="text-slate-500 font-bold">Số HS Khảo sát:</span>
                                        <span className="font-extrabold text-slate-850">{campusGradeTotal} HS</span>
                                      </div>
                                      <div className="flex justify-between py-0.5 border-t border-slate-100/50 mt-1 pt-1">
                                        ...
                                        <span className="font-extrabold text-slate-800">{stat.total > 0 ? ((campusGradeTotal / stat.total) * 100).toFixed(1) : 0}%</span>
                                      </div>
                                    </div>
                                  </div>
                                );
                ...
                <div className="lg:col-span-1 bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col justify-between hover:shadow-md transition-all duration-300">
                  <div className="flex items-center justify-between mb-5 border-b border-slate-100 pb-4">
                    ...
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 px-2 py-0.5 rounded">Trọng tâm</span>
                  </div>

                  <div className="space-y-5 flex-1 flex flex-col justify-center">
                    {[
                      { label: "Khối 1 (Tiểu học)", passed: enrollmentStats.g1_passed, enrolled: enrollmentStats.g1_enrolled, rate: enrollmentStats.g1Rate, color: "from-teal-400 to-teal-500", bg: "bg-teal-50 text-teal-700 border-teal-100" },
                      { label: "Khối 6 (THCS)", passed: enrollmentStats.g6_passed, enrolled: enrollmentStats.g6_enrolled, rate: enrollmentStats.g6Rate, color: "from-indigo-400 to-indigo-500", bg: "bg-indigo-50 text-indigo-700 border-indigo-100" },
                      { label: "Khối 10 (THPT)", passed: enrollmentStats.g10_passed, enrolled: enrollmentStats.g10_enrolled, rate: enrollmentStats.g10Rate, color: "from-amber-400 to-amber-500", bg: "bg-amber-50 text-amber-700 border-amber-100" },
                      { label: "Các khối khác", passed: enrollmentStats.other_passed, enrolled: enrollmentStats.other_enrolled, rate: enrollmentStats.otherRate, color: "from-slate-400 to-slate-500", bg: "bg-slate-50 text-slate-600 border-slate-200" }
                    ].map((g, idx) => (
                      <div key={idx} className="space-y-2">
                        <div className="flex justify-between items-center text-xs">
                          <span className="font-extrabold text-slate-750">{g.label}</span>
                          <span className={`px-2 py-0.5 rounded text-[10px] font-black ${g.bg}`}>
                            {g.enrolled} / {g.passed} HS ({g.rate}%)
                          </span>
                        </div>
                        <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden border border-slate-100/50">
                          <div className={`h-full bg-gradient-to-r ${g.color} rounded-full transition-all duration-500`} style={{ width: `${g.rate}%` }}></div>
                        </div>
                      </div>
                    ))}
                  </div>

                  <div className="mt-5 pt-4 border-t border-slate-100/80 text-[10px] font-bold text-slate-400 uppercase text-center tracking-wider">
                    Dựa trên dữ liệu HS lưu chuyển (Chuyển đến)
                  </div>
                </div>
              </div>

              {/* Campus Breakdown Table Card */}
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 overflow-hidden flex flex-col justify-between hover:shadow-md transition-all duration-300">
                <div className="flex items-center justify-between mb-5 border-b border-slate-100 pb-4">
                  <h4 className="font-black text-slate-800 text-sm tracking-tight uppercase flex items-center gap-2.5">
                    <span className="flex h-2 w-2 relative">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-teal-400 opacity-75"></span>
                  ...
                  <table className="w-full text-left whitespace-nowrap table-auto border-collapse">
                    <thead>
                      <tr className="border-b border-slate-200 bg-slate-50/50">
                        ...
                        <th className="py-3.5 px-4 text-right text-[9px] font-extrabold text-slate-500 uppercase tracking-wider rounded-r-xl">Tỷ lệ duyệt</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {campusStats.map(stat => {
                        ...
                        return (
                          <tr key={stat.id} className="hover:bg-slate-50/50 transition-colors duration-150 group/row text-xs font-semibold text-slate-700">
                            <td className={`py-3.5 px-4 border-l-4 ${borderLeftColor}`}>
                              <div className="flex items-center gap-2.5">
                                <span className={`px-2 py-1 rounded-md text-[10px] font-black tracking-wider ${badgeColor}`}>{shortCode}</span>
                                <span className="text-sm font-bold text-slate-800 group-hover/row:text-slate-900 transition-colors">{stat.campusName}</span>
                              </div>
                            </td>
                            ...
                            <td className="py-3.5 px-4 text-center">
                              <span className="font-extrabold text-slate-700 text-sm">{stat.surveyed}</span>
                            </td>
                            <td className="py-3.5 px-4 text-center">
                              {stat.passed > 0 ? (
                                <span className="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 text-xs font-bold">{stat.passed}</span>
                              ) : (
                            ...
                            <td className="py-3.5 px-4 text-center">
                              {stat.committed > 0 ? (
                                <span className="px-2 py-0.5 rounded bg-purple-50 text-purple-700 text-xs font-bold">{stat.committed}</span>
                              ) : (
                                <span className="text-slate-350 font-bold text-xs">—</span>
                              )}
                            </td>
                            <td className="py-3.5 px-4 text-center">
                              {stat.failed > 0 ? (
                                <span className="px-2 py-0.5 rounded bg-rose-50 text-rose-700 text-xs font-bold">{stat.failed}</span>
                              ) : (
                                <span className="text-slate-350 font-bold text-xs">—</span>
                              )}
                            </td>
                            <td className="py-3.5 px-4 text-center">
                              {stat.pending > 0 ? (
                                <span className="px-2 py-0.5 rounded bg-slate-50 text-slate-500 text-xs font-bold">{stat.pending}</span>
                              ) : (
                                <span className="text-slate-350 font-bold text-xs">—</span>
                              )}
                            </td>
                              ...
                              <div className="flex items-center justify-end gap-2.5">
                                ...
                                <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                  <div className={`h-full ${
                                    shortCode === "CS1" ? "bg-teal-500" : 
                                    shortCode === "CS2" ? "bg-pink-500" : 
                                    shortCode === "CS3" ? "bg-purple-500" : 
                                    shortCode === "CS4" ? "bg-amber-500" : 
                                    shortCode === "CS5" ? "bg-blue-500" : "bg-slate-400"
                                  } rounded-full`} style={{ width: `${campusSurveyRate}%` }}></div>
                                </div>
                              </div>
                            </td>
                            <td className="py-3.5 px-4 text-right">
                              <div className="flex items-center justify-end gap-2.5">
                                ...
                                <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                  <div className={`h-full ${
                                    ...
                                    shortCode === "CS4" ? "bg-amber-500" : 
                                    shortCode === "CS5" ? "bg-blue-500" : "bg-slate-400"
                                  } rounded-full`} style={{ width: `${campusApprovedRate}%` }}></div>
                                </div>
                              </div>
                            </td>
                          </tr>
                      ...
                      {campusStats.length === 0 && (
                        <tr>
                          <td colSpan={9} className="p-4 text-center text-xs font-bold text-slate-400 uppercase">Không có dữ liệu cơ sở</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left whitespace-nowrap table-auto border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        ...
                        return (
                          <tr key={stat.id} className="hover:bg-slate-50/50 transition-colors duration-150 group/row text-xs font-semibold text-slate-700">
                            <td className="py-3.5 px-4">
                              <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-lg bg-teal-50 text-teal-700 flex items-center justify-center border border-transparent shadow-sm">
                                  <Layers className="w-3.5 h-3.5" />
                                </div>
                                <span className="text-sm font-bold text-slate-800 group-hover/row:text-slate-900 transition-colors">{stat.batchName}</span>
                              </div>
                            </td>
                            <td className="py-3.5 px-4 text-center">
                              <span className="font-extrabold text-slate-900 text-sm">{stat.total}</span>
                            ...
                            <td className="py-3.5 px-4 text-right">
                              <div className="flex items-center justify-end gap-2.5">
                                ...
                                <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                  <div className="h-full bg-teal-500 rounded-full transition-all duration-300" style={{ width: `${batchSurveyRate}%` }}></div>
                                </div>
                              </div>
                            </td>
                            <td className="py-3.5 px-4 text-right">
                      ...
                      {batchStats.length === 0 && (
                        <tr>
                          <td colSpan={9} className="p-4 text-center text-xs font-bold text-slate-400 uppercase">Không có dữ liệu đợt khảo sát</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
            ...
            <>
                              ...
                              <button
                                ...
                                className={`px-3 py-1 rounded-xl text-xs font-black transition-all ${
                                  isSelected
                                    ...
                                    : 'bg-white text-[#00A99D] border border-[#00A99D] hover:bg-[#00A99D] hover:text-white'
                                }`}
                              >
                                Chi tiết
                              </button>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm relative overflow-hidden mb-4 text-left">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative z-10">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-gradient-to-tr from-indigo-600 to-violet-500 rounded-xl flex items-center justify-center text-white font-black text-lg shadow-md shrink-0">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 font-semibold mt-0.5">
                        Đối tượng TS: <strong className="text-slate-600 font-bold">{selectedReportStudent.targetType || "—"}</strong>
                      </p>
                    </div>
                  </div>

                  {/* Right Part: Grid of Metadata */}
                  <div className="flex-1 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-y-2 gap-x-3 border-t lg:border-t-0 lg:border-l border-slate-100 pt-3 lg:pt-0 lg:pl-4 text-[11px]">
                    <div className="flex flex-col">
                      <span className="text-slate-400 font-black uppercase text-[7px] tracking-wider mb-0.5">Ngày sinh</span>
                      <span className="font-bold text-slate-800">{selectedReportStudent.dateOfBirth ? new Date(selectedReportStudent.dateOfBirth).toLocaleDateString("vi-VN") : "—"}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-slate-400 font-black uppercase text-[7px] tracking-wider mb-0.5">Giới tính</span>
                      <span className="font-bold text-slate-800">{selectedReportStudent.gender === "M" || selectedReportStudent.gender === "Nam" ? "Nam" : selectedReportStudent.gender === "F" || selectedReportStudent.gender === "Nữ" ? "Nữ" : selectedReportStudent.gender || "—"}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-slate-400 font-black uppercase text-[7px] tracking-wider mb-0.5">Khối học</span>
                      <span className="font-black text-slate-900">K{selectedReportStudent.grade || "—"}</span>
                    </div>
                    <div className="flex flex-col">
                    ...
                    <div className="flex flex-col">
                      <span className="text-slate-400 font-black uppercase text-[7px] tracking-wider mb-0.5">Diện Khảo sát</span>
                      <span className="font-bold text-slate-700 truncate" title={selectedReportStudent.admissionCriteria}>{selectedReportStudent.admissionCriteria || "—"}</span>
                    </div>

                    {reportSelPeriod?.name?.toLowerCase().includes("open day") && (
                      <>
                        <div className="flex flex-col col-span-2">
                          <span className="text-slate-400 font-black uppercase text-[7px] tracking-wider mb-0.5">Đăng ký CS</span>
                          <span className="font-bold text-slate-800">
                            {campuses.find(c => c.id === selectedReportStudent.registeredCampus)?.campusName || selectedReportStudent.registeredCampus || "—"}
                          </span>
                        </div>
                        <div className="flex flex-col col-span-2">
                          <span className="text-slate-400 font-black uppercase text-[7px] tracking-wider mb-0.5">Ủy quyền xét duyệt</span>
                          <span className="font-bold text-slate-800">
                  ...
                  <div className="flex items-start lg:items-end flex-col shrink-0 lg:border-l border-slate-100 lg:pl-4 pt-3 lg:pt-0">
                    ...
                    {selectedReportStudent.signatureName && (
                      <span className="text-[9px] text-slate-400 font-semibold mt-0.5">
                        Ký: <strong className="text-slate-600 font-bold">{selectedReportStudent.signatureName}</strong>
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
                ...
                {/* LEFT COLUMN: ADMISSION DECISION FORM & RETEST HISTORY */}
                <div className="lg:col-span-4 space-y-4 lg:sticky lg:top-20 self-start transition-all duration-300">
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm space-y-3 text-left">
                    <h4 className="font-black text-slate-800 text-xs flex items-center justify-between border-b pb-2 mb-1">
                      ...
                      {selectedReportStudent.admissionResult ? (
                        ...
                        <span className="px-2 py-0.5 text-[9px] font-black bg-slate-100 text-slate-500 rounded-full border border-slate-200 uppercase tracking-wider">
                          Chưa duyệt
                        </span>
                      )}
                    </h4>
                      ...
                      <Field label="Môn Cam Kết">
                        <div className="grid grid-cols-2 gap-1.5 p-3 max-h-36 overflow-y-auto text-xs font-semibold">
                          {initialSubjects.map(sub => {
                            ...
                            return (
                              <label key={sub.id} className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-700 cursor-pointer hover:text-indigo-600 transition-colors">
                                <input 
                                  type="checkbox"
                                  checked={isChecked}
                                  disabled={!canApprove}
                  ...
                  {Array.isArray(retestHistory) && retestHistory.length > 1 && (
                    <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm space-y-3 animate-fade-in text-left">
                      ...
                      <div className="relative pl-3 border-l-2 border-slate-100 space-y-4 my-1 max-h-[260px] overflow-y-auto pr-1 scrollbar-thin">
                        {retestHistory.filter(Boolean).map((hist, idx) => {
                          ...
                          return (
                            <div key={hist?.id || idx} className="relative group text-left">
                              {/* Timeline node dot */}
                              <span className={`absolute -left-[19px] top-1.5 w-2 h-2 rounded-full border ${
                                isCurrent ? "bg-indigo-600 border-indigo-200 scale-125 ring-2 ring-indigo-50 animate-pulse" : "bg-slate-300 border-white group-hover:bg-indigo-400 transition-all duration-300"
                              }`} />
                              ...
                              <div className="space-y-1">
                                ...
                                {/* Past notes */}
                                {!isCurrent && sNote && (
                                  <p className="text-[9px] text-slate-500 italic mt-0.5 font-semibold bg-slate-100/50 p-2 rounded-xl border border-slate-200/50 leading-relaxed">
                                    "{sNote.trim()}"
                                  </p>
                                )}
                              </div>
                ...
                {/* RIGHT COLUMN: SUBJECT RESULTS SUMMARY & ACTIVE SUBJECT DETAILS */}
                <div className="lg:col-span-8 space-y-4">
                  <div className="flex items-center justify-between bg-white px-4 py-2.5 rounded-2xl border border-slate-200/80 shadow-sm text-left">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <h3 className="font-black text-slate-800 text-sm leading-snug">Kết quả khảo sát các môn</h3>
                  ...
                  {(!selectedReportStudent.scores || selectedReportStudent.scores.length === 0) ? (
                    <div className="bg-white border border-slate-200/80 rounded-2xl p-12 text-center text-slate-455 shadow-sm flex flex-col items-center justify-center gap-2">
                      ...
                      <div>
                        <p className="font-black text-slate-700 text-xs">Chưa có kết quả khảo sát</p>
                        <p className="text-[11px] text-slate-400 font-semibold mt-0.5">Học sinh này chưa có kết quả đánh giá môn học nào.</p>
                      </div>
                    </div>
                  ) : (
                    <div className="space-y-4">
                                ...
                                <table className="w-full text-left table-fixed border-collapse">
                                  ...
                                  <tbody className="divide-y divide-slate-100">
                                    {scoresList.map((sc) => {
                                      ...
                                      // Extract primary comment
                                      let commentPreview = "—";
                                      try {
                                        if (sc.comments) {
                                          const parsed = JSON.parse(sc.comments);
                                          const commentVals = Array.isArray(parsed) ? parsed : [parsed];
                                          const firstComment = commentVals.find(c => c && c.trim() !== "");
                                          if (firstComment) commentPreview = firstComment;
                                        }
                                      } catch {}
                            ...
                            <div className="space-y-4">
                              {scoresList
                                ...
                                .map((sc) => {
                                    ...
                                    return (
                                      <div key={sc.id} className="p-4 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden text-left animate-fade-in text-xs font-semibold">
                                        ...
                                        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3 mb-4">
                                          <div>
                                            <h4 className="font-black text-violet-900 text-sm leading-none">Tổng điểm Tiếng Anh</h4>
                                  ...
                                  return (
                                    <div key={sc.id} className={`${detailCardClass} rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden text-left animate-fade-in`}>
                                        ...
                                        <div>
                                          <div className="flex items-center gap-2">
                                            ...
                                            {subject.subjectType === "VIET_NAM" && (
                                              <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full border shadow-sm bg-[#00A99D]/5 text-[#00A99D] border-[#00A99D]/20`}>
                                                Giáo viên Việt Nam
                                              </span>
                                            )}
                                          </div>
                                          <div className="flex items-center gap-2 text-[11px] text-slate-400 font-semibold mt-1.5">
                                            <div className="w-4 h-4 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 font-black text-[8px] border shadow-inner select-none shrink-0">
                                              {sc.teacherName?.charAt(0) || "—"}
                                            </div>
                                            <span>GV chấm: <strong className="text-slate-600 font-bold">{sc.teacherName || "Chưa xác định"}</strong></span>
                                      ...
                                      {isPsych ? (
                                            ...
                                            return (
                                              <div className="space-y-3">
                                                ...
                                                <div className="flex items-center justify-between p-3 shadow-inner text-xs font-semibold">
                                                  ...
                                                  <div className={`px-3 py-1.5 rounded-xl border font-black text-[11px] uppercase tracking-wider shadow-sm flex items-center gap-1.5 ${activeLvl.bg}`}>
                                                    <span className={`w-1.5 h-1.5 rounded-full ${activeLvl.activeBg} animate-pulse`}></span>
                                                ...
                                                <div className="space-y-1.5">
                                                  <div className="flex items-center justify-between">
                                                    <span className="text-[9px] font-black uppercase tracking-widest text-slate-455">Thang đo chẩn đoán lâm sàng</span>
                                                    <span className="text-[9px] font-bold text-slate-400">Điểm tối đa: 80</span>
                                                  </div>
                                                    ...
                                                    {levels.map(l => {
                                                      ...
                                                      return (
                                                        <div key={l.name} className="space-y-1">
                                                          <div className={`h-1.5 rounded-full ${isActive ? l.activeBg : l.scaleBg} transition-all duration-500 ${isActive ? "shadow-md scale-y-110" : ""}`}></div>
                                                          <div className="text-center">
                                                            <span className={`text-[7px] leading-tight font-black block truncate ${isActive ? "text-slate-800 font-bold" : "text-slate-400"}`}>
                                                              {l.name}
                                                            </span>
                                                            <span className="text-[6px] text-slate-355 font-semibold block mt-0.5">{l.min}-{l.max}</span>
                                                          </div>
                                                        </div>
                                                      );
                                                    })}
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-left">
                                            <div className="space-y-1 p-3 text-xs font-semibold">
                                              ...
                                              <p className={`text-xs font-medium leading-relaxed whitespace-pre-wrap ${commentVals[0] ? "text-slate-700" : "text-slate-450 italic"}`}>
                                                {commentVals[0] ? `"${commentVals[0]}"` : "Chưa cập nhật nội dung nhận định."}
                                              </p>
                                            </div>
                                            ...
                                            <div className="space-y-1 p-3 text-xs font-semibold">
                                              ...
                                              <p className={`text-xs font-medium leading-relaxed whitespace-pre-wrap ${commentVals[1] ? "text-amber-800" : "text-slate-400 italic"}`}>
                                                {commentVals[1] ? `"${commentVals[1]}"` : "Chưa có khuyến nghị cụ thể."}
                                              </p>
                                            </div>
                                          </div>
                                        </div>
                                        ...
                                        <div className="space-y-4">
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-left">
                                            ...
                                            {scoreVals.some(v => v === "1") && (
                                              <div className="bg-slate-55 border border-slate-200 rounded-xl p-3">
                                        ...
                                        <div className="space-y-4">
                                          ...
                                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                            {[
                                              { label: "Logic", val: scoreVals[0], color: "text-indigo-600 bg-indigo-50 border-indigo-100", barColor: "bg-indigo-500" },
                                              { label: "Sáng tạo", val: scoreVals[1], color: "text-violet-600 bg-violet-50 border-violet-100", barColor: "bg-violet-500" },
                                              { label: "Phản biện", val: scoreVals[2], color: "text-cyan-600 bg-cyan-50 border-cyan-100", barColor: "bg-cyan-500" },
                                              { label: "Giải quyết VĐ", val: scoreVals[3], color: "text-teal-600 bg-teal-50 border-teal-100", barColor: "bg-teal-500" }
                                            ].map(item => {
                                              let pctVal = 0;
                                              if (item.val) {
                                              ...
                                              return (
                                                <div key={item.label} className={`border rounded-xl p-2.5 text-center flex flex-col justify-between shadow-sm relative overflow-hidden group ${item.color}`}>
                                                  <span className="text-[9px] font-black uppercase tracking-wider text-slate-555 block leading-tight">{item.label}</span>
                                                  <div className="text-base font-black text-slate-800 mt-1 mb-1 leading-none">{item.val || "—"}</div>
                                                  
                                                  {/* Micro Progress Bar */}
                                                  <div className="w-full bg-slate-100 rounded-full h-1 mt-auto">
                                                    <div className={`h-1 rounded-full ${item.barColor}`} style={{ width: `${Math.min(pctVal || 0, 100)}%` }}></div>
                                                  </div>
                                                </div>
                                              );
                                            })}
                                          ...
                                          <div className="bg-gradient-to-r from-indigo-50 to-violet-50/50 border border-indigo-100 rounded-xl p-3 flex items-center justify-between shadow-sm text-left">
                                            <div className="flex items-center gap-2">
                                              ...
                                              <div>
                                                <span className="text-[9px] font-black uppercase tracking-widest text-indigo-555">Hoàn thành thử thách</span>
                                                <p className="text-[10px] text-slate-555 font-bold mt-0.5">Mức độ hoàn thành nhiệm vụ</p>
                                              </div>
                                            </div>
                                        ...
                                        <div className="space-y-3">
                                          ...
                                          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 text-left">
                                            {Array.from({length: (subject.scoreColumns ?? 1)}).map((_, colIdx) => {
                                              ...
                                              return (
                                                <div key={colIdx} className={`border rounded-xl px-3 py-2 text-center shadow-sm flex flex-col justify-between ${isTotal ? "bg-indigo-50/50 border-indigo-150 text-indigo-700" : "bg-slate-50 border-slate-200/60 text-slate-600"}`}>
                                                  <span className="text-[9px] font-black uppercase tracking-wider text-slate-400 truncate block leading-tight">{colName}</span>
                                                  <div className={`text-lg font-black mt-1 leading-none ${isTotal ? "text-indigo-700" : "text-slate-800"}`}>
                                                    {displayVal}
                                                  </div>
                                                </div>
                                              );
                                                  ...
                                                  return (
                                                    <div key={colIdx} className="bg-white p-2.5 rounded-xl border-2 border-emerald-100/60 shadow-sm flex items-start gap-2.5 text-left animate-fade-in">
                                                      ...
                                                      <div className="flex-1">
                                                        <span className="text-[8px] font-black uppercase tracking-wider text-indigo-600 block mb-0.5">{colName}</span>
                                                        <p className="text-xs font-semibold text-slate-700 italic leading-relaxed">"{val}"</p>
                                                      </div>
                                                    </div>
                                                  );
                                                })}
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-[#00A99D]/20 focus:border-[#00A99D] outline-none bg-white cursor-pointer font-semibold"
                >
