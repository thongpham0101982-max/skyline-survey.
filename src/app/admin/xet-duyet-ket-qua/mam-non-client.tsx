...
export function XetDuyetMamNonClient({ academicYears, campuses, giaoVuCSUsers, grades: gradesProp, teachers, departments, currentUser, classes = [] }: { academicYears: AcademicYear[]; campuses: Camp[]; giaoVuCSUsers: any[]; grades: string[]; teachers: any[]; departments: any[]; currentUser: any; classes?: any[]; }) {
      ...
      return (
        ...
        >
          ...
          <div className="flex items-center gap-1.5 pt-1.5 border-t border-slate-100">
            <div className="w-5 h-5 rounded-full bg-[#00A99D]/10  flex items-center justify-center text-[8px] font-black text-slate-800 shrink-0">
              {initials}
            </div>
  ...
  const buildPreschoolLetterHtml = (student: any, config: any, isCommitmentFlag: boolean = false, isInvitationFlag: boolean = false) => {
    ...
    const logoHtml = config.logo 
      ...
      : '<svg class="logo-svg" style="height: 48px; fill: #00A99D;" viewBox="0 0 260 50"><text x="0" y="38" font-family="Arial, sans-serif" font-weight="900" font-size="34" letter-spacing="-1">SKY-LINE</text><circle cx="178" cy="26" r="6" /></svg>';
    ...
    const customFooterHtml = config.footer ? getImgTag(config.footer, "footer-img", "width: 100%; max-height: 100px; object-fit: contain;", "Footer") :
      '<div style="width: 100%; font-family: Arial, sans-serif; box-sizing: border-box; text-align: left;">' +
        '<div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px; width: 100%;">' +
          '<span style="font-weight: bold; color: #00A99D; white-space: nowrap; text-transform: uppercase; font-size: 9.5pt; letter-spacing: 0.5px;">HỆ THỐNG GIÁO DỤC SKY-LINE</span>' +
          '<div style="flex-grow: 1; border-top: 1px solid rgba(0, 166, 169, 0.7); height: 0; margin-top: 2px;"></div>' +
          '<span style="font-weight: 600; color: #00A99D; white-space: nowrap; text-transform: lowercase; font-size: 9pt;">www.skylineschool.edu.vn</span>' +
        '</div>' +
        '<div style="display: flex; justify-content: space-between; font-size: 7.5pt; color: #555555; position: relative; width: 100%;">' +
          '<div style="width: 32%; display: flex; flex-direction: column; gap: 4px;">' +
            '<div>' +
              '<p style="font-weight: bold; color: #00A99D; margin: 0; font-size: 8pt; line-height: 1.25;">SKY-LINE Riverside</p>' +
              '<p style="color: #555555; margin: 2px 0 0 0; line-height: 1.25;">Lô A2.4 Trần Đăng Ninh, Q. Hải Châu, TP. Đà Nẵng</p>' +
            '</div>' +
            '<div>' +
              '<p style="font-weight: bold; color: #00A99D; margin: 0; font-size: 8pt; line-height: 1.25;">SKY-LINE Central</p>' +
              '<p style="color: #555555; margin: 2px 0 0 0; line-height: 1.25;">Số 48 Nguyễn Du, Q. Hải Châu, TP. Đà Nẵng</p>' +
            '</div>' +
          '</div>' +
          '<div style="width: 32%; display: flex; flex-direction: column; gap: 4px;">' +
            '<div>' +
              '<p style="font-weight: bold; color: #00A99D; margin: 0; font-size: 8pt; line-height: 1.25;">SKY-LINE Global</p>' +
              '<p style="color: #555555; margin: 2px 0 0 0; line-height: 1.25;">Lô A2 Trần Đăng Ninh, Q. Hải Châu, TP. Đà Nẵng</p>' +
            '</div>' +
            '<div>' +
              '<p style="font-weight: bold; color: #00A99D; margin: 0; font-size: 8pt; line-height: 1.25;">SKY-LINE Beach</p>' +
              '<p style="color: #555555; margin: 2px 0 0 0; line-height: 1.25;">Số 199 Trần Anh Tông, Q. Thanh Khê, TP. Đà Nẵng</p>' +
            '</div>' +
          '</div>' +
          '<div style="width: 32%; display: flex; flex-direction: column; gap: 4px;">' +
            '<div>' +
              '<p style="font-weight: bold; color: #00A99D; margin: 0; font-size: 8pt; line-height: 1.25;">SKY-LINE Hill</p>' +
            ...
            '</div>' +
          '</div>' +
          '<div style="position: absolute; right: -4px; top: -4px; width: 50px; height: 38px; pointer-events: none; display: flex; align-items: center; justify-content: center; color: #00A99D;">' +
    ...
    const bgHtml = config.background 
      ...
      : '<svg class="print-watermark" style="display: block; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 110mm; height: auto; opacity: 0.04; z-index: 1; pointer-events: none;" viewBox="0 0 100 100"><path fill="#00A99D" d="M10,80 Q50,40 90,20 Q60,50 10,80 Z" /><path fill="#00A99D" d="M30,80 Q60,55 90,35 Q65,60 30,80 Z" /></svg>';
      ...
      '<body>' +
        '<div class="a4-page">' +
          ...
          '<div style="position: relative; z-index: 10; display: flex; flex-direction: column; flex-grow: 1;">' +
            '<div class="header-container" style="display: flex; flex-direction: column; border-bottom: 1.5px solid #00A99D; padding-bottom: 8px; margin-bottom: 24px;">' +
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={fetchPeriods} className="p-2 text-slate-400 hover:text-slate-800 hover:bg-[#00A99D]/5 rounded-none transition-all"><RefreshCw className="w-4 h-4" /></button>
              <button onClick={openAddPeriod} className="flex items-center gap-2 px-5 py-2.5 bg-[#00A99D] text-white text-[13px] font-black rounded-none hover:bg-[#00A99D]-700 transition-all shadow-none shadow-teal-100"><Plus className="w-4 h-4" /> Tạo Kỳ mới</button>
            </div>
          </div>
                ...
                <div key={p.id} className="bg-white rounded-none border border-slate-300 shadow-none overflow-hidden hover:border-slate-300 transition-all">
                  <div className="px-4 py-3.5 flex flex-wrap items-center justify-between gap-3 cursor-pointer" onClick={() => setExpandedId(expandedId === p.id ? null : p.id)}>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button onClick={e => { e.stopPropagation(); openEditPeriod(p); }} className="p-2 text-slate-400 hover:text-slate-800 hover:bg-[#00A99D]/5 rounded-none transition-all"><Edit2 className="w-4 h-4" /></button>
                  ...
                  {expandedId === p.id && (
                    <div className="border-t border-slate-300 p-5 bg-[#00A99D]/5/30">
                              ...
                              {p.batches.map(b => {
                                ...
                                return (
                                  <tr key={b.id} className="hover:bg-[#00A99D]/5/30 transition-colors">
                                    <td className="p-2 border border-slate-200"><span className="inline-flex items-center justify-center w-8 h-8 rounded-none bg-[#00A99D]/5 font-black text-slate-800 text-xs">#{b.batchNumber}</span></td>
                                    ...
                                    <td className="p-2 border border-slate-200"><div className="flex items-center gap-2">
                                       <button
                                         ...
                                         className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#00A99D] focus:ring-offset-2 ${
                                      ...
                                      <div className="flex items-center justify-between gap-2 group/assignee">
                                        ...
                                        {b.assignedUserId && (
                                          <button
                                            ...
                                            className="p-1 text-slate-400 hover:text-slate-800 hover:bg-[#00A99D]/5 rounded-md transition-all ml-2"
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="p-2 text-slate-400 hover:text-slate-800 hover:bg-[#00A99D]/5 rounded-none transition-all"
              >
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="bg-white rounded-3xl border border-slate-300 shadow-none p-5 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  {aActiveBatch && (() => {
                    ...
                    return (
                      <div className="bg-gradient-to-br from-[#00A99D]/5 via-white to-teal-50/10 border-l-4 border-l-[#00A99D] border-y border-r border-slate-300 rounded-none p-4 space-y-3.5 shadow-none animate-in fade-in slide-in-from-top-2 duration-300">
                        <div className="flex items-center gap-2 border-b border-[#00A99D]/10 pb-2">
                        ...
                        <div className="space-y-2">
                          ...
                          {mappings.map((m, idx) => (
                            <div key={idx} className="grid grid-cols-12 gap-2 items-center bg-white border border-teal-50 px-3 py-2.5 rounded-none shadow-2xs hover:border-[#00A99D]/30 hover:shadow-xs transition-all duration-200">
                        ...
                        return (
                          <label 
                            ...
                            className={`flex items-center gap-2.5 p-3 rounded-none border cursor-pointer transition-all ${
                              isChecked 
                                ? "bg-[#00A99D]/5 border-[#00A99D] text-slate-800 font-extrabold shadow-xs" 
              ...
              <div className="bg-white rounded-3xl border border-slate-300 shadow-none p-5 space-y-4">
                <div className="flex items-center justify-between pb-2 border-b border-slate-300">
                  ...
                  <span className="text-[10px] bg-[#00A99D]/5 text-slate-800 px-2 py-0.5 rounded-none font-black">
                  ...
                  {teachers
                    ...
                    .map(t => {
                      ...
                      return (
                        <label 
                          key={t.id} 
                          className={`flex items-center justify-between p-3 rounded-none cursor-pointer border transition-all ${isChecked ? "bg-[#00A99D]/5/50 border-slate-300" : "bg-white border-slate-300 hover:border-slate-300 hover:bg-slate-50/30"}`}
                        >
                            ...
                            <div>
                              <div className="text-sm font-bold text-slate-800 flex items-center gap-2">
                                ...
                                {t.departmentRel?.name && (
                                  <span className="text-[9px] font-black uppercase bg-[#00A99D]/5 text-slate-800 border border-slate-300 px-1.5 py-0.5 rounded-none tracking-wider">
                                    {t.departmentRel.name}
                                  </span>
                                )}
                              </div>
                ...
                <button 
                  ...
                  className="w-full py-3.5 bg-[#00A99D] text-white font-black rounded-none hover:bg-[#00A99D]-700 disabled:opacity-50 transition-all shadow-none shadow-teal-100/50 flex items-center justify-center gap-2 mt-4 hover:scale-[1.01] active:scale-[0.99] duration-150"
                >
                  {aSaving ? (
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="flex gap-1.5 mt-1 items-center">
                                <span className="text-[9px] font-black uppercase bg-[#00A99D]/5 text-slate-800 border border-slate-300 px-2 py-0.5 rounded-none tracking-wider">
                                  {assign.grade}
                                </span>
                                {assign.batch && (
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => openAddCfg("criteria")} className="flex items-center gap-2 px-5 py-2.5 bg-[#00A99D] text-white text-[13px] font-black rounded-none hover:bg-[#00A99D]-700 transition-all shadow-none shadow-teal-100"><Plus className="w-4 h-4" /> Thêm mục</button>
            </div>
          </div>
          {cfgLoading ? <Spin /> : (
                ...
                <div className="overflow-x-auto custom-scrollbar flex-1">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-teal-50">
                      {configs.map((c, i) => (
                        <tr key={c.id} className="hover:bg-[#00A99D]/5/30 transition-colors">
                          ...
                          <td className="p-2 border border-slate-200"><span className="text-[10px] font-black text-slate-800 bg-[#00A99D]/5 px-2 py-0.5 rounded-none  uppercase">{c.categoryType}</span></td>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-xs font-black tracking-wider transition-all duration-300 border-none cursor-pointer ${
                devTab === "stats" 
                  ...
                  : "bg-transparent text-slate-500 hover:text-slate-800 hover:bg-[#00A99D]/5"
              }`}
            >
...
<div className="group relative w-full">
                ...
                <div className="flex gap-2">
                  <button 
                    onClick={fetchStudentSummaries} 
                    className="h-10 px-4 text-xs font-black uppercase text-white bg-[#00A99D] hover:bg-[#008F89] hover:scale-[1.01] rounded-2xl shadow-sm hover:shadow-md shadow-[#00A99D]/15 transition-all flex items-center justify-center gap-1.5 cursor-pointer border-none w-full"
                  >
                              ...
                              <tr key={s.id} className={`group border-b border-slate-100 hover:bg-[#EBF5F4]/40 transition-colors duration-150 ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/40"} text-xs font-semibold`}>
                                ...
                                <td className="w-[170px] min-w-[170px] max-w-[170px] sticky left-10 z-10 shadow-[1px_0_0_0_#e2e8f0] whitespace-normal py-4 px-4 align-middle bg-inherit border border-slate-100">
                                  <div className="flex flex-col gap-1.5 w-full">
                                    ...
                                    <div className="text-[10px] text-slate-800 font-mono font-bold bg-[#00A99D]/5 px-2 py-0.5 rounded border border-[#00A99D]/15 w-fit">{s.studentCode}</div>
                                      ...
                                      <div className="flex flex-col gap-1.5 w-full items-start">
                                        {s.probationaryTeacher ? (
                                          <div className="flex flex-col gap-1 w-full items-start">
                                            ...
                                            <button onClick={() => openAssignModal(s)} className="inline-flex items-center justify-center gap-1 px-2.5 py-1 text-[9px] font-extrabold text-slate-800 bg-teal-50/50 hover:bg-[#00A99D]/10 active:scale-[0.98] rounded-lg border border-[#00A99D]/30 transition-all cursor-pointer">
                                              Phân công lại
                                            </button>
                                          </div>
                                        ) : (
                                  ...
                                  {s.probationaryTeacher ? (
                                    <div className="flex flex-col gap-1 items-start">
                                      ...
                                      <button
                                        ...
                                        className="inline-flex items-center justify-center gap-1 px-2 py-0.5 text-[9px] font-bold text-slate-800 hover:bg-[#00A99D]/10 rounded  transition-all w-fit cursor-pointer"
                                      >
                                        Phân công lại
                                      </button>
                                    </div>
                                    ...
                                    <button
                                      onClick={() => openAssignModal(s)}
                                      className="inline-flex items-center justify-center gap-1 px-3 py-1.5 text-[10px] font-extrabold text-white bg-[#00A99D] hover:bg-[#008F89] hover:scale-[1.02] rounded-xl transition-all shadow-xs w-fit border-none cursor-pointer"
                                    >
                                      Phân công
                                    </button>
                                  )}
              ...
              <div className="flex gap-2 p-1 bg-slate-100 rounded-none w-fit mb-2">
                ...
                >
                  Tiêu chí Học thử (Probation)
                            ...
                            <div className="flex flex-wrap gap-1.5 bg-[#00A99D]/5/50 p-1.5 rounded-none border border-slate-300 max-w-fit">
              ...
              {devType === "INPUT" && (() => {
                ...
                const renderGradeTags = (gradesStr: string, isPhase1: boolean) => {
                  ...
                  const tagBg = isPhase1 
                    ? "bg-[#00A99D]/5/60 text-teal-800 border-slate-300 hover:bg-teal-100/40" 
                  ...
                  <div className="bg-gradient-to-br from-teal-50/40 via-white to-indigo-50/20 border border-slate-300/60 rounded-3xl p-5 shadow-none max-w-4xl my-4 hover:shadow-none transition-all duration-300 relative overflow-hidden animate-fadeIn">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      ...
                      <div className="bg-[#00A99D]/5/20 border border-slate-300/40 rounded-none p-4 flex flex-col justify-between hover:border-slate-300/60 transition-all">
                        <div>
                          <div className="flex items-center justify-between">
                            <span className="bg-[#00A99D]/10 text-slate-800 px-2.5 py-0.5 rounded-none text-[9px] font-extrabold uppercase tracking-wider ">
                              Giai đoạn 1
                            </span>
                ...
                <div className="space-y-3">
                  {devAreas.map(area => (
                    <div key={area.id} className="bg-white rounded-none border border-slate-300 shadow-none overflow-hidden">
                      <div
                        className="px-4 py-3.5 flex items-center justify-between gap-3 cursor-pointer hover:bg-[#00A99D]/5/20 transition-all"
                      ...
                      {expAreaId === area.id && (
                        <div className="border-t border-slate-300 p-4 bg-[#00A99D]/5/10">
                              ...
                              <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-teal-50">
                                  {area.criteria.map((crit, idx) => (
                                    <tr key={crit.id} className="hover:bg-[#00A99D]/5/30 transition-colors">
                                      ...
                                      <td className="p-2 border border-slate-200"><span className="text-[10px] font-black text-slate-800 bg-[#00A99D]/5 px-2 py-0.5 rounded-none ">{crit.ageGroup}</span></td>
                                      <td className="p-2 text-right border border-slate-200">
                                        <div className="flex gap-1">
                                          <button
                                            onClick={() => openEditCriteria(crit)}
                                            className="p-1.5 text-slate-300 hover:text-slate-800 hover:bg-[#00A99D]/5 rounded-none"
                                          >
                ...
                {rptStats.gradeStats.map(g => {
                  ...
                  return (
                    <div key={g.grade} className="space-y-1.5">
                      ...
                      <div className="w-full bg-[#00A99D]/5 h-3 rounded-none overflow-hidden border border-slate-200"><div className="h-full bg-[#00A99D] transition-all duration-700" style={{ width: `${pct}%` }} /></div>
                    </div>
                  );
                })}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
          ...
          <div className="lg:col-span-5 bg-white border border-slate-300 shadow-none rounded-3xl p-8 space-y-6">
            <div className="flex items-center gap-3 border-b pb-4 mb-2">
              <div className="w-10 h-10 bg-[#00A99D]/5 text-slate-800 rounded-none flex items-center justify-center">
            ...
            <button onClick={savePreschoolReportConfig} className="w-full py-4 bg-[#00A99D] hover:bg-[#00A99D]-700 text-white font-black text-xs uppercase tracking-widest rounded-none shadow-none shadow-teal-100 transition-all flex items-center justify-center gap-2">
          ...
          <div className="lg:col-span-7 shadow-inner p-8 flex flex-col min-h-[500px] text-xs font-semibold">
            ...
            {/* Info note */}
            <div className="mt-4 bg-[#00A99D]/5 border border-slate-300 rounded-none p-3">
      ...
      {/* Modal: Batch */}
      <Modal open={bModal} onClose={() => setBModal(false)} title={editB ? "Sửa Đợt" : "Thêm Đợt mới"} footer={<><button onClick={() => setBModal(false)} className="flex-1 text-xs font-black uppercase text-slate-400 hover:text-slate-600">Hủy</button><button onClick={saveBatch} className="flex-1 py-3.5 bg-[#00A99D] text-white rounded-none text-xs font-black uppercase tracking-widest shadow-none shadow-teal-100">Lưu</button></>}>
        <div className="space-y-4">
      ...
      {/* Modal: Child */}
      <Modal open={cModal} onClose={() => setCModal(false)} title={editC ? "Sửa thông tin Bé" : "Thêm Bé Mầm non"} size="lg" footer={<><button onClick={() => setCModal(false)} className="flex-1 text-xs font-black uppercase text-slate-400">Đóng</button><button onClick={saveChild} className="flex-1 py-3.5 bg-[#00A99D] text-white rounded-none text-xs font-black uppercase tracking-widest shadow-none shadow-teal-100">Lưu</button></>}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Mã bé" required><input value={cForm.studentCode} onChange={e => setCForm({...cForm, studentCode: e.target.value})} disabled={!!editC} className={inp + (editC ? " bg-[#00A99D]/5/50" : "")} placeholder="MN001" /></Field>
          ...
          <Field label="Ngày sinh">
            ...
            />
            {ageInfo.months !== null && (
              <div className="text-[11px] font-black text-slate-800 mt-1.5 uppercase tracking-wider bg-[#00A99D]/5/50 rounded-none px-3 py-1.5 border border-slate-300 flex items-center gap-1.5 animate-in fade-in duration-200">
          ...
          <div className="bg-white rounded-3xl border border-slate-300 shadow-2xl max-w-2xl w-full overflow-hidden flex flex-col max-h-[90vh] animate-scale-up">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2 text-[10px] font-bold">
                        <button 
                          ...
                          className="text-slate-800 hover:text-violet-800 bg-[#00A99D]/5 px-2.5 py-1 rounded-none border border-slate-300 transition-colors cursor-pointer"
                        >
                          Chọn tất cả
                        </button>
                        <button 
                          ...
                          <div key={role} className="p-4 space-y-2">
                            <div className="flex items-center justify-between">
                              <span className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-none border tracking-wider ${
                                ...
                                role === "Giáo viên" ? "bg-[#00A99D]/5 text-slate-800 border border-[#00A99D]/30" :
                                ...
                                return (
                                  <label 
                                    ...
                                    className={`flex items-start gap-2.5 p-2 rounded-none border transition-all cursor-pointer select-none text-xs font-semibold ${
                                      r.checked 
                                        ? "bg-[#00A99D]/5/50 border-slate-300 text-violet-900 shadow-none" 
            ...
            <div className="p-4 flex items-center justify-end gap-3 text-xs font-semibold">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-6 py-2 text-xs font-black text-white bg-[#00A99D] hover:bg-[#00A99D]-700 rounded-none transition-all shadow-none shadow-teal-100 hover:shadow-violet-200 cursor-pointer"
                    ...
                    {[
                      ...
                      return (
                        <button
                          ...
                          className={"px-2 py-1 rounded-none text-[10px] font-bold border transition-all " + (isActive ? 'bg-[#00A99D]/10 border-indigo-200 text-indigo-700' : 'bg-slate-50 border-slate-300 text-slate-500 hover:bg-slate-100')}
                        >
                          + {p.label}
                        </button>
                      );
              ...
              >
                ...
                {studentCampusConfig?.background ? (
                  ...
                  <svg className="print-watermark" style={{ display: "block", position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "110mm", height: "auto", opacity: 0.04, zIndex: 0, pointerEvents: "none" }} viewBox="0 0 100 100">
                    <path fill="#00A99D" d="M10,80 Q50,40 90,20 Q60,50 10,80 Z" />
                    <path fill="#00A99D" d="M30,80 Q60,55 90,35 Q65,60 30,80 Z" />
                  </svg>
                )}
      ...
      >
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-[#EBF5F4]/60 via-[#F5FAF9]/20 to-[#EBF5F4]/40 p-5 rounded-3xl border border-[#00A99D]/15 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-sm">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#00A99D]/15 to-[#008075]/5 text-slate-800 flex items-center justify-center shadow-inner shrink-0">
              ...
              <div>
                ...
                <p className="text-[11px] font-extrabold text-slate-800 font-mono mt-1.5 bg-[#00A99D]/5 px-2 py-0.5 rounded-lg border border-[#00A99D]/10 w-fit">{evalStudent?.studentCode}</p>
              </div>
            </div>
                  ...
                  {showBghSection && (
                    <div className={`space-y-4 bg-white p-5 rounded-2xl border transition-all ${canApproveBGH ? 'border-[#00A99D]/20 shadow-xs' : 'border-slate-200 bg-slate-50/50 opacity-85'}`}>
                      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                        ...
                        {canApproveBGH && (
                          <span className="text-[9px] font-bold text-slate-800 bg-[#00A99D]/5 px-2 py-1 rounded-lg">
                            ✍️ Quyền duyệt
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          ...
                          { status: "DAT_HOC_THU", label: "ĐẠT - HỌC THỬ", color: "text-slate-800 border-[#00A99D]/20 hover:bg-teal-50/40 hover:border-[#00A99D]/40", activeColor: "bg-[#00A99D] text-white border-[#00A99D] shadow-xs" },
      ...
      >
        <div className="space-y-5">
          ...
          <div className="bg-gradient-to-r from-teal-50/90 via-emerald-50/60 to-teal-50/90 p-4 rounded-2xl border border-teal-200/70 shadow-sm flex flex-wrap items-center justify-between gap-4 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-[#00A99D]/10  flex items-center justify-center text-slate-800">
        ...
        footer={(
          <>
            ...
            <button
              ...
              className="flex-1 py-3.5 bg-[#00A99D] text-white rounded-none text-xs font-black uppercase tracking-widest"
            >
