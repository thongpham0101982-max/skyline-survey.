                  ...
                  <tbody className="divide-y divide-slate-200">
                    {filtChildren.map((child, i) => (
                      <tr key={child.id} className={`hover:bg-[#00A19A]/5/30 transition-colors ${cSelected.includes(child.id) ? "bg-[#00A19A]/5/50" : ""}`}>
                        <td className="p-2 border border-slate-300"><input type="checkbox" className="w-4 h-4 rounded accent-[#00A19A]" checked={cSelected.includes(child.id)} onChange={e => setCSelected(e.target.checked ? [...cSelected, child.id] : cSelected.filter(id => id !== child.id))} /></td>
