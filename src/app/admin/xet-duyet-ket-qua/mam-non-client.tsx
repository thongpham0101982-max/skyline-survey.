...
const getStatusBadge = (status: string | null | undefined) => {
  ...
  const badgeConfig: Record<string, { bg: string, text: string, border: string, label: string }> = {
    DAT_MIEN_HOC_THU: { bg: "bg-teal-50 text-slate-800 border-teal-200/60", label: "Miễn Học Thử" },
    MIEN_HOC_THU: { bg: "bg-teal-50 text-slate-800 border-teal-200/60", label: "Miễn Học Thử" },
    MIEN_HT: { bg: "bg-teal-50 text-slate-800 border-teal-200/60", label: "Miễn Học Thử" },
    ...
    DAT: { bg: "bg-emerald-50 text-slate-800 border-emerald-250/60", label: "Đạt" },
    KHONG_DAT: { bg: "bg-rose-50 text-slate-800 border-rose-250/60", label: "Không Đạt" },
    K_DAT: { bg: "bg-rose-50 text-slate-800 border-rose-250/60", label: "Không Đạt" },
    Y_KIEN_KHAC: { bg: "bg-amber-50 text-slate-800 border-amber-250/60", label: "Ý kiến khác" },
    Y_KIEN: { bg: "bg-amber-50 text-slate-800 border-amber-250/60", label: "Ý kiến khác" },
    CHUA_DANH_GIA: { bg: "bg-slate-50 text-slate-400 border-slate-200/60", label: "Chưa đánh giá" },
    DA_DANH_GIA: { bg: "bg-emerald-50 text-slate-800 border-emerald-200/60", label: "Đã đánh giá" }
  };
...
function Spin() {
  return <div className="flex items-center justify-center py-16"><div className="relative"><div className="w-12 h-12 rounded-full border-4 border-slate-300 border-t-teal-600 animate-spin"></div><Baby className="w-5 h-5 text-slate-800 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" /></div></div>;
}

function Empty({ text, sub }: { text: string; sub?: string }) {
  return <div className="flex flex-col items-center justify-center py-16 text-center"><div className="w-20 h-20 bg-[#36E08F]/5 rounded-full flex items-center justify-center mb-4"><Baby className="w-10 h-10 text-teal-400" /></div><p className="font-black text-slate-500 text-sm">{text}</p>{sub && <p className="text-xs text-slate-400 mt-1 font-medium">{sub}</p>}</div>;
}

function TypeBadge({ t }: { t: string }) {
  const map: Record<string, string> = {
    KHAO_SAT_LE: "bg-blue-50 text-blue-700 border border-blue-200",
    OPEN_DAY: "bg-[#36E08F]/5 text-purple-700 border border-purple-200"
  };
  const label: Record<string, string> = {
    KHAO_SAT_LE: "Khảo sát lẻ cơ sở",
    OPEN_DAY: "Khảo sát Open Day"
  };
  return <span className="text-[10px] font-bold text-blue-700 text-xs font-semibold">{label[t] || t}</span>;
}
...
function Field({ label, required, children }: any) {
  return <div className="space-y-1.5"><label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest">{label}{required && <span className="text-slate-800 ml-1">*</span>}</label>{children}</div>;
}
...
const renderAreaMiniCard = (s: any, areaCode: string) => {
  ...
  if (areaCode === "NHAN_THUC") {
    ...
    pillClass = "text-slate-800 bg-amber-50 border-amber-150/50";
    headerIcon = "🧩";
    ...
    pillClass = "text-slate-800 bg-rose-50 border-rose-150/50";
    headerIcon = "🎨";
  }
...
export function XetDuyetMamNonClient({ academicYears, campuses, giaoVuCSUsers, grades: gradesProp, teachers, departments, currentUser, classes = [] }: { academicYears: AcademicYear[]; campuses: Camp[]; giaoVuCSUsers: any[]; grades: string[]; teachers: any[]; departments: any[]; currentUser: any; classes?: any[]; }) {
  ...
  const filteredCampuses = useMemo(() => {
    return campuses.filter(c => !isGDCSUser || currentUser?.campusIds?.includes(c.id));
  }, [campuses, isGDCSUser, currentUser]);
  ...
  const renderApprovalCell = (s: any, isBGH: boolean) => {
    ...
    if (status) {
      ...
      return (
        <div 
          onClick={onOpen} 
          className="cursor-pointer hover:bg-slate-50/80 p-2 rounded-xl border border-slate-200 hover:border-[#36E08F]/40 transition-all w-full flex flex-col gap-1.5 bg-white shadow-2xs"
        >
          ...
          <div className="flex items-center gap-1.5 pt-1.5 border-t border-slate-100">
            <div className="w-5 h-5 rounded-full bg-[#36E08F]/10  flex items-center justify-center text-[8px] font-black text-slate-800 shrink-0">
              {initials}
            </div>
    ...
    return (
      <div className="flex items-center gap-2 w-full justify-between">
        <button 
          onClick={onOpen} 
          className="flex-1 inline-flex items-center justify-center gap-1 px-3 py-1.5 text-[10px] font-bold text-white bg-[#36E08F] hover:bg-[#009085] active:scale-[0.98] rounded-lg transition-all shadow-xs border border-[#009085]/10 cursor-pointer"
        >
          Xét duyệt
        </button>
        <button 
          ...
          onClick={onNotify} 
          className="p-1.5 text-slate-400 hover:text-slate-800 hover:bg-slate-100 active:scale-[0.98] rounded-lg transition-all border border-slate-200/50 cursor-pointer" 
...
﻿  const [chartCampusId, setChartCampusId] = useState("all");
  ...
  const filteredStudents = useMemo(() => {
    ...
    let baseFiltered = cleanSummaries;
    if (isGDCSUser && currentUser) {
      const userCampusIds = currentUser.campusIds || [];
      const userCampuses = campuses.filter(c => userCampusIds.includes(c.id));
      baseFiltered = cleanSummaries.filter(s =>
        userCampuses.some(c => isPreschoolCampusMatch(s.admissionCampus, c.campusCode, c.campusName) || s.admissionCampus === c.id)
      );
    }

    const campusFiltered = baseFiltered.filter(s => {
      if (!cCampusFilter) return true;
      return isPreschoolCampusMatch(s.admissionCampus, cCampusFilter, cCampusFilter);
    });
    const ageGroupFiltered = campusFiltered.filter(s => {
      if (!cAgeGroupFilter) return true;
      return s.resolvedAgeGroup === cAgeGroupFilter;
    });
    if (devTab === "assess") {
      return ageGroupFiltered.filter(s => !cSearch || (s.studentCode || "").toLowerCase().includes(cSearch.toLowerCase()) || (s.fullName || "").toLowerCase().includes(cSearch.toLowerCase()));
    }
    if (devTab === "xetDuyet") {
      return ageGroupFiltered
    ...
    return [];
  }, [studentSummaries, devTab, cSearch, approvalFilter, cCampusFilter, cAgeGroupFilter]);
  ...
  const renderPaginationControl = () => {
    ...
    return (
      <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/50 text-xs font-bold text-slate-505">
        <div>
          Hiển thị <span className="text-slate-800 font-extrabold">{Math.min(filteredStudents.length, (currentPage - 1) * pageSize + 1)}-{Math.min(filteredStudents.length, currentPage * pageSize)}</span> trong số <span className="text-slate-800 font-extrabold">{filteredStudents.length}</span> học sinh
        </div>
        <div className="flex items-center gap-2">
          <button
            disabled={currentPage === 1}
            onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
            className="px-3 py-1.5 rounded-xl border border-slate-200 hover:border-[#36E08F] hover:bg-white text-slate-655 disabled:opacity-40 disabled:hover:border-slate-200 disabled:hover:bg-transparent transition-all cursor-pointer font-black bg-transparent border-none"
          >
            Trước
          </button>
              ...
              return (
                <button
                  ...
                  className={"h-8 w-8 rounded-xl flex items-center justify-center transition-all cursor-pointer border-none font-black " + (
                    currentPage === pageNum
                      ? "bg-[#36E08F] text-white shadow-sm shadow-[#36E08F]/20"
  ...
  const periods = useMemo(() => {
    ...
    return rawPeriods.map(p => {
      let filteredBatches = (p.batches || []).filter(b => b.status === "ACTIVE");
      if (isGDCSUser && currentUser) {
        const userCampusIds = currentUser.campusIds || [];
        filteredBatches = filteredBatches.filter(b => 
          !b.campusId || 
          userCampusIds.includes(b.campusId) || 
          b.assignedUserId === currentUser.id
        );
      }
      return {
        ...p,
...
Trân trọng kính mời Quý phụ huynh và các em học sinh!`;
  ...
  const openEditBatch = (b: Batch) => {
    ...
    if (parts.length >= 6 && parts[3] === "KSĐV") {
      ...
      const m = b.name.match(/Đợt \d+ - (.*?) \|/);
      if (m) baseName = m[1];
      else {
        const m2 = b.name.match(/Đợt \d+ - (.*)/);
        if (m2) baseName = m2[1];
      }
    }
    setBForm({
  ...
  const saveBatch = async () => {
    if (!bForm.name.trim() || !bForm.startDate || !bForm.endDate) return notify("Cần nhập Tên Đợt KS, Ngày bắt đầu và kết thúc", "err");
    const campus = campuses.find(c => c.id === bForm.campusId);
    const campusName = campus ? (campus.campusCode || campus.campusName) : "Tất cả";
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={fetchPeriods} className="p-2 text-slate-400 hover:text-slate-800 hover:bg-[#36E08F]/5 rounded-none transition-all"><RefreshCw className="w-4 h-4" /></button>
              <button onClick={openAddPeriod} className="flex items-center gap-2 px-5 py-2.5 bg-[#36E08F] text-white text-[13px] font-black rounded-none hover:bg-[#36E08F]-700 transition-all shadow-none shadow-teal-100"><Plus className="w-4 h-4" /> Tạo Kỳ mới</button>
            </div>
          </div>
                ...
                <div key={p.id} className="bg-white rounded-none border border-slate-300 shadow-none overflow-hidden hover:border-slate-300 transition-all">
                  <div className="px-4 py-3.5 flex flex-wrap items-center justify-between gap-3 cursor-pointer" onClick={() => setExpandedId(expandedId === p.id ? null : p.id)}>
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-none bg-[#36E08F]/5 flex items-center justify-center flex-shrink-0"><Baby className="w-6 h-6 text-teal-400" /></div>
                      <div>
                    ...
                    <div className="flex items-center gap-1.5">
                      <button onClick={e => { e.stopPropagation(); openAddBatch(p.id); }} className="flex items-center gap-1.5 text-[11px] font-black text-slate-800 hover:bg-emerald-500 hover:text-white transition-all text-xs font-semibold"><Plus className="w-3.5 h-3.5" /> Thêm Đợt</button>
                      <button onClick={e => { e.stopPropagation(); openEditPeriod(p); }} className="p-2 text-slate-400 hover:text-slate-800 hover:bg-[#36E08F]/5 rounded-none transition-all"><Edit2 className="w-4 h-4" /></button>
                      <button onClick={e => { e.stopPropagation(); setConfirm({ msg: `Xóa kỳ "${p.name}"?`, fn: () => doDeletePeriod(p.id) }); }} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-all text-xs font-semibold"><Trash2 className="w-4 h-4" /></button>
                      <span className="text-slate-300 ml-2">{expandedId === p.id ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}</span>
                    </div>
                  </div>
                  {expandedId === p.id && (
                    <div className="border-t border-slate-300 p-5 bg-[#36E08F]/5/30">
                      {!p.batches || p.batches.length === 0 ? (
                        <div className="text-center py-8 text-slate-400 font-bold text-xs uppercase tracking-wider bg-white rounded-none border-2 border-dashed border-slate-300">Chưa có Đợt nào</div>
                      ) : (
                        <div className="overflow-x-auto bg-white border border-slate-300 rounded-none shadow-none">
                          <table className="w-full text-left border-collapse">
                            <thead className="bg-[#36E08F]/5/70 border-b border-slate-300">
                              <tr>
                            ...
                            <tbody>
                              {p.batches.map(b => {
                                ...
                                return (
                                  <tr key={b.id} className="hover:bg-[#36E08F]/5/30 transition-colors">
                                    <td className="p-2 border border-slate-200"><span className="inline-flex items-center justify-center w-8 h-8 rounded-none bg-[#36E08F]/5 font-black text-slate-800 text-xs">#{b.batchNumber}</span></td>
                                    ...
                                    <td className="p-2 border border-slate-200"><div className="flex items-center gap-2">
                                       <button
                                         ...
                                         className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#36E08F] focus:ring-offset-2 ${
                                      ...
                                      <div className="flex items-center justify-between gap-2 group/assignee">
                                        ...
                                        {b.assignedUserId && (
                                          <button
                                            ...
                                            className="p-1 text-slate-400 hover:text-slate-800 hover:bg-[#36E08F]/5 rounded-md transition-all ml-2"
                                    ...
                                    <td className="p-2 text-right border border-slate-200"><div className="flex justify-end gap-1"><button onClick={() => openEditBatch(b)} className="p-2 text-slate-300 hover:text-slate-800 hover:bg-[#36E08F]/5 rounded-none"><Edit2 className="w-4 h-4" /></button><button onClick={() => setConfirm({ msg: `Xóa đợt #${b.batchNumber}?`, fn: () => doDeleteBatch(b.id) })} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 text-xs font-semibold"><Trash2 className="w-4 h-4" /></button></div></td>
                                  </tr>
                                );
                              })}
                            </tbody>
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-black text-slate-600 uppercase tracking-widest flex items-center gap-2">
              <UserCheck className="w-4 h-4 text-slate-800" /> Phân công Giáo viên Khảo sát
            </h2>
            <div className="flex gap-2">
              <button 
                onClick={fetchAssignments} 
                className="p-2 text-slate-400 hover:text-slate-800 hover:bg-[#36E08F]/5 rounded-none transition-all"
              >
              ...
              <button 
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-fuchsia-500 to-pink-500 text-white text-[13px] font-black rounded-none hover:from-fuchsia-600 hover:to-pink-600 transition-all shadow-none shadow-pink-100 disabled:opacity-50"
              >
                {aNotifyingAll ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="bg-white rounded-3xl border border-slate-300 shadow-none p-5 space-y-4">
                <h3 className="text-[12px] font-black text-slate-800 uppercase tracking-widest flex items-center gap-1.5 pb-2 border-b border-slate-300">
                ...
                <div className="space-y-3">
                  ...
                  {aActiveBatch && (() => {
                    ...
                    return (
                      <div className="bg-gradient-to-br from-[#36E08F]/5 via-white to-teal-50/10 border-l-4 border-l-[#36E08F] border-y border-r border-slate-300 rounded-none p-4 space-y-3.5 shadow-none animate-in fade-in slide-in-from-top-2 duration-300">
                        <div className="flex items-center gap-2 border-b border-[#36E08F]/10 pb-2">
                        ...
                        <div className="space-y-2">
                          ...
                          {mappings.map((m, idx) => (
                            <div key={idx} className="grid grid-cols-12 gap-2 items-center bg-white border border-teal-50 px-3 py-2.5 rounded-none shadow-2xs hover:border-[#36E08F]/30 hover:shadow-xs transition-all duration-200">
                              ...
                              <span className="col-span-6 font-extrabold text-slate-800 text-[11px]">{m.actual}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-1">
                      {grades.map(g => {
                        ...
                        return (
                          <label 
                            key={g} 
                            className={`flex items-center gap-2.5 p-3 rounded-none border cursor-pointer transition-all ${
                              isChecked 
                                ? "bg-[#36E08F]/5 border-[#36E08F] text-slate-800 font-extrabold shadow-xs" 
                                : "bg-white border-slate-300 text-slate-600 hover:bg-slate-50 hover:border-slate-300"
                            }`}
                          >
                            <input 
                              type="checkbox" 
                              ...
                              className="w-4 h-4 rounded text-slate-800 focus:ring-teal-500 focus:ring-offset-0 cursor-pointer"
                            />
              ...
              <div className="bg-white rounded-3xl border border-slate-300 shadow-none p-5 space-y-4">
                <div className="flex items-center justify-between pb-2 border-b border-slate-300">
                  ...
                  <span className="text-[10px] bg-[#36E08F]/5 text-slate-800 px-2 py-0.5 rounded-none font-black">
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full outline-none focus:bg-white focus:border-teal-400 focus:ring-4 focus:ring-teal-400/10 text-xs font-semibold text-slate-700 transition-all shadow-none cursor-pointer text-xs font-semibold"
                  >
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-11 pr-4 outline-none text-sm font-medium text-slate-700 placeholder-slate-400 focus:bg-white focus:ring-4 focus:ring-teal-400/10 transition-all shadow-inner text-xs font-semibold"
                  />
                  {aSearchTeacher && (
                    <button 
                  ...
                  {teachers
                    ...
                    .map(t => {
                      ...
                      return (
                        <label 
                          key={t.id} 
                          className={`flex items-center justify-between p-3 rounded-none cursor-pointer border transition-all ${isChecked ? "bg-[#36E08F]/5/50 border-slate-300" : "bg-white border-slate-300 hover:border-slate-300 hover:bg-slate-50/30"}`}
                        >
                          <div className="flex items-center gap-3">
                            <input 
                              ...
                              className="w-4.5 h-4.5 rounded-none border-slate-300 text-slate-800 focus:ring-teal-500 focus:ring-offset-0 cursor-pointer"
                            />
                            <div>
                              <div className="text-sm font-bold text-slate-800 flex items-center gap-2">
                                <span>{t.teacherName}</span>
                                {t.departmentRel?.name && (
                                  <span className="text-[9px] font-black uppercase bg-[#36E08F]/5 text-slate-800 border border-slate-300 px-1.5 py-0.5 rounded-none tracking-wider">
                                    {t.departmentRel.name}
                                  </span>
                                )}
                              </div>
                          ...
                          {isChecked && <CheckCircle className="w-4.5 h-4.5 text-slate-800 animate-in zoom-in-50 duration-200" />}
                        </label>
                      );
                    })}
                </div>
                ...
                <button 
                  ...
                  className="w-full py-3.5 bg-[#36E08F] text-white font-black rounded-none hover:bg-[#36E08F]-700 disabled:opacity-50 transition-all shadow-none shadow-teal-100/50 flex items-center justify-center gap-2 mt-4 hover:scale-[1.01] active:scale-[0.99] duration-150"
                >
                  {aSaving ? (
            ...
            <div className="lg:col-span-7">
              <div className="bg-white rounded-3xl border border-slate-300 shadow-none p-5 space-y-4 h-full min-h-[500px]">
                <h3 className="text-[12px] font-black text-slate-800 uppercase tracking-widest flex items-center gap-1.5 pb-2 border-b border-slate-300">
                  ...
                  <div className="space-y-3">
                    {assignments.map((assign) => {
                      ...
                      return (
                        <div 
                          key={assign.id} 
                          className="flex items-center justify-between p-4 hover:border-slate-300 hover:bg-white hover:shadow-none hover:shadow-teal-50/20 transition-all duration-300 group text-xs font-semibold"
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 text-slate-800 flex items-center justify-center font-black text-sm text-xs font-semibold">
                            ...
                            <div>
                              <div className="text-sm font-black text-slate-800 flex items-center gap-2">
                                <span>{tName}</span>
                                {t?.departmentRel?.name && (
                                  <span className="text-[9px] font-black uppercase text-emerald-600 tracking-wider text-xs font-semibold">
                                    {t.departmentRel.name}
                                  </span>
                                )}
                              </div>
                              ...
                              <div className="flex gap-1.5 mt-1 items-center">
                                <span className="text-[9px] font-black uppercase bg-[#36E08F]/5 text-slate-800 border border-slate-300 px-2 py-0.5 rounded-none tracking-wider">
                                  {assign.grade}
                                </span>
                                {assign.batch && (
                          ...
                          <div className="flex gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                            ...
                            <button 
                              ...
                              className="w-9 h-9 flex items-center justify-center text-slate-500 hover:text-fuchsia-600 hover:bg-fuchsia-50 hover:border-fuchsia-200 transition-all shadow-none text-xs font-semibold"
                            >
                              {isNotifying ? (
                            ...
                            <button 
                              ...
                              title="Hủy phân công"
                              className="w-9 h-9 flex items-center justify-center text-slate-400 hover:text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition-all shadow-none text-xs font-semibold"
                            >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {cfgSelected.length > 0 && (
                <button onClick={() => setConfirm({ msg: `Xóa ${cfgSelected.length} mục đã chọn?`, fn: doBulkDeleteCfg })} className="flex items-center gap-2 text-white text-[13px] font-black hover:bg-rose-700 transition-all shadow-none shadow-rose-100 animate-in fade-in duration-200 text-xs font-semibold"><Trash2 className="w-4 h-4" /> Xóa mục đã chọn (${cfgSelected.length})</button>
              )}
              <button onClick={() => openAddCfg("criteria")} className="flex items-center gap-2 px-5 py-2.5 bg-[#36E08F] text-white text-[13px] font-black rounded-none hover:bg-[#36E08F]-700 transition-all shadow-none shadow-teal-100"><Plus className="w-4 h-4" /> Thêm mục</button>
            </div>
          </div>
          {cfgLoading ? <Spin /> : (
                ...
                <div className="overflow-x-auto custom-scrollbar flex-1">
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-[#36E08F]/5 border-b border-slate-300">
                      <tr>
                        <th className="p-2 w-12 border border-slate-200">
                          <input type="checkbox" className="w-4 h-4 rounded accent-[#36E08F]" checked={configs.length > 0 && cfgSelected.length === configs.length} onChange={e => setCfgSelected(e.target.checked ? configs.map(c => c.id) : [])} />
                        </th>
                    ...
                    <tbody className="divide-y divide-teal-50">
                      {configs.map((c, i) => (
                        <tr key={c.id} className="hover:bg-[#36E08F]/5/30 transition-colors">
                          <td className="p-2 w-12 border border-slate-200">
                            <input type="checkbox" className="w-4 h-4 rounded accent-[#36E08F]" checked={cfgSelected.includes(c.id)} onChange={e => setCfgSelected(e.target.checked ? [...cfgSelected, c.id] : cfgSelected.filter(id => id !== c.id))} />
                          </td>
                          <td className="p-2 text-slate-400 text-sm border border-slate-200">{i+1}</td>
                          <td className="p-2 border border-slate-200"><span className="text-[10px] font-black text-slate-800 bg-[#36E08F]/5 px-2 py-0.5 rounded-none  uppercase">{c.categoryType}</span></td>
                          <td className="p-2 font-mono text-xs font-bold text-slate-800 border border-slate-200">{c.code}</td>
                          <td className="p-2 font-semibold text-slate-700 text-sm border border-slate-200">{c.name}</td>
                          <td className="p-2 text-right border border-slate-200"><div className="flex justify-end gap-1"><button onClick={() => openEditCfg(c)} className="p-2 text-slate-300 hover:text-slate-800 hover:bg-[#36E08F]/5 rounded-none"><Edit2 className="w-4 h-4" /></button><button onClick={() => setConfirm({ msg: `Xóa "${c.name}"?`, fn: () => doDeleteCfg(c.id) })} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 text-xs font-semibold"><Trash2 className="w-4 h-4" /></button></div></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {tab === "children" && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="bg-white rounded-none border border-slate-300 shadow-none p-4 flex flex-wrap items-center gap-4">
            ...
            <button onClick={fetchChildren} className="flex items-center gap-1.5 text-sm font-bold text-slate-800 hover:bg-teal-100 text-xs font-semibold"><Search className="w-4 h-4" /> Tìm</button>
          ...
          <div className="bg-white rounded-none border border-slate-300 shadow-none p-4 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => fileRef.current?.click()} disabled={importing || !cPeriodId || cPeriodId === "all"} className="flex items-center gap-1.5 text-xs font-black text-slate-800 hover:bg-emerald-100 transition-all disabled:opacity-50 text-xs font-semibold"><Upload className="w-4 h-4" /> {importing ? "Đang import..." : "Import Excel"}</button>
                            <button 
                onClick={() => {
                  if (cPeriodId) {
                ...
                disabled={!cPeriodId} 
                className="flex items-center gap-1.5 px-4 py-2 text-xs font-black text-slate-800 bg-emerald-50 hover:bg-emerald-100 rounded-none transition-all disabled:opacity-50"
              >
                <Plus className="w-4 h-4" /> Tạo đợt
              </button>
<button onClick={openAddChild} disabled={!cPeriodId} className="flex items-center gap-1.5 px-4 py-2 text-xs font-black text-white bg-[#36E08F] hover:bg-[#36E08F]-700 rounded-none shadow-none shadow-teal-100 transition-all disabled:opacity-50"><Plus className="w-4 h-4" /> Thêm bé</button>
            </div>
          </div>
            ...
            {cLoading ? <Spin /> : filtChildren.length === 0 ? (
              ...
              <div className="overflow-x-auto custom-scrollbar flex-1">
                <table className="w-full text-left text-sm whitespace-nowrap border-collapse">
                  <thead className="bg-[#36E08F]/5 border-b border-slate-300">
                    <tr>
                      <th className="p-2 w-12 border border-slate-300"><input type="checkbox" className="w-4 h-4 rounded accent-[#36E08F]" checked={filtChildren.length > 0 && cSelected.length === filtChildren.length} onChange={e => setCSelected(e.target.checked ? filtChildren.map(c => c.id) : [])} /></th>
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {filtChildren.map((child, i) => (
                      <tr key={child.id} className={`hover:bg-[#36E08F]/5/30 transition-colors ${cSelected.includes(child.id) ? "bg-[#36E08F]/5/50" : ""}`}>
                        <td className="p-2 border border-slate-300"><input type="checkbox" className="w-4 h-4 rounded accent-[#36E08F]" checked={cSelected.includes(child.id)} onChange={e => setCSelected(e.target.checked ? [...cSelected, child.id] : cSelected.filter(id => id !== child.id))} /></td>
                        ...
                        <td className="p-2 border border-slate-300 text-xs font-black">{child.admissionResult ? <span className={child.admissionResult === "Học thử" ? "text-slate-800" : child.admissionResult.toUpperCase().includes("ĐẠT") && !child.admissionResult.toUpperCase().includes("KHÔNG") ? "text-slate-800" : child.admissionResult.toUpperCase().includes("KHÔNG") ? "text-slate-800" : "text-slate-800"}>{child.admissionResult}</span> : <span className="text-slate-300">Chưa</span>}</td>
                        <td className="p-2 text-right border border-slate-300"><div className="flex justify-end gap-1"><button onClick={() => openEditChild(child)} className="p-2 text-slate-300 hover:text-slate-800 hover:bg-[#36E08F]/5 rounded-none transition-all"><Edit2 className="w-4 h-4" /></button><button onClick={() => setConfirm({ msg: `Xóa bé "${child.fullName}"?`, fn: () => doDeleteChild(child.id) })} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 transition-all text-xs font-semibold"><Trash2 className="w-4 h-4" /></button></div></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {tab === "devAssess" && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-500">
          ...
          <div className="bg-[#EBF5F4]/60 backdrop-blur-md p-1.5 rounded-3xl flex flex-wrap gap-2 w-fit border border-[#36E08F]/15 shadow-sm no-print mb-6">
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-xs font-black tracking-wider transition-all duration-300 border-none cursor-pointer ${
                devTab === "stats" 
                  ? "bg-gradient-to-r from-[#36E08F] to-[#008075] text-white shadow-md shadow-[#36E08F]/20 scale-[1.02]" 
                  : "bg-transparent text-slate-500 hover:text-slate-800 hover:bg-[#36E08F]/5"
              }`}
            >
...
﻿          {/* Sub-tab: Thống kê Phân tích */}
          {devTab === "stats" && (
            <div className="space-y-8 animate-in fade-in duration-300 text-slate-800 p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm bg-white">
              ...
              <div className="bg-[#EBF5F4]/40 p-5 rounded-2xl border border-[#36E08F]/15 grid grid-cols-1 sm:grid-cols-2 gap-4 items-end mb-6">
                <div className="group relative w-full">
                  <label className="block text-[10px] font-black tracking-widest uppercase mb-2 text-[#008075] flex items-center gap-1.5 ml-1">
                    <Calendar className="w-3.5 h-3.5 text-slate-800"/> Kỳ Khảo sát
                  </label>
                  <div className="relative">
                    <select 
                      value={cPeriodId} 
                      onChange={e => { setCPeriodId(e.target.value); setCBatchId(""); }} 
                      className="w-full bg-[#FBFDFD] border border-slate-200 hover:border-[#36E08F] focus:border-[#008075] focus:ring-4 focus:ring-[#36E08F]/10 rounded-xl pl-3.5 pr-8 py-2 outline-none font-semibold text-slate-700 shadow-2xs transition-all text-xs cursor-pointer appearance-none"
                    >
                      <option value="all">Tất cả các kỳ</option>
                      {periods.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </select>
                    <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-slate-400 group-hover:text-slate-800 transition-colors">
                ...
                <div className="group relative w-full">
                  <label className="block text-[10px] font-black tracking-widest uppercase mb-2 text-[#008075] flex items-center gap-1.5 ml-1">
                    <Layers className="w-3.5 h-3.5 text-slate-800"/> Đợt khảo sát
                  </label>
                  <div className="relative">
                    <select 
                      value={cBatchId} 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div className="relative overflow-hidden bg-white p-6 rounded-2xl border border-slate-200/80 border-l-4 border-l-[#36E08F] shadow-sm flex flex-col justify-between group hover:-translate-y-1.5 hover:shadow-[0_12px_30px_rgba(0,169,157,0.1)] hover:border-teal-300 transition-all duration-500 cursor-pointer">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Tổng trẻ Khảo sát</span>
                    <div className="p-2.5 bg-teal-50 text-slate-800 rounded-xl transition-all duration-300 group-hover:scale-110 group-hover:bg-teal-100">
                      <Users className="w-4 h-4" />
                    </div>
                  </div>
                  ...
                  <div className="text-[10px] text-slate-500 mt-2 font-bold flex justify-between">
                    ...
                    <div className="w-24 h-2 bg-slate-100 rounded-full overflow-hidden self-center">
                      <div className="h-full bg-[#36E08F] rounded-full" style={{ width: `${overallKPIs.surveyedRate}%` }}></div>
                    </div>
                  </div>
                </div>
                  ...
                  {campusStats.map(stat => {
                    ...
                    if (shortCode === "CS1") {
                      campusColor = {
                        ...
                        text: "text-slate-800",
                        bg: "bg-teal-50/10",
                        accent: "bg-teal-50 text-slate-800 border border-teal-200",
                        bar: "bg-teal-500",
                      ...
                      >
                        ...
                        <div className="space-y-1.5 pt-2 border-t border-slate-100 text-[10px]">
                          ...
                          <div className="grid grid-cols-3 gap-1 pt-1.5 font-bold text-center">
                            <div className="bg-emerald-50 text-slate-800 rounded py-0.5">
                            ...
                            <div className="bg-rose-50 text-slate-800 rounded py-0.5">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 border-b border-slate-100 pb-4 gap-3">
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-auto">
                      ...
                      <select
                        ...
                        className="bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-black text-slate-700 py-1.5 px-3 outline-none cursor-pointer hover:border-[#36E08F] transition-colors shadow-sm"
                      >
                      ...
                      <ComposedChart data={gradeStats} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        <defs>
                          <linearGradient id="colorTotal" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%" stopColor="#36E08F" stopOpacity={0.15}/>
                            <stop offset="95%" stopColor="#36E08F" stopOpacity={0.02}/>
                          </linearGradient>
                                ...
                                return (
                                  <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-lg text-xs font-semibold text-slate-800 space-y-2">
                                    <div className="font-black text-sm border-b border-slate-100 pb-1.5 flex justify-between gap-4">
                                      ...
                                      <span className="text-slate-800">{stat.total} Bé</span>
                                    </div>
                                ...
                                return (
                                  <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-lg text-xs font-semibold text-slate-800 space-y-2">
                                    <div className="font-black text-sm border-b border-slate-100 pb-1.5 flex justify-between gap-4">
                                      ...
                                      <span className="text-slate-800">{campusGradeSurveyed}/{campusGradeTotal} Bé</span>
                                    </div>
          ...
          {devTab === "assess" && (
            <div className="space-y-6">
              ...
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/80 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end mb-6">
                ...
                <div className="group relative w-full">
                  <label className="block text-[10px] font-black tracking-widest uppercase mb-2 text-[#008075] flex items-center gap-1.5 ml-1">
                    <Building className="w-3.5 h-3.5 text-slate-800"/> Cơ sở
                  </label>
                  <div className="relative">
                    <select 
                      value={cCampusFilter} 
                ...
                <div className="group relative w-full">
                  <label className="block text-[10px] font-black tracking-widest uppercase mb-2 text-[#008075] flex items-center gap-1.5 ml-1">
                    <Layers className="w-3.5 h-3.5 text-slate-800"/> Nhóm tuổi
                  </label>
                  <div className="relative">
                    <select 
                    ...
                    >
                      ...
                      <option value="18 đến 24 tháng">18 đến 24 tháng</option>
                      <option value="24 đến 36 tháng">24 đến 36 tháng</option>
                      <option value="3 tuổi đến 4 tuổi">3 tuổi đến 4 tuổi</option>
                      <option value="4 tuổi đến 5 tuổi">4 tuổi đến 5 tuổi</option>
                      <option value="5 tuổi đến 6 tuổi">5 tuổi đến 6 tuổi</option>
                    </select>
                    <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-slate-400 group-hover:text-slate-800 transition-colors">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7"></path></svg>
                    </div>
                ...
                <div className="group relative w-full">
                  <label className="block text-[10px] font-black tracking-widest uppercase mb-2 text-[#008075] flex items-center gap-1.5 ml-1">
                    <Search className="w-3.5 h-3.5 text-slate-800"/> Tìm nhanh
                  </label>
                  <div className="relative">
                    <input 
                      value={cSearch} 
                      ...
                      className="w-full bg-[#FBFDFD] border border-slate-200 hover:border-[#36E08F] focus:border-[#008075] focus:ring-4 focus:ring-[#36E08F]/10 rounded-xl pl-9 pr-3.5 py-2 outline-none font-semibold text-slate-700 shadow-2xs transition-all text-xs" 
                    />
...
<div className="flex gap-2">
                  <button 
                    ...
                    className="h-9 px-4 text-xs font-black uppercase text-white bg-gradient-to-r from-[#36E08F] to-teal-600 hover:from-[#009085] hover:to-teal-700 rounded-xl shadow-md shadow-teal-500/15 transition-all flex items-center justify-center gap-1.5 cursor-pointer border-none w-full"
                  >
                      ...
                      <tbody className="divide-y divide-slate-100/80">
                        {paginatedStudents.map((s, idx) => {
                            const statusBadge = () => {
                              if (s.scoredCount > 0) {
                                return (
                                  <span className="inline-flex items-center gap-1 text-[10px] font-extrabold px-2.5 py-1 rounded-full bg-emerald-50 text-slate-800  shadow-xs">
                                    Đã đánh giá
                                  </span>
                                );
                              }
                            ...
                            return (
                              <tr key={s.id} className="hover:bg-[#EBF5F4]/40 even:bg-slate-50/40 transition-colors duration-150 text-xs font-semibold">
                                ...
                                <td className="w-20 min-w-[80px] p-3 align-middle bg-inherit border border-slate-100">
                                  <span className="font-mono font-bold text-slate-800 bg-teal-50/90 px-2 py-0.5 rounded-md  text-[11px]">
                                    {s.studentCode}
                                  </span>
                                </td>
                                ...
                                <td className="w-28 min-w-[112px] p-3 align-middle bg-inherit font-semibold">
                                  <span className="px-2.5 py-1 bg-teal-50 text-slate-800 rounded-lg text-[11px] font-bold">
                                    {s.resolvedAgeGroup || "—"}
                                  </span>
                                </td>
                                ...
                                <td className="w-32 min-w-[128px] p-3 align-middle bg-inherit border border-slate-100">{statusBadge()}</td>
                                <td className="w-28 min-w-[112px] p-3 align-middle bg-inherit border border-slate-100">
                                  <button
                                    onClick={() => openEvaluation(s)}
                                    className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-[10px] font-extrabold text-slate-800 bg-violet-50 hover:bg-violet-100  rounded-xl transition-all shadow-xs w-fit cursor-pointer hover:scale-[1.02]"
                                  >
          ...
          {devTab === "xetDuyet" && (
            <div className="space-y-6">
              ...
              {/* Premium Dashboard-style Filter Bar */}
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/80 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 items-end mb-6">
                <div className="group relative w-full">
                  <label className="block text-[10px] font-black tracking-widest uppercase mb-2 text-[#008075] flex items-center gap-1.5 ml-1">
                    <Calendar className="w-3.5 h-3.5 text-slate-800"/> Kỳ Khảo sát
                  </label>
...
<div className="group relative w-full">
                  <label className="block text-[10px] font-black tracking-widest uppercase mb-2 text-[#008075] flex items-center gap-1.5 ml-1">
                    <Sparkles className="w-3.5 h-3.5 text-slate-800"/> Kết quả
                  </label>
                  <div className="relative">
                    <select 
                ...
                <div className="flex gap-2">
                  <button 
                    onClick={fetchStudentSummaries} 
                    className="h-10 px-4 text-xs font-black uppercase text-white bg-[#36E08F] hover:bg-[#008F89] hover:scale-[1.01] rounded-2xl shadow-sm hover:shadow-md shadow-[#36E08F]/15 transition-all flex items-center justify-center gap-1.5 cursor-pointer border-none w-full"
                  >
                  ...
                  <button
                    ...
                    className="h-10 px-4 text-xs font-black uppercase text-slate-800 bg-white hover:bg-[#EBF5F4]/40 hover:scale-[1.01] rounded-2xl shadow-sm border border-emerald-250/80 transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-40 disabled:scale-100 disabled:bg-slate-50 w-full"
                  >
                        ...
                        {paginatedStudents.map((s, idx) => {
                            const getResultBadge = (res: string) => {
                              if (res === "Đạt - Miễn Học Thử" || res === "DAT_MIEN_HOC_THU") {
                                return (
                                  <span className="inline-flex items-center justify-center bg-teal-50 text-slate-800  rounded-full px-2.5 py-1 text-[10px] font-extrabold shadow-xs w-fit">
                                    Miễn HT
                                  </span>
                                );
                              }
                              ...
                              if (res === "Đạt" || res === "DAT") {
                                return (
                                  <span className="inline-flex items-center justify-center bg-emerald-50 text-slate-800  rounded-full px-2.5 py-1 text-[10px] font-extrabold shadow-xs w-fit">
                                    Đạt
                                  </span>
                                );
                              }
                              if (res === "Không đạt" || res === "KHONG_DAT") {
                                return (
                                  <span className="inline-flex items-center justify-center bg-rose-50 text-slate-800  rounded-full px-2.5 py-1 text-[10px] font-extrabold shadow-xs w-fit">
                                    K.Đạt
                                  </span>
                                );
                              }
                              if (res === "Ý kiến khác" || res === "Y_KIEN_KHAC") {
                                return (
                                  <span className="inline-flex items-center justify-center bg-amber-50 text-slate-800 border border-amber-250/50 rounded-full px-2.5 py-1 text-[10px] font-extrabold shadow-xs w-fit">
                                    Ý kiến
                                  </span>
                                );
                              }
                              return <span className="text-[10px] text-slate-355">—</span>;
                            };
                            ...
                            const renderAreaCell = (areaCode: string, tdClass: string) => {
                              const areaScores = s.scores?.filter((sc: any) => sc.criteria?.area?.code === areaCode) || [];
                              if (areaScores.length === 0) {
                                return <td className={`p-3 align-top border border-slate-100 text-slate-355 text-xs font-semibold ${tdClass}`}>—</td>;
                              }
                              ...
                              let progressBg = "bg-[#36E08F]";
                              let progressTrack = "bg-teal-50";
                              let pillClass = "text-slate-800 bg-teal-50 border-amber-200/50";
                              let headerIcon = "🧠";
                              ...
                              return (
                                <td className={`p-3 align-top border border-slate-100 ${tdClass}`}>
                                  <div className="flex flex-col gap-2 w-full">
                                    <div className="flex flex-col gap-1.5 w-full">
                                      <div className={`border rounded-2xl p-3 space-y-2.5 transition-all hover:scale-[1.01] hover:shadow-sm ${cardClass}`}>
                                        <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-wider pb-1.5 border-b border-white/60">
                                          <span className="flex items-center gap-1">
                                            <span>{headerIcon}</span>
                                        ...
                                        <div className={`w-full h-1.5 rounded-full overflow-hidden border border-slate-350/20 ${progressTrack}`}>
                                        ...
                                        <div className="flex items-center justify-between gap-1 mt-1.5">
                                          <span className="text-[10px] font-black text-slate-800 flex items-center gap-1">
                                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> {datCount} Đạt
                                          </span>
                                          <span className="text-[10px] font-black text-slate-800 flex items-center gap-1">
                                            <span className="w-1.5 h-1.5 rounded-full bg-rose-500" /> {khongDatCount} K.Đạt
                                          </span>
                                          {chuaDanhGiaCount > 0 && (
                                            <span className="text-[10px] font-black text-slate-500 flex items-center gap-1">
                                              <span className="w-1.5 h-1.5 rounded-full bg-slate-400" /> {chuaDanhGiaCount} Chưa
                                            </span>
                                          )}
                                        </div>
                                      </div>
                              ...
                              <tr key={s.id} className={`group border-b border-slate-100 transition-colors duration-150 ${(s.enrollmentStatus === "COMPLETED" || s.enrollmentStatus === "ENROLLED" || s.enrollmentStatus === "PENDING" || !!s.enrollmentClass || !!s.enrollmentClassId) ? "bg-emerald-50/60 hover:bg-emerald-100/80" : idx % 2 === 0 ? "bg-white hover:bg-[#EBF5F4]/40" : "bg-slate-50/40 hover:bg-[#EBF5F4]/40"} text-xs font-semibold`}>
                                ...
                                <td className="w-[170px] min-w-[170px] max-w-[170px] sticky left-10 z-10 shadow-[1px_0_0_0_#e2e8f0] whitespace-normal py-4 px-4 align-middle bg-inherit border border-slate-100">
                                  <div className="flex flex-col gap-1.5 w-full">
                                    ...
                                    <div className="text-[10px] text-slate-800 font-mono font-bold bg-[#36E08F]/5 px-2 py-0.5 rounded border border-[#36E08F]/15 w-fit">{s.studentCode}</div>
                                ...
                                <td className="w-28 min-w-[112px] p-3 align-middle bg-inherit border border-slate-100 text-slate-700 font-extrabold whitespace-normal">
                                  {getStandardGrade(s.grade)}
                                </td>

                                <td className="w-24 min-w-[96px] p-3 align-middle bg-inherit border border-slate-100 text-slate-655 font-bold truncate" title={s.admissionCampus}>
                                  {s.admissionCampus || "—"}
                                </td>
                                      ...
                                      <div className="flex flex-col gap-1.5 w-full items-start">
                                        {s.probationaryTeacher ? (
                                          <div className="flex flex-col gap-1 w-full items-start">
                                            ...
                                            <button onClick={() => openAssignModal(s)} className="inline-flex items-center justify-center gap-1 px-2.5 py-1 text-[9px] font-extrabold text-slate-800 bg-teal-50/50 hover:bg-[#36E08F]/10 active:scale-[0.98] rounded-lg border border-[#36E08F]/30 transition-all cursor-pointer">
                                              Phân công lại
                                            </button>
                                          </div>
                                        ) : (
                                          <button onClick={() => openAssignModal(s)} className="inline-flex items-center justify-center gap-1.5 px-3.5 py-1.5 text-[10px] font-bold text-white bg-[#36E08F] hover:bg-[#009085] active:scale-[0.98] rounded-lg transition-all shadow-sm border border-[#009085]/10 cursor-pointer">
                                            Phân công
                                          </button>
                                        )}
                                      </div>
                              ...
                              <tr key={s.id} className="hover:bg-[#EBF5F4]/40 even:bg-slate-50/40 transition-colors duration-150 text-xs font-semibold">
                                  ...
                                  {s.probationaryTeacher ? (
                                    <div className="flex flex-col gap-1 items-start">
                                      ...
                                      <button
                                        ...
                                        className="inline-flex items-center justify-center gap-1 px-2 py-0.5 text-[9px] font-bold text-slate-800 hover:bg-[#36E08F]/10 rounded  transition-all w-fit cursor-pointer"
                                      >
                                        Phân công lại
                                      </button>
                                    </div>
                                    ...
                                    <button
                                      onClick={() => openAssignModal(s)}
                                      className="inline-flex items-center justify-center gap-1 px-3 py-1.5 text-[10px] font-extrabold text-white bg-[#36E08F] hover:bg-[#008F89] hover:scale-[1.02] rounded-xl transition-all shadow-xs w-fit border-none cursor-pointer"
                                    >
                                      Phân công
                                    </button>
                                  )}
                                ...
                                <td className="w-28 min-w-[112px] p-3 align-middle text-center bg-inherit font-semibold">
                                  {(() => {
                                    ...
                                    if (status === "DAT") {
                                      return (
                                        <span className="inline-flex items-center justify-center bg-emerald-50 text-slate-800 rounded-full px-2.5 py-1 text-[10px] font-extrabold">
                                          ✓ Đạt
                                        </span>
                                      );
                                    }
                                    if (status === "KHONG_DAT") {
                                      return (
                                        <span className="inline-flex items-center justify-center bg-rose-50 text-slate-800 rounded-full px-2.5 py-1 text-[10px] font-extrabold">
                                          ✗ Không đạt
                                        </span>
                                      );
                                    }
                                    if (status === "Y_KIEN_KHAC") {
                                      return (
                                        <span className="inline-flex items-center justify-center bg-amber-50 text-slate-800 rounded-full px-2.5 py-1 text-[10px] font-extrabold">
                                          ★ Ý kiến khác
                                        </span>
                                      );
                                    }
                                  ...
                                  <button
                                    ...
                                    className={`inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-[10px] font-extrabold rounded-xl transition-all hover:scale-[1.02] ${
                                      isExempt 
                                        ...
                                        : 'text-slate-800 bg-teal-50 hover:bg-teal-100 cursor-pointer'
                                    }`}
                                  >
                        ...
                        {paginatedStudents.map((s, idx) => {
                            const resultBadge = () => {
                              if (s.probationaryResult === "DAT") {
                                return (
                                  <span className="inline-flex items-center justify-center bg-emerald-50 text-slate-800  rounded-full px-2.5 py-1 text-[10px] font-extrabold shadow-xs">
                                    ✓ ĐẠT - SAU HỌC THỬ
                                  </span>
                                );
                              }
                              return (
                                <span className="inline-flex items-center justify-center bg-teal-50 text-slate-800  rounded-full px-2.5 py-1 text-[10px] font-extrabold shadow-xs">
                                  ✓ ĐẠT - MIỄN HỌC THỬ
                                </span>
                              );
                            };
                            ...
                            return (
                              <tr key={s.id} className="hover:bg-[#EBF5F4]/40 even:bg-slate-50/40 transition-colors duration-150 text-xs font-semibold">
                                ...
                                <td className="w-64 min-w-[256px] p-3 align-middle bg-inherit border border-slate-100">
                                  <div className="flex gap-2">
                                    <button
                                      onClick={() => { setSelectedReportStudent(s); setIsInvitation(false); setIsCommitment(false); setIsPrintModalOpen(true); }}
                                      className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-extrabold text-slate-800 bg-emerald-50 hover:bg-emerald-100 hover:scale-[1.02]  rounded-xl transition-all shadow-xs w-fit cursor-pointer"
                                    >
                                    ...
                                    <button
                                      ...
                                      className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-extrabold text-slate-800 bg-amber-50 hover:bg-amber-100 hover:scale-[1.02]  rounded-xl transition-all shadow-xs w-fit cursor-pointer"
                                    >
                                    ...
                                    <button
                                      ...
                                      className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-extrabold text-slate-800 bg-violet-50 hover:bg-violet-100 hover:scale-[1.02]  rounded-xl transition-all shadow-xs w-fit cursor-pointer"
                                    >
      ...
      {/* Tab: Quản lý Tiêu chí & Lĩnh vực */}
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-black text-slate-600 uppercase tracking-widest flex items-center gap-2"><Settings className="w-4 h-4 text-teal-400" /> Quản lý Tiêu chí &amp; Lĩnh vực Khảo sát</h2>
              ...
              <div className="flex gap-2 p-1 bg-slate-100 rounded-none w-fit mb-2">
                <button
                  onClick={() => setDevType("INPUT")}
                  className={`px-4 py-2 rounded-none text-sm font-bold transition-all ${devType === "INPUT" ? "bg-white text-slate-800 shadow-none" : "text-slate-500 hover:text-slate-700"}`}
                >
                  Tiêu chí Khảo sát (Đầu vào)
                </button>
                <button
                  onClick={() => setDevType("PROBATION")}
                  className={`px-4 py-2 rounded-none text-sm font-bold transition-all ${devType === "PROBATION" ? "bg-white text-emerald-600 shadow-none" : "text-slate-500 hover:text-slate-700"}`}
                >
                  Tiêu chí Học thử (Probation)
                </button>
              </div>
                            <div className="flex flex-wrap gap-1.5 bg-[#36E08F]/5/50 p-1.5 rounded-none border border-slate-300 max-w-fit">
                {criteriaGrades.map(g => (
                  <button
                    key={g}
                    onClick={() => setAgeGroupFilter(g)}
                    className={`px-4 py-2 text-[11px] font-bold rounded-none transition-all ${ageGroupFilter === g ? "bg-white text-slate-800 shadow-none border border-slate-300" : "text-slate-500 hover:text-slate-800"}`}
                  >
                    {g}
                  </button>
                ))}
              ...
              {devType === "INPUT" && (() => {
                ...
                const renderGradeTags = (gradesStr: string, isPhase1: boolean) => {
                  if (gradesStr === "Không áp dụng" || !gradesStr) {
                    return (
                      <span className="text-slate-400 italic text-xs font-bold mt-2.5 inline-block text-xs font-semibold">
                  ...
                  const tagBg = isPhase1 
                    ? "bg-[#36E08F]/5/60 text-teal-800 border-slate-300 hover:bg-teal-100/40" 
                    : "bg-indigo-50/60 text-indigo-800 border-indigo-100 hover:bg-indigo-100/40";
                  return (
                    <div className="flex flex-wrap gap-1.5 mt-2.5">
                      {items.map(item => (
                        <span key={item} className={`px-2.5 py-1 text-[11px] font-black rounded-none border ${tagBg} shadow-none transition-all select-none`}>
                          {item}
                        </span>
                      ))}
                    </div>
                ...
                return (
                  <div className="bg-gradient-to-br from-teal-50/40 via-white to-indigo-50/20 border border-slate-300/60 rounded-3xl p-5 shadow-none max-w-4xl my-4 hover:shadow-none transition-all duration-300 relative overflow-hidden animate-fadeIn">
                    {/* Decorative backdrop blur glow */}
                    <div className="absolute right-0 top-0 w-32 h-32 blur-2xl pointer-events-none text-xs font-semibold" />
                    <div className="absolute left-1/3 bottom-0 w-24 h-24 blur-xl pointer-events-none text-xs font-semibold" />

                    {/* Header */}
                    <div className="flex items-center gap-3.5 pb-4 border-b border-slate-300">
                      <div className="w-10 h-10 rounded-none bg-gradient-to-tr from-teal-500 to-indigo-500 flex items-center justify-center text-white font-extrabold shadow-none shadow-teal-100 shrink-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      {/* Giai đoạn 1 */}
                      <div className="bg-[#36E08F]/5/20 border border-slate-300/40 rounded-none p-4 flex flex-col justify-between hover:border-slate-300/60 transition-all">
                        <div>
                          <div className="flex items-center justify-between">
                            <span className="bg-[#36E08F]/10 text-slate-800 px-2.5 py-0.5 rounded-none text-[9px] font-extrabold uppercase tracking-wider ">
                              Giai đoạn 1
                            </span>
                      ...
                      {/* Giai đoạn 2 */}
                      <div className="p-4 flex flex-col justify-between hover:border-indigo-200/60 transition-all text-xs font-semibold">
                        <div>
                          <div className="flex items-center justify-between">
                            <span className="text-indigo-700 text-[9px] font-extrabold uppercase tracking-wider text-xs font-semibold">
                              Giai đoạn 2
                            </span>
                ...
                <div className="space-y-3">
                  {devAreas.map(area => (
                    <div key={area.id} className="bg-white rounded-none border border-slate-300 shadow-none overflow-hidden">
                      <div
                        className="px-4 py-3.5 flex items-center justify-between gap-3 cursor-pointer hover:bg-[#36E08F]/5/20 transition-all"
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="flex items-center gap-1.5 text-[10px] font-black text-slate-800 hover:bg-emerald-500 hover:text-white transition-all text-xs font-semibold"
                          >
                      ...
                      {expAreaId === area.id && (
                        <div className="border-t border-slate-300 p-4 bg-[#36E08F]/5/10">
                          {(!area.criteria || area.criteria.length === 0) ? (
                            <div className="text-center py-6 text-slate-400 font-bold text-xs bg-white rounded-none border border-dashed border-slate-300">
                              Chưa có tiêu chí nào cho nhóm tuổi này
                            </div>
                          ) : (
                            <div className="overflow-x-auto bg-white border border-slate-300 rounded-none shadow-none">
                              <table className="w-full text-left border-collapse">
                                <thead className="bg-[#36E08F]/5/50 border-b border-slate-300">
                                  <tr>
                                ...
                                <tbody className="divide-y divide-teal-50">
                                  {area.criteria.map((crit, idx) => (
                                    <tr key={crit.id} className="hover:bg-[#36E08F]/5/30 transition-colors">
                                      <td className="p-2 text-slate-400 text-xs border border-slate-200">{idx + 1}</td>
                                      <td className="p-2 font-mono text-xs font-bold text-slate-800 border border-slate-200">{crit.code}</td>
                                      ...
                                      <td className="p-2 border border-slate-200"><span className="text-[10px] font-black text-slate-800 bg-[#36E08F]/5 px-2 py-0.5 rounded-none ">{crit.ageGroup}</span></td>
                                      <td className="p-2 text-right border border-slate-200">
                                        <div className="flex gap-1">
                                          <button
                                            onClick={() => openEditCriteria(crit)}
                                            className="p-1.5 text-slate-300 hover:text-slate-800 hover:bg-[#36E08F]/5 rounded-none"
                                          >
                                            <Edit2 className="w-3.5 h-3.5" />
                                          </button>
                                          <button
                                            onClick={() => setConfirm({ msg: `Xóa tiêu chí "${crit.name}"?`, fn: () => doDeleteCriteria(crit.id) })}
                                            className="p-1.5 text-slate-300 hover:text-rose-500 hover:bg-rose-50 text-xs font-semibold"
                                          >
      ...
      {tab === "reports" && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="bg-white rounded-none border border-slate-300 shadow-none p-4 flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <label className="text-xs font-black text-slate-500 uppercase tracking-wider">Đợt:</label>
              <select value={rptBatchId} onChange={e => setRptBatchId(e.target.value)} className="border border-slate-300 rounded-none p-2 text-sm outline-none focus:ring-2 focus:ring-violet-300 min-w-[140px]" disabled={!rptPeriodId}>
            ...
            {rptBatchId !== "all" && (
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-bold text-slate-800 hover:bg-teal-100 transition-all disabled:opacity-50 cursor-pointer text-xs font-semibold"
              >
                {sendingBatchEmail ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Mail className="w-3.5 h-3.5" />}
                Thông báo GĐCS xét duyệt đợt
              </button>
            )}
            <button onClick={() => { setCPeriodId(rptPeriodId); fetchChildren(); }} className="flex items-center gap-1.5 text-sm font-bold text-slate-800 hover:bg-teal-100 text-xs font-semibold"><RefreshCw className="w-4 h-4" /> Cập nhật</button>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[{ label: "Tổng trẻ KS", value: rptStats.total, color: "from-teal-600 to-teal-500", icon: Baby }, { label: "Đạt yêu cầu", value: rptStats.passed, color: "from-emerald-500 to-teal-400", icon: CheckCircle }, { label: "Chưa duyệt", value: rptStats.pending, color: "from-amber-500 to-orange-500", icon: AlertCircle }, { label: "Không đạt", value: rptStats.failed, color: "from-rose-500 to-red-400", icon: X }].map((stat, i) => (
              <div key={i} className={`bg-gradient-to-br ${stat.color} rounded-none p-5 text-white shadow-none relative overflow-hidden`}>
          ...
          <div className="bg-white rounded-none border border-slate-300 shadow-none p-6">
                ...
                {rptStats.gradeStats.map(g => {
                  ...
                  return (
                    <div key={g.grade} className="space-y-1.5">
                      <div className="flex items-center justify-between text-sm"><span className="font-bold text-slate-700">{g.grade}</span><span className="font-black text-slate-800">{g.count} trẻ ({pct}%)</span></div>
                      <div className="w-full bg-[#36E08F]/5 h-3 rounded-none overflow-hidden border border-slate-200"><div className="h-full bg-[#36E08F] transition-all duration-700" style={{ width: `${pct}%` }} /></div>
                    </div>
                  );
                })}
              </div>
      ...
      {tab === "report_config" && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
          {/* Left: Settings Panel */}
          <div className="lg:col-span-5 bg-white border border-slate-300 shadow-none rounded-3xl p-8 space-y-6">
            <div className="flex items-center gap-3 border-b pb-4 mb-2">
              <div className="w-10 h-10 bg-[#36E08F]/5 text-slate-800 rounded-none flex items-center justify-center">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5 ml-1">
                ...
                <button
                  ...
                  className="text-[10px] font-extrabold text-slate-800 hover:text-slate-800 hover:bg-teal-100 transition-all text-xs font-semibold"
                >
              ...
              <p className="text-[10px] text-slate-400 mt-1 font-semibold">
                Từ khóa tự điền: <span className="text-slate-800 font-bold">{"{{fullName}}"}</span>, <span className="text-slate-800 font-bold">{"{{grade}}"}</span>, <span className="text-slate-800 font-bold">{"{{admissionCampus}}"}</span>, <span className="text-slate-800 font-bold">{"{{academicYear}}"}</span>, <span className="text-slate-800 font-bold">{"{{surveyFormType}}"}</span>
              </p>
            </div>
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  {rcLogo ? (
                    <div className="relative w-16 h-16 rounded-none border border-slate-300 bg-white p-1 flex items-center justify-center">
                      <img crossOrigin={rcLogo?.startsWith("data:") ? undefined : "anonymous"} src={rcLogo} alt="Logo" className="max-w-full max-h-full object-contain rounded-none" />
                      <button onClick={() => setRcLogo("")} className="absolute -top-1.5 -right-1.5 w-5 h-5 text-white flex items-center justify-center shadow-none text-xs font-semibold"><X className="w-3 h-3"/></button>
                    </div>
                  ) : (
                    <label className="w-16 h-16 rounded-none border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 cursor-pointer hover:border-teal-400 hover:text-slate-800 transition-all">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  {rcSignature ? (
                    <div className="relative w-16 h-16 rounded-none border border-slate-300 bg-white p-1 flex items-center justify-center">
                      <img crossOrigin={rcSignature?.startsWith("data:") ? undefined : "anonymous"} src={rcSignature} alt="Chữ ký" className="max-w-full max-h-full object-contain rounded-none" />
                      <button onClick={() => setRcSignature("")} className="absolute -top-1.5 -right-1.5 w-5 h-5 text-white flex items-center justify-center shadow-none text-xs font-semibold"><X className="w-3 h-3"/></button>
                    </div>
                  ) : (
                    <label className="w-16 h-16 rounded-none border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 cursor-pointer hover:border-teal-400 hover:text-slate-800 transition-all">
                      <Upload className="w-5 h-5"/>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  {rcBackground ? (
                    <div className="relative w-16 h-16 rounded-none border border-slate-300 bg-white p-1 flex items-center justify-center">
                      <img crossOrigin={rcBackground?.startsWith("data:") ? undefined : "anonymous"} src={rcBackground} alt="Hình nền" className="max-w-full max-h-full object-contain rounded-none" />
                      <button onClick={() => setRcBackground("")} className="absolute -top-1.5 -right-1.5 w-5 h-5 text-white flex items-center justify-center shadow-none text-xs font-semibold"><X className="w-3 h-3"/></button>
                    </div>
                  ) : (
                    <label className="w-16 h-16 rounded-none border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 cursor-pointer hover:border-teal-400 hover:text-slate-800 transition-all">
                      <Upload className="w-5 h-5"/>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  {rcFooter ? (
                    <div className="relative w-16 h-16 rounded-none border border-slate-300 bg-white p-1 flex items-center justify-center">
                      <img crossOrigin={rcFooter?.startsWith("data:") ? undefined : "anonymous"} src={rcFooter} alt="Footer" className="max-w-full max-h-full object-contain rounded-none" />
                      <button onClick={() => setRcFooter("")} className="absolute -top-1.5 -right-1.5 w-5 h-5 text-white flex items-center justify-center shadow-none text-xs font-semibold"><X className="w-3 h-3"/></button>
                    </div>
                  ) : (
                    <label className="w-16 h-16 rounded-none border-2 border-dashed border-slate-300 flex items-center justify-center text-slate-400 cursor-pointer hover:border-teal-400 hover:text-slate-800 transition-all">
                      <Upload className="w-5 h-5"/>
            ...
            <button onClick={savePreschoolReportConfig} className="w-full py-4 bg-[#36E08F] hover:bg-[#36E08F]-700 text-white font-black text-xs uppercase tracking-widest rounded-none shadow-none shadow-teal-100 transition-all flex items-center justify-center gap-2">
              <CheckCircle2 className="w-4 h-4" /> Lưu cấu hình
            </button>
          </div>

          {/* Right: Live Preview */}
          <div className="lg:col-span-7 shadow-inner p-8 flex flex-col min-h-[500px] text-xs font-semibold">
            <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest mb-5">Xem trước mẫu thiết kế thực tế</span>
            <div className="bg-white rounded-none border border-slate-300 p-8 shadow-none flex flex-col w-full aspect-[210/297] relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <div className="border-b border-slate-300 pb-2 mb-3">
                  {rcLogo ? (
                    ...
                    <span className="text-[10px] font-black tracking-tight text-slate-800 uppercase">SKY-LINE</span>
                  )}
                ...
                {rcFooter ? (
                  ...
                  <div className="border-t border-teal-500/30 pt-2 mt-2 text-[6px] text-slate-400">
                    <p className="text-center text-slate-800 font-bold">www.skylineschool.edu.vn • Hotline: (+84.236) 378 7777</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Info note */}
            <div className="mt-4 bg-[#36E08F]/5 border border-slate-300 rounded-none p-3">
              <p className="text-[11px] font-bold text-slate-800">💡 Lưu ý:</p>
              <p className="text-[10px] text-slate-800 mt-1">Cấu hình sẽ tự động được áp dụng khi xuất thư cho trẻ thuộc cơ sở <strong>{getCampusFullName(campuses.find(c => c.id === rcCampusId)?.campusName || "")}</strong>. Mỗi cơ sở có thể có cấu hình riêng.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Modal: Period */}
      <Modal open={pModal} onClose={() => setPModal(false)} title={editP ? "Sửa Kỳ" : "Tạo Kỳ mới"} footer={<><button onClick={() => setPModal(false)} className="flex-1 text-xs font-black uppercase text-slate-400 hover:text-slate-600">Hủy</button><button onClick={savePeriod} className="flex-1 py-3.5 bg-[#36E08F] text-white rounded-none text-xs font-black uppercase tracking-widest shadow-none shadow-teal-100">{editB ? "Cập nhật đợt" : "Tạo đợt"}</button></>}>
        <div className="space-y-4">
      ...
      {/* Modal: Batch */}
      <Modal open={bModal} onClose={() => setBModal(false)} title={editB ? "Sửa Đợt" : "Thêm Đợt mới"} footer={<><button onClick={() => setBModal(false)} className="flex-1 text-xs font-black uppercase text-slate-400 hover:text-slate-600">Hủy</button><button onClick={saveBatch} className="flex-1 py-3.5 bg-[#36E08F] text-white rounded-none text-xs font-black uppercase tracking-widest shadow-none shadow-teal-100">Lưu</button></>}>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Tên đợt (Tự động theo cấu trúc)" required>
              ...
              <div className="mt-1.5 p-3 text-xs font-semibold">
                <p className="text-[10px] font-black text-slate-800 uppercase tracking-widest mb-1">Hiển thị khoa học & Xét duyệt:</p>
          ...
          <Field label="Cơ sở">
            ...
            >
              <option value="">-- Tất cả cơ sở --</option>
              {filteredCampuses.map(c => <option key={c.id} value={c.id}>{c.campusName}</option>)}
            </select>
          </Field>
      ...
      {/* Modal: Child */}
      <Modal open={cModal} onClose={() => setCModal(false)} title={editC ? "Sửa thông tin Bé" : "Thêm Bé Mầm non"} size="lg" footer={<><button onClick={() => setCModal(false)} className="flex-1 text-xs font-black uppercase text-slate-400">Đóng</button><button onClick={saveChild} className="flex-1 py-3.5 bg-[#36E08F] text-white rounded-none text-xs font-black uppercase tracking-widest shadow-none shadow-teal-100">Lưu</button></>}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Mã bé" required><input value={cForm.studentCode} onChange={e => setCForm({...cForm, studentCode: e.target.value})} disabled={!!editC} className={inp + (editC ? " bg-[#36E08F]/5/50" : "")} placeholder="MN001" /></Field>
          ...
          <Field label="Ngày sinh">
            ...
            />
            {ageInfo.months !== null && (
              <div className="text-[11px] font-black text-slate-800 mt-1.5 uppercase tracking-wider bg-[#36E08F]/5/50 rounded-none px-3 py-1.5 border border-slate-300 flex items-center gap-1.5 animate-in fade-in duration-200">
          ...
          <Field label="Cơ sở">
            ...
            >
              <option value="">-- Chọn cơ sở --</option>
              {filteredCampuses.map(c => <option key={c.campusName} value={c.campusName}>{c.campusName}</option>)}
            </select>
          </Field>
          <Field label="Đợt KS">
            <select
      ...
      {isEmailCongratsModalOpen && emailCongratsStudent && (
        <div className="fixed inset-0 z-550 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl border border-slate-300 shadow-2xl max-w-2xl w-full overflow-hidden flex flex-col max-h-[90vh] animate-scale-up">
            ...
            <div className="bg-gradient-to-r from-teal-600 to-indigo-700 p-6 text-white relative">
              ...
              <p className="text-xs text-white/80 mt-1 font-medium">Học sinh: <strong className="text-white text-sm font-black">{emailCongratsStudent.fullName}</strong> ({emailCongratsStudent.studentCode}) • Cơ sở: <span className="bg-white/20 px-2 py-0.5 rounded-none font-bold text-[11px]">{emailCongratsStudent.admissionCampus || "—"}</span></p>
            </div>
                ...
                <>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full p-3.5 text-sm outline-none focus:ring-2 focus:ring-violet-300 focus:border-teal-400 transition-all text-slate-700 resize-none shadow-inner text-xs font-semibold"
                    />
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2 text-[10px] font-bold">
                        <button 
                          ...
                          className="text-slate-800 hover:text-violet-800 bg-[#36E08F]/5 px-2.5 py-1 rounded-none border border-slate-300 transition-colors cursor-pointer"
                        >
                          Chọn tất cả
                        </button>
                        <button 
                          onClick={() => setEmailCongratsRecipients(prev => prev.map(r => ({ ...r, checked: false })))}
                          className="text-slate-500 hover:text-slate-700 transition-colors cursor-pointer text-xs font-semibold"
                        >
                          Bỏ chọn tất cả
                        </button>
                      </div>
                    ...
                    <div className="overflow-hidden divide-y divide-teal-50 text-xs font-semibold">
                      {["Tư vấn", "Giáo vụ", "Tổ/Môn", "GĐCS", "BGH", "Giáo viên"].map((role) => {
                        ...
                        return (
                          <div key={role} className="p-4 space-y-2">
                            <div className="flex items-center justify-between">
                              <span className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-none border tracking-wider ${
                                role === "GĐCS" ? "bg-emerald-50 text-emerald-800 border-emerald-300" :
                                role === "BGH" ? "bg-teal-50 text-slate-800 border-teal-300" :
                                role === "Giáo vụ" ? "bg-slate-50 text-slate-700 border border-slate-300" :
                                role === "Tổ/Môn" ? "bg-cyan-50 text-cyan-800 border border-cyan-300" :
                                role === "Giáo viên" ? "bg-[#36E08F]/5 text-slate-800 border border-[#36E08F]/30" :
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-1">
                              {emailCongratsRecipients.map((r, idx) => {
                                ...
                                return (
                                  <label 
                                    key={idx} 
                                    className={`flex items-start gap-2.5 p-2 rounded-none border transition-all cursor-pointer select-none text-xs font-semibold ${
                                      r.checked 
                                        ? "bg-[#36E08F]/5/50 border-slate-300 text-violet-900 shadow-none" 
                                  ...
                                  >
                                    <input
                                      ...
                                      className="mt-0.5 rounded text-slate-800 focus:ring-teal-400 border-slate-300 w-3.5 h-3.5"
                                    />
                                    <div className="flex-1 min-w-0">
                                      <div className="font-bold truncate">{r.name}</div>
                                      <div className="text-[10px] text-slate-400 truncate mt-0.5">{r.email}</div>
                                      {r.info && <div className="text-[10px] text-slate-800 font-semibold truncate mt-0.5">🏫 {r.info}</div>}
                                    </div>
                                  </label>
                                );
                              })}
            ...
            {/* Footer */}
            <div className="p-4 flex items-center justify-end gap-3 text-xs font-semibold">
              <button
                type="button"
                onClick={() => setIsEmailCongratsModalOpen(false)}
                className="px-5 py-2 text-xs font-black text-slate-500 bg-white hover:bg-slate-100 rounded-none border border-slate-300 transition-all cursor-pointer"
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-6 py-2 text-xs font-black text-white bg-[#36E08F] hover:bg-[#36E08F]-700 rounded-none transition-all shadow-none shadow-teal-100 hover:shadow-violet-200 cursor-pointer"
          ...
          <div className="bg-white rounded-3xl w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border-2 border-emerald-100 animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="bg-[#0c363f] p-6 text-white shrink-0 relative overflow-hidden border-b border-[#14b8a6]/10">
              ...
              <div className="relative z-10 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-none bg-white/10 border border-white/20 flex items-center justify-center text-emerald-400 shadow-inner">
                ...
                <button 
                  ...
                  className="p-2 hover:bg-white/10 rounded-none transition-colors text-white/80 hover:text-white"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 custom-scrollbar bg-[#f8fafc]">
              ...
              <div className="bg-white p-6 rounded-3xl border-2 border-slate-300/80 shadow-none space-y-4 text-left">
                {/* TO field */}
                <div>
                  ...
                  <input value={recipientEmail} onChange={e => setRecipientEmail(e.target.value)} placeholder="Nhập email người nhận (phân cách bằng dấu phẩy)..." className="w-full outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-400/10 text-sm font-medium text-slate-700 transition-all shadow-none text-xs font-semibold" />
                    ...
                    {[
                      ...
                      return (
                        <button
                          ...
                          className={"px-2 py-1 rounded-none text-[10px] font-bold border transition-all " + (isActive ? 'bg-[#36E08F]/10 border-indigo-200 text-indigo-700' : 'bg-slate-50 border-slate-300 text-slate-500 hover:bg-slate-100')}
                        >
                          + {p.label}
                        </button>
                      );
                ...
                <div>
                  ...
                  <input value={ccEmail} onChange={e => setCcEmail(e.target.value)} placeholder="Nhập email CC (phân cách bằng dấu phẩy)..." className="w-full outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-400/10 text-sm font-medium text-slate-700 transition-all shadow-none text-xs font-semibold" />
              ...
              {/* Email Subject Block */}
              <div className="bg-white p-6 rounded-3xl border-2 border-rose-100/80 shadow-none space-y-4 text-left">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-400/10 text-sm font-medium text-slate-700 transition-all shadow-none text-xs font-semibold"
                  />
                </div>
              ...
              <div className="bg-white p-6 rounded-3xl border-2 border-slate-300/80 shadow-none space-y-4 text-left">
                ...
                <div className="border border-slate-300 rounded-none overflow-hidden max-h-[260px] overflow-y-auto">
                        ...
                        >
                          <td className="p-2 text-center border border-slate-200" onClick={e => e.stopPropagation()}>
                            <input 
                              ...
                              className="rounded border-slate-300 text-slate-800 focus:ring-indigo-500 w-4 h-4 cursor-pointer"
                            />
                          </td>
                          ...
                          <td className="p-2 border border-slate-200">
                            <span className="text-[10px] font-black uppercase tracking-wider text-slate-800 text-xs font-semibold">
            ...
            {/* Footer */}
            <div className="p-4 bg-white border-t border-slate-300 flex justify-end items-center gap-3 shrink-0">
              <button 
                onClick={() => setIsEmailModalOpen(false)} 
                className="px-5 py-2.5 rounded-none hover:bg-slate-100 font-bold text-slate-700 text-xs transition-colors cursor-pointer"
              >
                Đóng lại
              </button>
              <button
                ...
                className="bg-[#0c363f] hover:bg-[#08262c] text-white font-bold text-xs px-6 py-2.5 rounded-none shadow-none hover:shadow-none transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
                {emailSending ? (
                  <>
...
<style>{`
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 bg-slate-50 border-b border-slate-200/90 no-print text-xs font-semibold z-10">
              <div className="flex items-center gap-2">
                {isInvitation ? <Mail className="w-5 h-5 text-slate-800"/> : <GraduationCap className="w-5 h-5 text-slate-800"/>}
              ...
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className="bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98] text-white text-xs font-black uppercase tracking-wider rounded-xl shadow-md shadow-emerald-600/20 px-4 py-2.5 flex items-center gap-2 transition-all cursor-pointer"
                >
                ...
                <button type="button"
                  ...
                  className="w-8 h-8 flex items-center justify-center hover:bg-slate-50 text-slate-500 shadow-none transition-all cursor-pointer text-xs font-semibold"
                >
            ...
            <div id="print-body-scroll-wrapper" className="overflow-y-auto p-4 bg-slate-100 flex justify-center max-h-[80vh]">
              <div id="print-main-container" className="block relative bg-slate-200">
                <div 
                  ...
                  className="bg-white shadow-none border border-slate-300 relative text-slate-800 text-sm leading-relaxed print-page"
              ...
              >
                ...
                {studentCampusConfig?.background ? (
                  ...
                  <svg className="print-watermark" style={{ display: "block", position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "110mm", height: "auto", opacity: 0.04, zIndex: 0, pointerEvents: "none" }} viewBox="0 0 100 100">
                    <path fill="#36E08F" d="M10,80 Q50,40 90,20 Q60,50 10,80 Z" />
                    <path fill="#36E08F" d="M30,80 Q60,55 90,35 Q65,60 30,80 Z" />
                  </svg>
                )}
                  ...
                  <div className="flex flex-col gap-1 border-b pb-2 mb-3">
                    <div className="flex items-center justify-between">
                      {studentCampusConfig?.logo ? (
                        ...
                        <div className="flex items-center gap-1.5">
                          <span className="text-2xl font-black tracking-tight text-slate-800" style={{ fontFamily: "Arial, sans-serif" }}>SKY-LINE</span>
                ...
                {studentCampusConfig?.footer ? (
                  <div className="border-t border-slate-300 pt-3 z-10 w-full print-footer" style={{ marginTop: "auto", width: "100%", paddingLeft: "0", paddingRight: "0", boxSizing: "border-box" }}>
                  ...
                  <div className="w-full pt-2 z-10 print-footer" style={{ marginTop: "auto", width: "100%", boxSizing: "border-box", fontFamily: "Arial, sans-serif" }}>
                    ...
                    <div className="flex items-center gap-2 mb-2 w-full">
                      <span className="font-bold text-slate-800 whitespace-nowrap uppercase text-[9.5pt] tracking-wide">HỆ THỐNG GIÁO DỤC SKY-LINE</span>
                      <div className="flex-grow border-t border-[#36E08F]/70 h-0 mt-0.5"></div>
                      <span className="font-semibold text-slate-800 whitespace-nowrap lowercase text-[9pt]">www.skylineschool.edu.vn</span>
                    </div>
                    ...
                    <div className="flex flex-row justify-between w-full relative text-[7.5pt] text-slate-500 leading-tight">
                      ...
                      <div className="w-[32%] flex flex-col gap-1 text-left">
                        <div>
                          <p className="font-bold text-slate-800 text-[8pt]">SKY-LINE Riverside</p>
                          <p className="mt-0.5">Lô A2.4 Trần Đăng Ninh, Q. Hải Châu, TP. Đà Nẵng</p>
                        </div>
                        <div>
                          <p className="font-bold text-slate-800 text-[8pt]">SKY-LINE Central</p>
                      ...
                      <div className="w-[32%] flex flex-col gap-1 text-left">
                        <div>
                          <p className="font-bold text-slate-800 text-[8pt]">SKY-LINE Global</p>
                          <p className="mt-0.5">Lô A2 Trần Đăng Ninh, Q. Hải Châu, TP. Đà Nẵng</p>
                        </div>
                        <div>
                          <p className="font-bold text-slate-800 text-[8pt]">SKY-LINE Beach</p>
                      ...
                      <div className="w-[32%] flex flex-col gap-1 text-left">
                        <div>
                          <p className="font-bold text-slate-800 text-[8pt]">SKY-LINE Hill</p>
                      ...
                      <div className="absolute right-[-4px] top-[-4px] w-[50px] h-[38px] opacity-100 pointer-events-none flex items-center justify-center text-slate-800">
                ...
                {([]) && ([]).length > 0 && (
                  <div 
                    className="bg-white shadow-none border border-slate-300 relative text-slate-800 text-sm leading-relaxed print-page mt-8"
                ...
                {/* Top Logo and Header (Synchronized perfectly with Page 1) */}
                    ...
                    {/* Footer Contact */}
                    {studentCampusConfig?.footer ? (
                      <div className="border-t border-slate-300 pt-3 z-10 w-full print-footer" style={{ marginTop: "40px", width: "100%", paddingLeft: "15mm", paddingRight: "15mm", boxSizing: "border-box" }}>
                        <img crossOrigin={studentCampusConfig.footer.startsWith("data:") ? undefined : "anonymous"}  src={studentCampusConfig.footer} alt="Footer Print" className="w-full" style={{ maxHeight: "100px", objectFit: "contain" }} />
                      </div>
                    ) : (
                    ...
                    <div className="flex items-center gap-2 mb-2.5 w-full">
                      <span className="font-bold text-slate-800 whitespace-nowrap uppercase text-[11.5px] tracking-wide">HỆ THỐNG GIÁO DỤC SKY-LINE</span>
                      <div className="flex-grow border-t border-[#36E08F]/70 h-0 mt-0.5"></div>
                      <span className="font-semibold text-slate-800 whitespace-nowrap lowercase text-[11px]">www.skylineschool.edu.vn</span>
                    </div>
                    ...
                    <div className="flex flex-row justify-between w-full relative text-[9px]">
                      ...
                      <div className="w-[30%] flex flex-col gap-1.5 text-left">
                        <div>
                          <p className="font-bold text-slate-800 text-[9.5px] leading-tight">SKY-LINE Riverside</p>
                          <p className="text-[#555555] text-[8.5px] leading-tight mt-0.5">Lô A2.4 Trần Đăng Ninh, P. Hòa Cường, TP. Đà Nẵng</p>
                        </div>
                        <div>
                          <p className="font-bold text-slate-800 text-[9.5px] leading-tight">SKY-LINE Central</p>
                          <p className="text-[#555555] text-[8.5px] leading-tight mt-0.5">Số 48 Nguyễn Du, P. Hải Châu, TP. Đà Nẵng</p>
                        </div>
                        <div>
                          <p className="font-bold text-slate-800 text-[9.5px] leading-tight">SKY-LINE Global</p>
                      ...
                      <div className="w-[30%] flex flex-col gap-1.5 text-left">
                        <div>
                          <p className="font-bold text-slate-800 text-[9.5px] leading-tight">SKY-LINE Beach</p>
                          <p className="text-[#555555] text-[8.5px] leading-tight mt-0.5">Số 199 Trần Anh Tông, P. Thanh Khê, TP. Đà Nẵng</p>
                        </div>
                        <div>
                          <p className="font-bold text-slate-800 text-[9.5px] leading-tight">SKY-LINE Hill</p>
                          <p className="text-[#555555] text-[8.5px] leading-tight mt-0.5">Khối Hà My Đông A, P. Điện Bàn Đông, TP. Đà Nẵng</p>
                        </div>
                        <div>
                          <p className="font-bold text-slate-800 text-[9.5px] leading-tight">Trung tâm sống thành công - SLS</p>
                          <p className="text-[#555555] text-[8.5px] leading-tight mt-0.5">Số 48 Nguyễn Du, P. Hải Châu, TP. Đà Nẵng</p>
                        </div>
                      </div>
                    ...
                    <div className="absolute right-[-5px] top-[2px] w-16 h-12 opacity-100 pointer-events-none flex items-center justify-center text-slate-800">
    ...
    "Chỉ số 61. Tô màu kín, không chờm ra ngoài đường viền các hình có chi tiết nhỏ."
                      ...
                      <div className="flex flex-col relative z-10 w-full" style={{ flex: "1 1 auto", minHeight: 0, overflow: "hidden" }}>
                        ...
                        <div className="space-y-8 mt-4">
                          {failedCriteria.length > 0 && (
                            <div className="space-y-3">
                              <h4 className="text-base font-bold text-slate-800 border-b border-rose-200 pb-1 uppercase tracking-wide">Các tiêu chí chưa đạt</h4>
                          ...
                          {skippedCriteria.length > 0 && (
                            <div className="space-y-3">
                              <h4 className="text-base font-bold text-slate-600 border-b border-slate-300 pb-1 uppercase tracking-wide">Các tiêu chí chưa thực hiện</h4>
                              <ul className="space-y-2 list-none">
                                {skippedCriteria.map((name, i) => (
                                  <li key={i} className="flex items-start gap-3 text-[14px]">
                                    <span className="mt-1.5 w-1.5 h-1.5 rounded-none bg-slate-400 shrink-0"></span>
                                    {name}
                                  </li>
                                ))}
                              </ul>
        ...
        footer={isAssessmentLocked ? (
          ...
          <div className="flex items-center justify-end gap-3 w-full px-6 py-4 bg-slate-50 border-t border-slate-200/60 -mx-6 -mb-6 mt-6 rounded-b-3xl">
            ...
            <button
              ...
              className="px-6 py-2.5 bg-gradient-to-r from-[#36E08F] to-[#008075] hover:from-[#008075] hover:to-[#007067] text-white font-black rounded-2xl transition-all disabled:opacity-50 active:scale-[0.98] cursor-pointer border-none text-[11px] tracking-wider uppercase shadow-md shadow-[#36E08F]/15"
            >
      ...
      >
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-[#EBF5F4]/60 via-[#F5FAF9]/20 to-[#EBF5F4]/40 p-5 rounded-3xl border border-[#36E08F]/15 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-sm">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#36E08F]/15 to-[#008075]/5 text-slate-800 flex items-center justify-center shadow-inner shrink-0">
              ...
              <div>
                ...
                <p className="text-[11px] font-extrabold text-slate-800 font-mono mt-1.5 bg-[#36E08F]/5 px-2 py-0.5 rounded-lg border border-[#36E08F]/10 w-fit">{evalStudent?.studentCode}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-6 border-t md:border-t-0 border-slate-200/60 pt-4 md:pt-0 w-full md:w-auto">
              ...
              <div className="text-xs">
                ...
                <span className="text-xs font-black text-slate-800 bg-emerald-50 px-2.5 py-1 rounded-xl border border-emerald-100/50">{getStandardGrade(evalStudent?.grade)}</span>
              </div>
              <div className="text-xs">
          ...
          {devLoading ? (
            ...
            <div className="space-y-6">
              {devAreas.map(area => (
                <div key={area.id} className="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden hover:border-[#36E08F]/20 transition-all duration-300">
                  ...
                  <div className="divide-y divide-slate-100 px-6">
                    {area.criteria.map((crit, idx) => {
                      ...
                      const radioOpts = [
                        ...
                        { key: "DAT", label: "Đạt", color: "peer-checked:bg-emerald-500 peer-checked:text-white peer-checked:border-emerald-500 border-slate-200 text-emerald-600 hover:bg-emerald-50/50 hover:text-slate-800 hover:border-emerald-200" },
                        { key: "KHONG_DAT", label: "Không đạt", color: "peer-checked:bg-rose-500 peer-checked:text-white peer-checked:border-rose-500 border-slate-200 text-rose-600 hover:bg-rose-50/50 hover:text-slate-800 hover:border-rose-200" }
                      ];
                        ...
                        <div key={crit.id} className="py-5 flex flex-col gap-3">
                          ...
                          {isPhysical && (
                            <div className="flex flex-wrap items-center gap-3 pl-8">
                              <div className="flex items-center bg-[#F8FAFC] border border-slate-200 rounded-2xl overflow-hidden focus-within:border-[#36E08F] focus-within:ring-4 focus-within:ring-[#36E08F]/10 transition-all shadow-xs">
                                <input
                                  type="number"
                                  step={isHeight ? "1" : "0.1"}
                                  min="0"
                          ...
                          <div className="pl-8">
                            <input
                              ...
                              disabled={isAssessmentLocked}
                              className="w-full text-xs font-semibold outline-none border border-slate-200 focus:border-[#36E08F] focus:bg-white focus:ring-4 focus:ring-[#36E08F]/10 rounded-xl px-3 py-2 bg-[#F8FAFC] transition-all disabled:bg-slate-100/50 disabled:cursor-not-allowed disabled:text-slate-450"
                            />
                          </div>
                        </div>
                      );
          ...
          <div className="bg-slate-50/60 p-6 rounded-3xl border border-slate-200/80 space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  disabled={isAssessmentLocked}
                  className="w-full bg-white border border-slate-200 rounded-2xl px-4 py-3 text-xs font-semibold outline-none focus:border-[#36E08F] focus:ring-4 focus:ring-[#36E08F]/10 transition-all resize-none placeholder:text-slate-400 disabled:bg-slate-100/50 disabled:cursor-not-allowed disabled:text-slate-500"
                />
              </div>
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest text-slate-800">Lưu ý quan trọng</label>
              <textarea
                value={devNote}
          ...
          {(() => {
            ...
            const getCalcBadge = (res: string) => {
              if (res === "Đạt - Miễn Học Thử") {
                return <span className="text-xs font-black text-slate-800 bg-teal-50  rounded-full px-2.5 py-1 flex items-center gap-1 shadow-xs">✓ ĐẠT - MIỄN HỌC THỬ</span>;
              }
              ...
              if (res === "Đạt") {
                return <span className="text-xs font-black text-slate-800 bg-emerald-50  rounded-full px-2.5 py-1 flex items-center gap-1 shadow-xs">✓ ĐẠT</span>;
              }
              if (res === "Không đạt") {
                return <span className="text-xs font-black text-slate-800 bg-rose-50  rounded-full px-2.5 py-1 flex items-center gap-1 shadow-xs">✗ KHÔNG ĐẠT</span>;
              }
              if (res === "Ý kiến khác") {
                return <span className="text-xs font-black text-slate-800 bg-amber-50 border border-amber-250/50 rounded-full px-2.5 py-1 flex items-center gap-1 shadow-xs">★ Ý KIẾN KHÁC</span>;
              }
            ...
            return (
              <div className="bg-[#EBF5F4]/40 border border-[#36E08F]/15 p-6 rounded-3xl space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#36E08F]/10 pb-4">
                  <div className="flex items-center gap-2">
                ...
                <div className={`grid grid-cols-1 ${showBghSection && showGdcsSection ? 'md:grid-cols-2' : 'md:grid-cols-1'} gap-6`}>
                  {/* BGH MN Column */}
                  {showBghSection && (
                    <div className={`space-y-4 bg-white p-5 rounded-2xl border transition-all ${canApproveBGH ? 'border-[#36E08F]/20 shadow-xs' : 'border-slate-200 bg-slate-50/50 opacity-85'}`}>
                      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                        <div className="flex items-center gap-2">
                        ...
                        {canApproveBGH && (
                          <span className="text-[9px] font-bold text-slate-800 bg-[#36E08F]/5 px-2 py-1 rounded-lg">
                            ✍️ Quyền duyệt
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          ...
                          { status: "DAT_HOC_THU", label: "ĐẠT - HỌC THỬ", color: "text-slate-800 border-[#36E08F]/20 hover:bg-teal-50/40 hover:border-[#36E08F]/40", activeColor: "bg-[#36E08F] text-white border-[#36E08F] shadow-xs" },
                      ...
                      <textarea
                        ...
                        className={`w-full border rounded-2xl px-4 py-2.5 text-xs font-semibold outline-none transition-all resize-none placeholder:text-slate-400 ${
                          canApproveBGH && !isAssessmentLocked
                            ? 'bg-slate-50 border-slate-200 focus:border-[#36E08F] focus:bg-white focus:ring-4 focus:ring-[#36E08F]/10' 
                  ...
                  {/* GĐCS Column */}
                  {showGdcsSection && (
                    <div className={`space-y-4 bg-white p-5 rounded-2xl border transition-all ${canApproveGDCS ? 'border-[#36E08F]/20 shadow-xs' : 'border-slate-200 bg-slate-50/50 opacity-85'}`}>
                      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                        <div className="flex items-center gap-2">
          ...
          {(() => {
            ...
            return (
              <div className="bg-[#EBF5F4]/30 border border-[#36E08F]/10 p-6 rounded-3xl space-y-4">
                <div className="flex items-center gap-2 pb-3 border-b border-slate-200/60">
                  ...
                  <span className="text-[10px] font-black text-slate-800 ml-auto bg-teal-50 px-2 py-0.5  rounded-lg">{pastAssessments.length} đợt</span>
                </div>
                {loadingHistory ? (
                  ...
                  <div className="space-y-3">
                    {pastAssessments.map((histRec: any) => {
                      ...
                      return (
                        <div key={histRec.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden hover:border-[#36E08F]/20 hover:scale-[1.002] transition-all">
                          <div 
                          ...
                          >
                            <div className="flex items-start justify-between">
                              ...
                              <span className={`text-[9px] font-black px-2.5 py-0.5 rounded-full border ${
                                histRec.devAssessmentResult === "DAT" ? "bg-emerald-50 text-slate-800 border-emerald-200/50" :
                                histRec.devAssessmentResult === "KHONG_DAT" ? "bg-rose-50 text-slate-800 border-rose-200/50" : "bg-teal-50 text-slate-800 border-teal-250/50"
                              }`}>
                              ...
                              {histRec.scores && histRec.scores.length > 0 && (
                                <div className="pt-3 border-t border-slate-200/60">
                                  ...
                                  <div className="flex gap-4">
                                    <div className="text-[11px] font-extrabold text-slate-800 bg-emerald-50/60 px-2.5 py-1 rounded-xl ">
                                      Đạt: <span className="font-bold">${histRec.scores.filter((s: any) => s.result === "DAT").length}</span>
                                    </div>
                                    <div className="text-[11px] font-extrabold text-slate-800 bg-rose-50/60 px-2.5 py-1 rounded-xl border border-rose-200/50">
            ...
            <div className="flex items-center gap-2">
              {probStudent && (
                <button
                  ...
                  className="px-4 py-2.5 bg-emerald-50 hover:bg-emerald-100 text-slate-800 rounded-xl border border-emerald-200/80 text-xs font-extrabold transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-2xs"
                >
              ...
              <button
                ...
                className="px-6 py-2.5 bg-gradient-to-r from-[#36E08F] to-teal-600 hover:from-[#009085] hover:to-teal-700 text-white rounded-xl text-xs font-black uppercase tracking-wider shadow-md shadow-teal-500/20 disabled:opacity-50 transition-all flex items-center gap-2 cursor-pointer"
              >
      ...
      >
        <div className="space-y-5">
          ...
          <div className="bg-gradient-to-r from-teal-50/90 via-emerald-50/60 to-teal-50/90 p-4 rounded-2xl border border-teal-200/70 shadow-sm flex flex-wrap items-center justify-between gap-4 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-[#36E08F]/10  flex items-center justify-center text-slate-800">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className="bg-white/80 backdrop-blur-sm px-3 py-1.5 rounded-xl border border-slate-200/80 shadow-2xs">
                ...
                <p className="text-xs font-black text-slate-800 font-mono">{probStudent?.studentCode}</p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50/60 p-4 rounded-2xl border border-slate-200/80 text-xs font-semibold">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:border-[#36E08F] focus:ring-4 focus:ring-[#36E08F]/10 bg-white transition-all shadow-2xs"
              />
            </div>
            <div>
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 flex items-center gap-1">
          ...
          {devLoading ? (
            <div className="flex flex-col items-center justify-center p-12 gap-2 bg-slate-50/50 rounded-2xl border border-slate-200/80">
              ...
              <span className="text-slate-800 font-bold text-xs">Đang tải tiêu chí đánh giá học thử...</span>
            </div>
          ) : devAreas.length === 0 ? (
              ...
              {devAreas.map(area => (
                <div key={area.id} className="bg-white rounded-2xl border border-slate-200/90 shadow-2xs overflow-hidden transition-all">
                  <div className="flex items-center gap-2.5 px-4 py-3 bg-slate-50/80 border-b border-slate-200/80">
                    <div className="w-3 h-3 rounded-full shadow-2xs" style={{ backgroundColor: area.color || "#36E08F" }} />
                    <h4 className="font-black text-slate-800 text-xs uppercase tracking-wider">{area.name}</h4>
                  </div>
                            ...
                            <tr key={crit.id} className="hover:bg-teal-50/20 transition-colors text-xs font-semibold">
                                ...
                                <button
                                  ...
                                  className={`px-2.5 py-1.5 rounded-xl border text-[10px] font-extrabold transition-all cursor-pointer flex items-center justify-center gap-1 mx-auto ${
                                    currentScore.result === "CHUA_THE_HIEN"
                                      ...
                                      : "border-amber-200 bg-amber-50/40 text-slate-800 hover:bg-amber-100/80"
                                  }`}
                                >
                                ...
                                <button
                                  ...
                                  className={`px-2.5 py-1.5 rounded-xl border text-[10px] font-extrabold transition-all cursor-pointer flex items-center justify-center gap-1 mx-auto ${
                                    currentScore.result === "THE_HIEN_TOT"
                                      ...
                                      : "border-emerald-200 bg-emerald-50/40 text-slate-800 hover:bg-emerald-100/80"
                                  }`}
                                >
                              ...
                              <td className="p-2 align-middle">
                                <input
                                  ...
                                  className="w-full px-3 py-1.5 border border-slate-200 rounded-xl text-xs font-normal outline-none focus:border-[#36E08F] focus:ring-2 focus:ring-[#36E08F]/10 bg-white"
                                />
                              </td>
                            </tr>
                          );
          ...
          <div className="bg-slate-50/70 p-5 rounded-2xl border border-slate-200/80 space-y-5 text-xs font-semibold">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex gap-2">
                {[
                  { status: "DAT", label: "ĐẠT - ĐỦ ĐIỀU KIỆN", activeBg: "bg-emerald-600 text-white border-emerald-700 shadow-md shadow-emerald-600/20", inactiveBg: "bg-emerald-50/70 text-slate-800 border-emerald-200 hover:bg-emerald-100" },
                  { status: "CHUA_DAT", label: "CHƯA ĐẠT", activeBg: "bg-rose-600 text-white border-rose-700 shadow-md shadow-rose-600/20", inactiveBg: "bg-rose-50/70 text-slate-800 border-rose-200 hover:bg-rose-100" }
                ].map(opt => (
                  <button
                    key={opt.status}
                    type="button"
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-3 border border-slate-200 rounded-xl text-xs font-medium outline-none focus:border-[#36E08F] focus:ring-4 focus:ring-[#36E08F]/10 bg-white transition-all shadow-2xs"
              />
            </div>
              ...
              return (
                <div className="pt-4 border-t border-slate-200/80 space-y-4">
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                      {[
                        { status: "DAT", label: "ĐẠT", activeBg: "bg-emerald-600 text-white border-emerald-700 shadow-md shadow-emerald-600/20", inactiveBg: "bg-emerald-50/70 text-slate-800 border-emerald-200 hover:bg-emerald-100" },
                        { status: "KHONG_DAT", label: "KHÔNG ĐẠT", activeBg: "bg-rose-600 text-white border-rose-700 shadow-md shadow-rose-600/20", inactiveBg: "bg-rose-50/70 text-slate-800 border-rose-200 hover:bg-rose-100" },
                        { status: "Y_KIEN_KHAC", label: "Ý KIÊN KHÁC", activeBg: "bg-amber-500 text-white border-amber-600 shadow-md shadow-amber-500/20", inactiveBg: "bg-amber-50/70 text-slate-800 border-amber-200 hover:bg-amber-100" }
                      ].map(opt => (
                        <button
                          key={opt.status}
                          type="button"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      disabled={!canApproveBGH}
                      rows={2}
                      className="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-xs font-medium outline-none focus:border-[#36E08F] focus:ring-4 focus:ring-[#36E08F]/10 bg-white transition-all disabled:bg-slate-100/60 disabled:text-slate-500 disabled:cursor-not-allowed shadow-2xs"
                    />
                  </div>
                              ...
                              <div className="mt-1">
                                <div className="flex flex-wrap items-center gap-1.5">
                                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-black tracking-wider ${
                                    log.result === "DAT" ? "bg-emerald-50 text-slate-800 border border-emerald-200" : "bg-rose-50 text-slate-800 border border-rose-200"
                                  }`}>
                          ...
                          {logs.map((log, idx) => (
                            <div key={idx} className="pt-2 first:pt-0 text-[11px] text-slate-650 leading-relaxed font-semibold">
                              ...
                              <div className="mt-1 flex items-center gap-2">
                                <span className={`px-2 py-0.5 rounded-full text-[9px] font-black tracking-wider ${
                                  log.status === "DAT" ? "bg-emerald-50 text-slate-800 border border-emerald-200" : log.status === "KHONG_DAT" ? "bg-rose-50 text-slate-800 border border-rose-200" : "bg-amber-50 text-slate-800 border border-amber-200"
                                }`}>
          ...
          <Field label="Cơ sở" required>
            <div className="relative group">
              <select
                ...
                className="w-full px-4 py-3 border border-slate-200 hover:border-[#36E08F] focus:border-[#008075] focus:ring-4 focus:ring-[#36E08F]/10 rounded-2xl text-xs font-bold outline-none bg-[#FBFDFD] transition-all cursor-pointer appearance-none text-slate-700"
              >
                <option value="">-- Chọn Cơ sở --</option>
          ...
          <Field label="Tên lớp" required>
            <div className="relative group">
              <select
                ...
                className="w-full px-4 py-3 border border-slate-200 hover:border-[#36E08F] focus:border-[#008075] focus:ring-4 focus:ring-[#36E08F]/10 rounded-2xl text-xs font-bold outline-none bg-[#FBFDFD] transition-all cursor-pointer appearance-none text-slate-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        footer={(
          <>
            ...
            <button
              ...
              className="flex-1 py-3.5 bg-[#36E08F] text-white rounded-none text-xs font-black uppercase tracking-widest"
            >
