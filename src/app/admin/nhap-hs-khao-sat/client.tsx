...
interface NhapHsKhaoSatClientProps {
  initialTab: string
  academicYears: any[]
  campuses: any[]
  giaoVuCSUsers: any[]
...
export function NhapHsKhaoSatClient({
  initialTab,
  academicYears,
  campuses,
  giaoVuCSUsers,
  ...
  if (!hasK12Permission && !hasPreschoolPermission) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
        <div className="w-16 h-16 bg-rose-50 rounded-2xl flex items-center justify-center mb-4 border border-rose-100 animate-bounce">
          <ShieldAlert className="w-8 h-8 text-rose-500" />
        </div>
        <h2 className="text-xl font-bold text-slate-800 mb-2">Không có quyền truy cập</h2>
        <p className="text-sm text-slate-500 text-center max-w-md">
          Tài khoản của bạn không được phân quyền truy cập trang Nhập HS Khảo sát. Vui lòng liên hệ Quản trị viên để được hỗ trợ.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 max-w-[1440px] mx-auto">
      {/* Header Info */}
      <div className="mb-4">
        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Nhập HS Khảo sát</h1>
        <p className="text-slate-500 mt-1 text-sm font-medium">
          Khai báo, import danh sách học sinh và trẻ tham gia khảo sát năng lực đầu vào.
        </p>
      </div>
      ...
      <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-2 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
          ...
          {hasK12Permission && (
            <button
              ...
              className={`flex items-center justify-center gap-2.5 px-6 py-3 rounded-lg text-xs font-bold transition-all duration-300 ${
                activeTab === "k12"
                  ? "bg-[#00A19A] text-white shadow-md shadow-teal-500/10 scale-[1.02]"
        ...
        <div className="hidden lg:flex items-center gap-2 px-4 py-2 bg-[#00A19A]/5 rounded-xl border border-[#00A19A]/10">
          <span className="w-2 h-2 rounded-full bg-[#00A19A] animate-ping" />
          <span className="text-[11px] font-bold text-[#00A19A] uppercase tracking-wider">
            Nhập hồ sơ học sinh khảo sát
          </span>
        </div>
      </div>
