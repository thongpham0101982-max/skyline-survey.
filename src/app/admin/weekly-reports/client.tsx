    ...
    <div className="space-y-6 max-w-[1600px] mx-auto p-4 sm:p-6 bg-slate-50/50 min-h-screen">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20">
          <div className="w-10 h-10 border-4 border-teal-200 border-t-[#00A99D] rounded-full animate-spin"></div>
        </div>
      ) : (
        <>
          {/* ============ HISTORY TAB ============ */}
          ...
          {activeTab === "consolidated" && isAdmin && (
            <div className="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden space-y-4 p-5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
                ...
                <span className="text-xs bg-teal-50 text-[#00A99D] font-extrabold px-3 py-1 rounded-full border border-teal-100">
                      ...
                      {items.length === 0 && (
                        <tr>
                          <td colSpan={7} className="p-12 text-center text-slate-400">
                            ...
                            <button onClick={() => addRows(3)} className="mt-2 text-[#00A99D] font-bold text-xs hover:underline">
                          ...
                          <tr key={i} className="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td className="p-3 whitespace-normal break-words align-top">
                              <textarea
                                ...
                                className="w-full border border-slate-200 rounded-xl p-2 text-xs font-bold outline-none focus:ring-2 focus:ring-[#00A99D] bg-white text-indigo-900 resize-none leading-snug"
                              />
                            </td>
                                ...
                                editingItemNote === (item.id || String(i)) ? (
                                  <div className="flex flex-col gap-1.5">
                                    <textarea
                                      ...
                                      className="w-full border border-slate-200 rounded-xl p-2 text-xs outline-none focus:ring-2 focus:ring-[#00A99D] bg-white resize-none"
                                    />
                                    <div className="flex gap-1 justify-end">
                                      <button onClick={() => setEditingItemNote(null)} className="text-[11px] text-slate-500 px-2 py-1 rounded-lg border">Hủy</button>
                                      <button onClick={() => item.id && handleItemNote(item.id)} className="text-[11px] bg-[#00A99D] text-white px-2 py-1 rounded-lg font-bold">Lưu</button>
                                    </div>
                                  </div>
                                ) : (
                                  <div 
                                  ...
                                  >
                                    {item.managerNote ? (
                                      ...
                                      <span className="text-xs text-slate-400 group-hover:text-[#00A99D] italic font-bold">+ Nhấn để ghi nhận xét...</span>
                                    )}
                                  </div>
                                )
                              ) : (
              ...
              {isAdmin && reportId && (
                <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm space-y-3">
                  <h3 className="text-xs font-bold text-slate-700 flex items-center gap-2">
                    <MessageSquare className="w-4 h-4 text-[#00A99D]" /> Thêm nhận xét tổng thể cho Báo cáo này
                  </h3>
                  <textarea
                    value={mgmtComment}
                    ...
                    className="w-full border border-slate-200 rounded-2xl p-3 text-xs outline-none focus:ring-2 focus:ring-[#00A99D] resize-none"
                  />
                  <div className="flex justify-end">
                    <button
                      onClick={() => handleManagerComment(reportId)}
                      className="flex items-center gap-2 bg-[#00A99D] text-white px-5 py-2 rounded-xl hover:bg-[#007A72] text-xs font-bold shadow-sm"
                    >
