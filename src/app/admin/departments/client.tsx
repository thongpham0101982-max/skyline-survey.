  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex justify-between items-center bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
        ...
        <button onClick={() => { setEditingId(null); setForm({ code: "", name: "", description: "", blockCM: "", teamsWebhookUrl: "" }); setIsOpen(true); }} className="px-4 py-2.5 bg-[#36E08F] text-white rounded-xl shadow-sm hover:bg-[#009085] font-semibold flex items-center gap-2"><Plus className="w-5 h-5"/>Thêm Tổ / Phòng</button>
      </div>
      ...
      <div className="bg-white rounded-[1.5rem] shadow-sm border-2 border-violet-100/80 overflow-hidden flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-hidden">
        ...
        <table className="w-full text-sm text-left border-collapse">
            <thead className="text-slate-500 uppercase text-xs font-semibold text-xs font-semibold">
              <tr>
                <th className="w-10 p-2 p-2 border border-slate-200"><input type="checkbox" checked={selectedIds.length === filtered.length && filtered.length > 0} onChange={() => setSelectedIds(selectedIds.length === filtered.length ? [] : filtered.map(d=>d.id))} className="w-4 h-4 rounded text-[#36E08F] focus:ring-indigo-500" /></th>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {loading ? <tr><td colSpan={6} className="p-2 text-center text-slate-400 border border-slate-200">Đang tải...</td></tr> : filtered.length === 0 ? <tr><td colSpan={6} className="p-2 text-center text-slate-400 border border-slate-200">Chưa có Tổ chuyên môn nào.</td></tr> : filtered.map(d => (
                <tr key={d.id} className="hover:bg-slate-50/80 transition-colors text-xs font-semibold">
                  <td className="p-2 p-2 border border-slate-200"><input type="checkbox" checked={selectedIds.includes(d.id)} onChange={() => setSelectedIds(p=>p.includes(d.id)?p.filter(x=>x!==d.id):[...p, d.id])} className="w-4 h-4 rounded text-[#36E08F]" /></td>
                  <td className="p-2 p-2 font-mono font-medium text-[#36E08F] border border-slate-200">{d.code}</td>
                  ...
                  <td className="p-2 p-2 border border-slate-200">
                    {d.blockCM ? (
                      <span className={`px-2.5 py-1 rounded-full text-xs font-semibold inline-block ${
                        ...
                        d.blockCM === "Phổ thông" ? "bg-indigo-50 text-[#36E08F] border border-indigo-200" :
                  ...
                  <td className="p-2 p-2 text-center border border-slate-200">
                    <div className="flex gap-1 justify-center">
                      <button onClick={() => handleEdit(d)} className="p-1.5 text-slate-400 hover:text-[#36E08F] rounded-lg hover:bg-slate-100"><Pencil className="w-4 h-4"/></button>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex justify-end gap-3">
                ...
                <button type="submit" className="px-5 py-2.5 bg-[#36E08F] text-white font-medium rounded-xl shadow-sm hover:bg-[#009085]">Lưu thông tin</button>
              </div>
            </form>
          </div>
        </div>
