...
export function StudentInfoClient({ 
  ...
  }, [formState.targetType, isFormOpen]);
    ...
    if (isFormOpen) {
      if (formMode === "edit" && formState.kqgdTieuHoc) {
        ...
        if (match) {
          ...
          const lines = formState.kqgdTieuHoc.split("\n");
          if (lines.length > 1) {
          ...
          setSelectedProvince("");
          setSelectedCountry("");
          setOriginalKqgd(formState.kqgdTieuHoc || "");
        }
      } else {
        setSchoolNameInput("");
        setSchoolTypeInput("");
        ...
        if (activeTab === "general") {
          ...
          if (targetType) {
            ...
            if (targetType === "Nội tỉnh") {
              ...
              locDetail = wardName ? `${wardName} - ${cityName}` : cityName;
                      ...
                      {selectedLocationType && (
                        <div className="mt-4 p-5 bg-[#F8FAFC] border border-[#D9E2EC] rounded-2xl space-y-4 animate-in slide-in-from-top-2 duration-200">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            {/* BỘ LỌC BẬC HỌC CĂN CỨ THEO QUẢN LÝ TRƯỜNG HỌC */}
                            <div className="col-span-2 md:col-span-1">
                              <label className="block text-[10px] font-bold text-[#64748B] uppercase tracking-wider mb-1.5">Bộ lọc Bậc học (Danh mục trường) *</label>
                              <div className="flex items-center gap-2">
                                <button
                                  type="button"
                                  onClick={() => setOldSchoolLevelFilter("PHO_THONG")}
                                  className={`h-10 px-4 text-xs font-bold rounded-xl transition-all border flex items-center gap-1.5 ${
                                    oldSchoolLevelFilter === "PHO_THONG"
                                      ? "bg-[#004C97] text-white border-[#004C97] shadow-sm"
                                      : "bg-white text-slate-600 border-[#D9E2EC] hover:bg-slate-50"
                                  }`}
                                >
                                  Phổ thông
                                </button>
                                <button
                                  type="button"
                                  onClick={() => setOldSchoolLevelFilter("MAM_NON")}
                                  className={`h-10 px-4 text-xs font-bold rounded-xl transition-all border flex items-center gap-1.5 ${
                                    oldSchoolLevelFilter === "MAM_NON"
                                      ? "bg-[#004C97] text-white border-[#004C97] shadow-sm"
                                      : "bg-white text-slate-600 border-[#D9E2EC] hover:bg-slate-50"
                                  }`}
                                >
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                              ...
                              <input
                                required
                                type="text"
                                list="destination-schools-list-info"
                                value={schoolNameInput}
                                onChange={(e) => {
                                  const val = e.target.value;
                                  setSchoolNameInput(val);
                                  const matched = (destinationSchools || []).find(
                                ...
                                placeholder="Chọn hoặc nhập tên trường cũ..."
                              ...
                              />
                              <datalist id="destination-schools-list-info">
                                {(destinationSchools || [])
                                  .filter((s: any) => s.level === oldSchoolLevelFilter)
                                  .map((s: any) => (
                               ...
                               <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl md:col-span-3">
                                 ...
                                 <div className="text-xs font-semibold text-slate-700 mt-1.5 space-y-1.5">
                                   ...
                                   {selectedStudent.targetType === "Nội tỉnh" && (
                                     <div><span className="text-slate-400">Địa chỉ trường cũ:</span> {selectedStudent.wardName} - {selectedStudent.cityName || "TP Đà Nẵng"}</div>
                                   )}
                                   {selectedStudent.targetType === "Ngoại tỉnh" && (
                                     <div><span className="text-slate-400">Địa chỉ trường cũ:</span> {selectedStudent.wardName} - {selectedStudent.cityName}</div>
                                   )}
