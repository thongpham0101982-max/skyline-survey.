                ...
                {selectedStudent ? (
                  <div>
                    ...
                    {activeTab === "announcements" && (
                      <div className="space-y-6 animate-in fade-in duration-300">
                        <h4 className="text-sm font-black text-slate-850 uppercase tracking-wide border-b border-slate-100 pb-3 flex justify-between items-center">
                          <span>Bản tin rèn luyện & Thông báo học sinh</span>
                          <span className="bg-indigo-50 text-indigo-650 border border-indigo-100 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider flex items-center gap-1">
                          ...
                          <div className="space-y-4">
                            {(selectedStudent.highlightComments || [])
                              ...
                              .map((c) => {
                                ...
                                return (
                                  <div key={c.id} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-4 hover:border-slate-300 transition-all">
                                    {/* Post Header */}
                                    <div className="flex justify-between items-start">
                                    ...
                                    {/* Post Body */}
                                    <p className="text-xs text-slate-700 bg-slate-50/30 p-3 rounded-xl border border-slate-100 font-semibold leading-relaxed whitespace-pre-wrap">
                                      {c.comment}
                                    </p>
                                    ...
                                    <div className="flex items-center gap-6 border-t border-b border-slate-100 py-2 px-1 text-slate-500 font-bold text-[11px]">
                                      <button
                                        ...
                                        className={`flex items-center gap-1.5 transition-colors cursor-pointer hover:text-rose-500 ${isLiked ? "text-rose-500" : ""}`}
                                      >
                                        ...
                                        {comments.map((comm, idx) => (
                                          <div key={idx} className="text-xs space-y-0.5 border-b border-slate-100/50 pb-2 last:border-0 last:pb-0">
                                            <div className="flex justify-between items-center">
                                              <span className="font-extrabold text-slate-850">{comm.author}</span>
                                    ...
                                    <div className="flex gap-2">
                                      <input
                                        ...
                                        onKeyDown={(e) => {
                                          if (e.key === "Enter") {
                                            handleAddComment(c.id, "Admin")
                                          }
                                        }}
                                      />
                                      <button
                                        onClick={() => handleAddComment(c.id, "Admin")}
                        ...
                        {selectedStudent.commitment ? (
                          <div className="bg-slate-50/50 border-2 border-slate-200/60 rounded-3xl p-6 space-y-5 shadow-xs relative overflow-hidden">
                            <div className="absolute top-4 right-4 w-16 h-16 rounded-full border-4 border-slate-200/50 flex items-center justify-center select-none pointer-events-none">
                              <span className="text-[7px] text-slate-300 font-black tracking-widest uppercase">Verified</span>
                            </div>
                            ...
                            <div className="flex items-center justify-between border-b border-slate-200/60 pb-3">
                              ...
                              <span className={`text-[9px] font-black uppercase px-2.5 py-1 rounded-full border ${
                                selectedStudent.commitment.status === "COMPLETED"
                                  ...
                                  : selectedStudent.commitment.status === "VIOLATED"
                                  ? "bg-red-50 text-red-700 border-red-200"
                                  : "bg-blue-50 text-blue-700 border-blue-200"
                              }`}>
                                {selectedStudent.commitment.status === "COMPLETED" ? "Đã hoàn thành tốt" : selectedStudent.commitment.status === "VIOLATED" ? "Vi phạm cam kết" : "Đang thực hiện"}
                              </span>
                            </div>
                            ...
                            <div className="bg-white p-5 rounded-2xl border border-slate-200 text-xs font-semibold text-slate-750 leading-relaxed whitespace-pre-line shadow-inner max-w-2xl mx-auto italic">
                              "${selectedStudent.commitment.content}"
                            </div>
                            
                            <div className="text-[9px] text-slate-455 font-bold text-right pt-2 border-t border-slate-100">
                              Lập bởi: {selectedStudent.commitment.teacherName} • Cập nhật cuối: {new Date(selectedStudent.commitment.updatedAt).toLocaleDateString('vi-VN')}
                            </div>
                          </div>
                        ) : (
                          <div className="text-xs text-slate-400 italic text-center py-12">
                        ...
                        {(!selectedStudent.projects || selectedStudent.projects.length === 0) ? (
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            {selectedStudent.projects.map((p) => (
                              <div key={p.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs font-semibold hover:border-slate-355 hover:shadow-2xs transition-all animate-in fade-in duration-200">
                                <div className="flex items-start justify-between gap-3">
                                  <div>
                                    <h4 className="font-extrabold text-sm text-slate-850">{p.projectName}</h4>
