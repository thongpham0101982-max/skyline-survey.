  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl p-8 max-w-sm w-full text-center animate-in zoom-in-95 duration-200">
        <div className="w-14 h-14 flex items-center justify-center mx-auto mb-4 rounded-full bg-rose-50 text-rose-600"><AlertCircle className="w-7 h-7" /></div>
        <h3 className="text-base font-black text-slate-800 mb-2">Xác nhận</h3>
        <p className="text-sm text-slate-500 mb-6 leading-relaxed">{message}</p>
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 text-xs font-bold text-slate-650 hover:bg-slate-50 border border-slate-200 rounded-xl transition-all">Hủy</button>
          <button onClick={() => { onConfirm(); onClose() }} className="flex-1 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl shadow-lg shadow-rose-100 transition-all">Xác nhận</button>
        </div>
      </div>
    </div>
  )
        ...
        {assignLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-in fade-in duration-300">
            {assignments.map((assign: any) => {
              ...
              return (
                <div key={assign.id} className="bg-white rounded-2xl border border-slate-200/80 p-5 flex flex-col justify-between hover:shadow-md hover:border-[#00A99D]/40 transition-all duration-300 group relative overflow-hidden">
                  
                  {/* Subtle side highlight line */}
                  <div className="absolute top-0 left-0 bottom-0 w-1 bg-[#00A99D]" />

                  <div className="flex gap-3">
                    {/* Initials Avatar Bubble */}
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs text-white shrink-0 shadow-xs" style={{ background: TEAL }}>
                      {initials}
                    </div>
                    
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-bold text-slate-800 truncate pr-16" title={tName}>
                        {tName}
                      </div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase truncate mt-0.5">{tCode} • {tEmail}</div>
                      
                      <div className="flex flex-wrap gap-1.5 mt-3">
                        <span className="text-[9px] font-black uppercase px-2.5 py-1 rounded-full bg-teal-50 text-[#00A99D] border border-teal-100/50">
                          {assign.grade}
                        </span>
                        {assign.batch && (
                          <span className="text-[9px] font-black uppercase text-fuchsia-600 bg-fuchsia-50/50 border border-fuchsia-150 px-2.5 py-1 rounded-full max-w-[150px] truncate" title={assign.batch.name}>
                            {assign.batch.name?.split(" | ")[0]}
                          </span>
                        )}
                      </div>
                      
                      <div className="text-[10px] text-slate-400 mt-3 flex items-center gap-1.5 border-t border-slate-100/80 pt-2.5">
                  ...
                  {/* Actions buttons absolute group (faded by default, visible on group-hover) */}
                  <div className="absolute top-4 right-4 flex gap-1 bg-slate-50/90 p-0.5 rounded-xl border border-slate-200/50 transition-all duration-300 md:opacity-0 md:group-hover:opacity-100 shadow-xs">
                    <button 
                      onClick={() => sendTeacherNotification(assign.id, tName)} 
                      disabled={isNotifying || !canUpdate}
                      title="Gửi email thông báo phân công"
                      className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-500 hover:text-fuchsia-600 hover:bg-white hover:shadow-3xs transition-all disabled:opacity-30">
                    ...
                    <button 
                      ...
                      disabled={!canDelete} 
                      title="Hủy phân công"
                      className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-rose-600 hover:bg-white hover:shadow-3xs transition-all disabled:opacity-30">
