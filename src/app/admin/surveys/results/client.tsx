    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-[#00A19A] font-black text-[10px] uppercase tracking-[0.2em]">
      ...
      {/* Filters */}
      <div className="bg-white p-4 rounded-2xl border border-[#00A19A]/20 shadow-xs flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="relative w-full md:w-80 group">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-[#00A19A] transition-colors" />
          <input 
            type="text" 
      ...
      <div className="grid gap-6">
        {filteredStats.map((s) => {
          ...
          return (
            <div 
              key={s.id} 
              className="bg-white rounded-[2rem] border-2 border-[#00A19A]/20 hover:border-[#00A19A]/40 hover:shadow-xl transition-all duration-300 p-6 group relative overflow-hidden"
            >
              {/* Subtle top decoration */}
              <div className="absolute top-0 left-0 right-0 h-1.5 bg-[#00A19A]/10 group-hover:bg-[#00A19A]/30 transition-colors" />
              ...
              <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                ...
                <div className="flex-1 space-y-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="px-2.5 py-1 rounded-xl text-[10px] font-black uppercase tracking-wider bg-[#00A19A]/10 text-[#00A19A] border border-[#00A19A]/20">{s.academicYear?.name}</span>
                  </div>

                  <h3 className="text-xl font-black text-slate-800 leading-snug group-hover:text-[#00A19A] transition-colors">{s.name}</h3>
                ...
                <div className="shrink-0 flex items-center">
                  <Link
                    ...
                    className="w-full lg:w-auto flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl text-xs font-black text-white shadow-xs transition-all hover:scale-105 active:scale-95 whitespace-nowrap bg-[#00A19A] hover:bg-[#008f89] border border-[#00A19A] shadow-[#00A19A]/20"
                  >
