...
export function ResultsDashboard({ periodId, periodName, periodCode, questions, forms, totalForms }: Props) {
  useEffect(() => {
    console.log("--- DASHBOARD RE-LOADED WITH BAR CHARTS ONLY ---");
  }, []);
    ...
    <div className="bg-[#f1f5f9] min-h-screen p-4 lg:p-8 font-sans text-slate-800">
      <div className="max-w-[1600px] mx-auto space-y-8">
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-2xl border border-slate-100 flex flex-col lg:flex-row justify-between items-center gap-8 relative overflow-hidden">
          ...
          <div className="flex items-center gap-6 relative z-10">
             ...
             <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                   <span className="bg-[#BE1E2E] text-white px-3 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest animate-pulse">Bản mới nhất: Biểu đồ cột</span>
                   <span className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">v3.0 • {periodCode}</span>
                </div>
             </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 bg-slate-50 p-2.5 rounded-3xl border border-slate-200 relative z-10">
            <div className="flex bg-white rounded-2xl border border-slate-200 p-1 shadow-sm">
               <button onClick={() => setCompareBy('CAMPUS')} className={`px-5 py-2 rounded-xl text-xs font-black transition-all ${compareBy === 'CAMPUS' ? 'bg-[#BE1E2E] text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>XEM THEO CƠ SỞ</button>
               <button onClick={() => setCompareBy('CLASS')} className={`px-5 py-2 rounded-xl text-xs font-black transition-all ${compareBy === 'CLASS' ? 'bg-[#BE1E2E] text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>XEM THEO LỚP</button>
            </div>
            <select value={selectedCampus} onChange={(e) => { setSelectedCampus(e.target.value); setFilterType(e.target.value === 'ALL' ? 'ALL' : 'CAMPUS'); }}
              className="px-6 py-3 rounded-2xl border-2 border-slate-200 bg-white text-xs font-black outline-none focus:border-[#BE1E2E] transition-all cursor-pointer">
              <option value="ALL">TẤT CẢ CƠ SỞ</option>
              {campuses.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            <Link href="/admin/surveys" className="px-6 py-3 bg-slate-900 hover:bg-black text-white font-black rounded-2xl transition-all text-xs uppercase shadow-xl">
              Thoát
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
           {[
             { label: 'CHỈ SỐ NPS', value: stats.nps + '%', sub: 'N=' + stats.totalNpsResponses, color: 'text-[#BE1E2E]', bg: 'bg-red-50' },
             { label: 'ĐIỂM TRUNG BÌNH', value: stats.average, sub: 'SCALE 1-10', color: 'text-slate-800', bg: 'bg-indigo-50' },
             { label: 'ĐIỂM TRUNG VỊ', value: stats.median, sub: 'MEDIAN SCORE', color: 'text-slate-800', bg: 'bg-amber-50' },
             { label: 'PHIẾU ĐÃ NỘP', value: filteredForms.length, sub: 'SUBMITTED', color: 'text-slate-800', bg: 'bg-emerald-50' }
           ].map((kpi, idx) => (
             <div key={idx} className={`bg-white p-8 rounded-[2rem] border border-slate-100 shadow-xl text-center group hover:${kpi.bg} transition-all`}>
                <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-3 group-hover:scale-105 transition-transform">{kpi.label}</p>
                <h3 className={`text-5xl font-black mb-2 ${kpi.color}`}>{kpi.value}</h3>
                <p className="text-[9px] font-bold text-slate-300 uppercase tracking-widest">{kpi.sub}</p>
             </div>
           ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           {/* Opinion Section */}
           <div className="lg:col-span-1 bg-[#1e293b] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden border border-white/5">
              <div className="p-8 bg-slate-900 border-b border-white/10 flex items-center justify-between">
                 <h3 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-3">
                   <MessageSquare className="w-5 h-5 text-emerald-400" /> Ý KIẾN ĐÓNG GÓP
                 </h3>
                 <div className="bg-emerald-500/20 text-emerald-400 px-4 py-1.5 rounded-full text-[11px] font-black uppercase">
                   {questionAnalytics.flatMap(q => q.textResponses).length} FEEDBACKS
                 </div>
              </div>
                       ...
                       {questionAnalytics.flatMap(q => q.textResponses).map((op, idx) => (
                         <tr key={idx} className="border-b border-white/5 hover:bg-white/5 transition-all">
                            <td className="p-6 align-top w-1/4">
                               <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{op.className}</span>
                            </td>
                            <td className="p-6 text-[13px] text-slate-300 leading-relaxed italic border-l border-white/5">
                               "{op.text}"
                            </td>
                         </tr>
                       ))}
           ...
           <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl p-10 flex flex-col">
              <div className="flex items-center justify-between mb-10 border-b border-slate-50 pb-8">
                 <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest flex items-center gap-3">
                    <Target className="w-8 h-8 text-[#BE1E2E]" /> SO SÁNH TỶ LỆ PHẦN TRĂM CÁC CƠ SỞ/LỚP
                 </h3>
                 <div className="flex items-center gap-6 text-[10px] font-black text-slate-400 uppercase">
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#BE1E2E]" /> THẤP</div>
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-amber-400" /> TRUNG BÌNH</div>
                    <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-emerald-500" /> CAO</div>
                 </div>
              </div>
              <div className="flex-1 min-h-[400px]">
                 <ResponsiveContainer width="100%" height="100%">
                    <BarChart layout="vertical" data={questionAnalytics[0]?.comparisonData || []} margin={{ left: 100, right: 30 }}>
        ...
        {/* All Detailed Questions as Bar Charts */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
           {questionAnalytics.slice(1).map((q, i) => (
             <div key={q.id} className="bg-white rounded-[3.5rem] border border-slate-100 shadow-2xl p-12 space-y-10 hover:translate-y-[-5px] transition-all">
                <div className="flex justify-between items-start gap-8">
                   ...
                   <div className="bg-slate-50 px-6 py-3 rounded-3xl text-center border border-slate-100 shadow-inner">
                      <p className="text-[10px] font-black text-slate-400 mb-1">AVERAGE</p>
                ...
                <div className="h-[350px] w-full">
                   <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={q.chartData} margin={{ top: 20, right: 20, left: -20, bottom: 0 }}>
                         ...
                         <Bar dataKey="value" radius={[10, 10, 0, 0]} barSize={40}>
                            {q.chartData.map((entry, index) => {
                               ...
                               return <Cell key={`cell-${index}`} fill={isNaN(score) ? COLORS[index % COLORS.length] : (score >= 9 ? '#10b981' : score >= 7 ? '#fbbf24' : '#BE1E2E')} />
                            })}
                         </Bar>
                      </BarChart>
                   </ResponsiveContainer>
                ...
                <div className="grid grid-cols-2 gap-4">
                   {q.chartData.map((item, idx) => (
                      <div key={idx} className="flex justify-between items-center p-4 bg-slate-50 rounded-[1.5rem] border border-slate-100">
