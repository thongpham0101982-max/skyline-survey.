...
function Modal({ open, onClose, title, size="md", children, footer }: {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className={`relative bg-white w-full ${w} rounded-xl shadow-2xl flex flex-col max-h-[95vh] overflow-hidden animate-in zoom-in-95 duration-200 border border-[#D9E2EC]` } onClick={e=>e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#D9E2EC] bg-white">
          ...
          <button onClick={onClose} className="p-2 text-[#64748B] hover:text-[#00B5E2] hover:bg-[#E6F8FD] transition-all rounded-lg cursor-pointer"><X className="w-5 h-5"/></button>
        </div>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {filtStu.map((s, idx) => (
                        <tr key={s.id} className={`group hover:bg-slate-50 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                           ...
                           {selPeriod?.name?.toLowerCase().includes("open day") && (
                             <>
                               <td className="p-2 text-center text-xs text-slate-600 border border-slate-300">
                                 {campuses.find(c => c.id === s.registeredCampus)?.campusName || s.registeredCampus || "-"}
                               </td>
                               <td className="p-2 text-center text-xs text-slate-600 font-bold border border-slate-300">
                                        ...
                                        <div className="bg-gradient-to-r from-indigo-50 to-violet-50/50 border border-indigo-100 rounded-2xl p-4 flex items-center justify-between shadow-sm text-left">
                                          ...
                                          <div className="text-right">
                                            ...
                                            <div className="w-28 bg-slate-100 rounded-full h-2 mt-1 border border-slate-200/50 overflow-hidden shadow-inner shrink-0">
                                              <div className="h-2 text-xs font-semibold" style={{ width: `${scoreVals[4] || 0}%` }}></div>
                                            </div>
                                          </div>
                                        </div>
