    ...
    <div className="space-y-6 font-outfit">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <button onClick={() => { setShowAddForm(true); setEditingId(null); setForm({ name: "", code: "", systemInstruction: "", allowedRoles: ["TEACHER"], isActive: true }) }}
          className="flex items-center gap-2 bg-[#36E08F] text-white px-6 py-3 rounded-2xl hover:bg-[#009085] font-bold shadow-lg shadow-indigo-200 transition-all active:scale-95">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-2xl shadow-slate-200/50 animate-in fade-in slide-in-from-top-4">
          <h3 className="text-xl font-black text-slate-900 mb-6 flex items-center gap-3">
            <div className="p-2 text-xs font-semibold">
              <Settings className="w-6 h-6 text-[#36E08F]" />
            </div>
          ...
          <div className="flex gap-4 mt-8">
            <button onClick={handleSave} disabled={saving} className="bg-[#36E08F] text-white px-8 py-3 rounded-2xl font-black text-sm shadow-xl shadow-indigo-200 hover:bg-[#009085] disabled:opacity-50 transition-all active:scale-95">
        ...
        <div className="overflow-x-auto custom-scrollbar flex-1">
          <table className="w-full text-left whitespace-nowrap border-collapse">
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.map((c: any) => (
                <tr key={c.id} className="hover:bg-[#36E08F]/10/30 transition-colors group">
                  <td className="p-2 p-2 border border-slate-200">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 text-[#36E08F] flex items-center justify-center font-bold text-xs font-semibold">
                  ...
                  <td className="p-2 p-2 text-right border border-slate-200">
                    <div className="flex items-center justify-end gap-2 opacity-70 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => startEdit(c)} className="p-2 text-slate-400 hover:text-[#36E08F] hover:bg-slate-50 transition-all text-xs font-semibold">
