    ...
    <div className="space-y-6">
      ...
      {viewModal.open && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col border border-slate-100 animate-in fade-in zoom-in duration-200">
            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-4 text-xs font-semibold">
              {modalLoading && (
                    ...
                    {modalData.responses.map((r: any, idx: number) => (
                      <div key={r.id} className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm hover:border-indigo-100 transition-colors">
                        <div className="flex items-center justify-between mb-3">
                           <span className="text-[10px] text-indigo-600 font-black uppercase tracking-tighter text-xs font-semibold">{r.question?.section?.name || "Nội dung"}</span>
            ...
            <div className="p-6 flex justify-end text-xs font-semibold">
      ...
      {addModal.open && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full max-h-[85vh] overflow-hidden flex flex-col border border-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="p-6 flex items-center justify-between text-xs font-semibold">
               <div>
                        ...
                        <label key={s.id} className={`flex items-center gap-3 p-3 rounded-2xl border transition-all cursor-pointer ${selectedStudentToAddIds.includes(s.id) ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-slate-100 hover:border-slate-300'} ${!hasParent ? 'opacity-50 cursor-not-allowed grayscale' : ''}`}>
                           ...
                           <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {!hasParent && <span className="text-[9px] text-rose-600 font-bold uppercase tracking-widest text-xs font-semibold">Thiếu PHHS</span>}
                              </div>
                           </div>
                        </label>
                      )
            ...
            <div className="p-6 flex items-center justify-between text-xs font-semibold">
               ...
               <div className="flex gap-2">
                 ...
                 <button 
                   ...
                   className="text-white font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all active:scale-95 disabled:opacity-50 disabled:grayscale text-xs font-semibold"
                 >
      ...
      <div className="bg-white p-8 rounded-[32px] shadow-sm border-2 border-amber-100 space-y-6">
        <div className="flex items-center justify-between gap-4">
           <div className="flex items-center gap-3">
              <div className="w-12 h-12 text-indigo-600 flex items-center justify-center shadow-inner text-xs font-semibold"><LayoutDashboard className="w-6 h-6" /></div>
              <div>
           ...
           <div className="flex items-center gap-3">
             {selectedFormIds.length > 0 && (
               <button
                 ...
                 disabled={deleting}
                 className="flex items-center gap-2 text-white font-bold shadow-xl shadow-rose-100 hover:bg-rose-700 transition-all active:scale-95 group text-xs font-semibold"
               >
             ...
             <button
               ...
               className="flex items-center gap-2 text-white font-bold shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed text-xs font-semibold"
             >
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <select 
                ...
                className="flex-1 p-4 focus:border-indigo-500 font-bold text-indigo-900 outline-none transition-all shadow-sm text-xs font-semibold"
              >
              ...
              {true && (
                <Link 
                  ...
                  className="text-white font-black text-xs uppercase flex items-center gap-2 hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 text-xs font-semibold"
                >
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <select 
                ...
                className="w-full p-4 pl-12 focus:border-indigo-500 font-bold text-slate-700 outline-none transition-all shadow-sm text-xs font-semibold"
              >
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white p-8 rounded-[2rem] border-2 border-blue-100 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
               ...
               <div className="p-4 text-sky-600 shadow-inner text-xs font-semibold"><Users className="w-8 h-8" /></div>
            </div>
            ...
            <div className="bg-white p-8 rounded-[2rem] border-2 border-emerald-100 shadow-sm flex items-center justify-between border-b-4 border-b-emerald-500 hover:shadow-md transition-shadow">
               ...
               <div className="p-4 text-emerald-500 shadow-inner text-xs font-semibold"><CheckCircle2 className="w-8 h-8" /></div>
            </div>
            ...
            <div className="bg-white p-8 rounded-[2rem] border-2 border-rose-100 shadow-sm flex items-center justify-between border-b-4 border-b-rose-500 hover:shadow-md transition-shadow">
               ...
               <div className="p-4 text-rose-500 shadow-inner text-xs font-semibold"><Clock className="w-8 h-8" /></div>
            </div>
            ...
            <div className="bg-slate-900 p-8 rounded-[2rem] shadow-xl flex items-center justify-between border-2 border-slate-700 text-white overflow-hidden relative group">
               ...
               <div className="absolute top-0 right-0 w-32 h-32 blur-3xl -mr-16 -mt-16 text-xs font-semibold"></div>
            </div>
          </div>
                ...
                <div key={cls.id} className={`bg-white rounded-[32px] overflow-hidden transition-all duration-300 border-2 ${isExpanded ? 'border-indigo-200 shadow-xl' : 'border-slate-100 hover:border-slate-300 shadow-sm'}`}>
                   ...
                   >
                     <div className="flex items-center space-x-5">
                       ...
                       <div>
                         <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] text-indigo-700 font-black uppercase tracking-widest text-xs font-semibold">{cls.level || "No Level"}</span>
                         </div>
                         <div className="flex items-center space-x-6 text-[11px] font-bold mt-2">
                           <span className="flex items-center text-rose-500 text-xs font-semibold"><UserCog className="w-3.5 h-3.5 mr-1.5" /> <strong className="text-slate-700">{cls.homeroom}</strong></span>
                           <span className="flex items-center text-teal-600 text-xs font-semibold"><UserCheck className="w-3.5 h-3.5 mr-1.5" /> <strong className="text-slate-700 truncate max-w-[200px]">{cls.subjectTeachers}</strong></span>
                         </div>
                       </div>
                     </div>
                     ...
                     <div className="flex items-center space-x-8 mt-4 lg:mt-0">
                       ...
                       <div className="flex items-center gap-1.5">
                         <div className="text-emerald-700 text-sm font-black flex flex-col items-center text-xs font-semibold">
                            <span className="text-[8px] uppercase font-black opacity-50">Done</span>
                            {cData.completed}
                         </div>
                         <div className="text-slate-400 text-sm font-black flex flex-col items-center text-xs font-semibold">
                       ...
                       <div className="flex items-center space-x-2">
                         <button 
                           ...
                           className="group flex items-center space-x-2 p-2 text-white hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all font-bold text-xs text-xs font-semibold"
                         >
                   ...
                   {isExpanded && (
                     <div className="px-6 pb-6 animate-in fade-in slide-in-from-top-4 duration-300">
                        <div className="overflow-hidden shadow-inner text-xs font-semibold">
                          <table className="w-full text-left whitespace-nowrap border-collapse">
                            <thead>
                              <tr className="bg-slate-100/50 text-[10px] text-slate-400 uppercase font-black tracking-widest border-b border-slate-200/50">
                                ...
                                <th className="p-2 p-2 border border-slate-200">Mã Học Sinh</th>
                                <th className="p-2 p-2 border border-slate-200">Họ và Tên Học Sinh</th>
                                <th className="p-2 p-2 border border-slate-200">PHHS Đại diện</th>
                                <th className="p-2 p-2 border border-slate-200">Trạng thái</th>
                              ...
                              {cData.forms.map((f: any) => {
                                ...
                                return (
                                  <tr key={f.id} className={`group transition-all ${isSelected ? 'bg-rose-50/50' : 'hover:bg-white bg-transparent'}`}>
                                    ...
                                    <td className="p-2 p-2 border border-slate-200">
                                       <div className="flex items-center gap-3">
                                          <div className="w-8 h-8 text-indigo-700 flex items-center justify-center font-black text-xs uppercase text-xs font-semibold">{f.student?.studentName.charAt(0)}</div>
                                    ...
                                    <td className="p-2 p-2 border border-slate-200">
                                      {isDone ? (
                                        <div className="flex items-center gap-2">
                                           <div className="w-2 h-2 animate-pulse text-xs font-semibold"></div>
                                           <span className="text-xs font-black text-emerald-600 uppercase tracking-widest text-xs font-semibold">Đã nộp bài</span>
                                        </div>
                                      ) : (
                                        <div className="flex items-center gap-2">
                                           <div className="w-2 h-2 text-xs font-semibold"></div>
                                           <span className="text-xs font-black text-rose-500 uppercase tracking-widest text-xs font-semibold">{f.status === 'DRAFT' ? 'Đang làm' : 'Chưa mở'}</span>
                                        </div>
                                      )}
                                    </td>
                                    <td className="p-2 p-2 text-center border border-slate-200">
                                      <div className="flex items-center justify-center gap-2">
                                        ...
                                        <button
                                          ...
                                          className="p-2 text-rose-300 hover:text-rose-600 hover:border-rose-200 transition-all hover:bg-rose-50 text-xs font-semibold"
