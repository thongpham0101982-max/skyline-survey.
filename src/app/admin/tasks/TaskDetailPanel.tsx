      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100" onClick={e => e.stopPropagation()}>
        ...
        {task.description && (
          <div className="mx-5 mt-3 p-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs space-y-1">
            <div className="font-bold text-slate-700 flex items-center gap-1.5">
              <FileText className="w-4 h-4 text-[#48BFE3]" /> Chi tiết công việc:
            </div>
        ...
        {task.collaborators && (
          <div className="mx-5 mt-2.5 p-3 bg-teal-50/70 border border-teal-100 rounded-2xl text-xs flex items-center gap-2">
            <Users className="w-4 h-4 text-[#48BFE3] flex-shrink-0" />
            <div>
        ...
        <div className="flex border-b border-slate-200 mt-2">
          <button onClick={() => setTab("comments")}
            className={"flex-1 flex items-center justify-center gap-2 py-3 text-xs font-bold border-b-2 transition-colors " + (tab === "comments" ? "border-[#48BFE3] text-[#48BFE3] bg-teal-50/50" : "border-transparent text-slate-500 hover:text-slate-700")}>
            <MessageSquare className="w-4 h-4" /> Trao đổi & Trao đổi ({comments.length})
          </button>
          <button onClick={() => setTab("attachments")}
            className={"flex-1 flex items-center justify-center gap-2 py-3 text-xs font-bold border-b-2 transition-colors " + (tab === "attachments" ? "border-[#48BFE3] text-[#48BFE3] bg-teal-50/50" : "border-transparent text-slate-500 hover:text-slate-700")}>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            <div className="flex items-center justify-center p-12">
              <div className="w-8 h-8 border-3 border-teal-200 border-t-[#48BFE3] rounded-full animate-spin"></div>
            </div>
          ) : tab === "comments" ? (
                ...
                return (
                  <div key={c.id} className={"flex " + (isOwn ? "justify-end" : "justify-start")}>
                    <div className={"max-w-[85%] group " + (isOwn ? "order-2" : "")}>
                      <div className={"rounded-2xl px-4 py-2.5 shadow-sm " + (isOwn ? "bg-[#48BFE3] text-white rounded-br-md" : "bg-slate-100 text-slate-800 rounded-bl-md")}>
                        {!isOwn && <div className={"text-[11px] font-bold mb-1 text-[#48BFE3]"}>{c.user?.fullName}</div>}
                ...
                return (
                  <div key={a.id} className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-xl hover:border-teal-300 hover:shadow-sm transition-all group">
                    <div className="w-10 h-10 rounded-xl bg-teal-50 flex items-center justify-center flex-shrink-0">
                      <IconComp className="w-5 h-5 text-[#48BFE3]" />
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => handleDownload(a)} className="p-1.5 text-[#48BFE3] hover:bg-teal-50 rounded-lg" title="Tải xuống">
          ...
          {tab === "comments" ? (
            <div className="flex items-end gap-2">
              <textarea
                ...
                rows={1}
                className="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 text-xs resize-none focus:outline-none focus:ring-2 focus:ring-[#48BFE3] bg-white"
              />
              <button
