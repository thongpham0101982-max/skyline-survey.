      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden animate-in" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex border-b border-slate-200">
          <button onClick={() => setTab("comments")}
            className={"flex-1 flex items-center justify-center gap-2 py-3 text-sm font-semibold border-b-2 transition-colors " + (tab === "comments" ? "border-[#00A99D] text-indigo-700 bg-[#00A99D]/10/50" : "border-transparent text-slate-500 hover:text-slate-700")}>
            <MessageSquare className="w-4 h-4" /> Bình luận ({comments.length})
          </button>
          <button onClick={() => setTab("attachments")}
            className={"flex-1 flex items-center justify-center gap-2 py-3 text-sm font-semibold border-b-2 transition-colors " + (tab === "attachments" ? "border-[#00A99D] text-indigo-700 bg-[#00A99D]/10/50" : "border-transparent text-slate-500 hover:text-slate-700")}>
          ...
          {loading ? (
                  ...
                  <div key={c.id} className={"flex " + (isOwn ? "justify-end" : "justify-start")}>
                    <div className={"max-w-[80%] group " + (isOwn ? "order-2" : "")}>
                      <div className={"rounded-2xl px-4 py-2.5 shadow-sm " + (isOwn ? "bg-[#00A99D] text-white rounded-br-md" : "bg-slate-100 text-slate-800 rounded-bl-md")}>
                        {!isOwn && <div className={"text-xs font-bold mb-1 " + (isOwn ? "text-indigo-200" : "text-[#00A99D]")}>{c.user?.fullName}</div>}
            ...
            <div className="p-4 space-y-2">
              ...
              {attachments.map(a => {
                ...
                return (
                  <div key={a.id} className="flex items-center gap-3 p-3 hover:border-indigo-200 hover:bg-[#00A99D]/10/30 transition-all group text-xs font-semibold">
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleDownload(a)} className="p-1.5 text-[#00A99D] hover:bg-indigo-100 text-xs font-semibold" title="Tải xuống">
          ...
          {tab === "comments" ? (
            <div className="flex items-end gap-2">
              ...
              <button
                ...
                className="bg-[#00A99D] text-white p-2.5 rounded-xl hover:bg-[#009085] disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm"
              >
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                disabled={uploading}
                className="flex items-center gap-2 bg-[#00A99D] text-white px-5 py-2.5 rounded-xl hover:bg-[#009085] disabled:opacity-50 font-medium text-sm transition-colors shadow-sm"
              >
