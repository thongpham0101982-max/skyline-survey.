...
const inp = "w-full bg-white border border-slate-200 rounded-2xl px-4 py-3 outline-none focus:border-pink-400 focus:ring-4 focus:ring-pink-400/10 text-sm font-medium text-slate-700 transition-all shadow-sm";
...
function Spin() {
  return <div className="flex items-center justify-center py-16"><div className="relative"><div className="w-12 h-12 rounded-full border-4 border-pink-100 border-t-pink-500 animate-spin"></div><Baby className="w-5 h-5 text-pink-400 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" /></div></div>;
}

function Empty({ text, sub }: { text: string; sub?: string }) {
  return <div className="flex flex-col items-center justify-center py-16 text-center"><div className="w-20 h-20 bg-pink-50 rounded-full flex items-center justify-center mb-4"><Baby className="w-10 h-10 text-pink-300" /></div><p className="font-black text-slate-500 text-sm">{text}</p>{sub && <p className="text-xs text-slate-400 mt-1 font-medium">{sub}</p>}</div>;
}
    ...
    <div className="fixed inset-0 z-[9990] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={onClose}>
      <div className={`bg-white rounded-3xl shadow-2xl w-full ${sizeMap[size]} overflow-hidden animate-in zoom-in-95 duration-200`} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <h3 className="font-black text-slate-800 text-base flex items-center gap-2"><Baby className="w-5 h-5 text-pink-400" />{title}</h3>
...
function Field({ label, required, children }: any) {
  return <div className="space-y-1.5"><label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest">{label}{required && <span className="text-pink-500 ml-1">*</span>}</label>{children}</div>;
}
...
export function PreschoolInputAssessmentsClient({ academicYears, campuses, giaoVuCSUsers, grades: gradesProp, teachers, departments, currentUser }: { academicYears: AcademicYear[]; campuses: Camp[]; giaoVuCSUsers: any[]; grades: string[]; teachers: any[]; departments: any[]; currentUser: any; }) {
  ...
  const fetchConfigs = useCallback(async () => {
    setCfgLoading(true);
    try { const r = await fetch("/api/assessment-configs"); if (r.ok) setConfigs(await r.json()); } finally { setCfgLoading(false); }
  }, []);
  ...
  const saveCfg = async () => {
    ...
    const r = editCfg
      ? await fetch("/api/assessment-configs", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: editCfg.id, name: cfgForm.name, code: cfgForm.code }) })
      : await fetch("/api/assessment-configs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(cfgForm) });
    if (r.ok) { setCfgModal(false); fetchConfigs(); notify("Đã lưu"); }
  };
  const doDeleteCfg = async (id: string) => { await fetch(`/api/assessment-configs?id=${id}`, { method: "DELETE" }); fetchConfigs(); notify("Đã xóa"); };
  ...
  return (
    <div className="space-y-3 font-sans max-w-[1440px] mx-auto pb-16">
      ...
      {/* Header */}
      <div className="bg-white border border-pink-100 shadow-sm rounded-2xl px-4 py-3 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 px-3 py-2 bg-pink-50 rounded-xl border border-pink-100">
      ...
      {/* Tabs */}
      <div className="bg-white border border-pink-100 shadow-sm rounded-2xl px-1 py-1">
        <div className="flex flex-wrap gap-0.5">
          {[{ id: "periods", label: "Kỳ KS", icon: Clock }, { id: "categories", label: "Danh mục", icon: Settings }, { id: "children", label: "DS Trẻ", icon: Users }, { id: "reports", label: "Tổng hợp KQKS", icon: BarChart3 }].map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} className={`flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-[11px] font-bold transition-all duration-200 ${tab === t.id ? "bg-pink-500 text-white shadow-sm" : "text-slate-500 hover:bg-pink-50 hover:text-pink-600"}`}>
      ...
      {tab === "periods" && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-black text-slate-600 uppercase tracking-widest flex items-center gap-2"><Clock className="w-4 h-4 text-pink-400" /> Kỳ &amp; Đợt Khảo sát Mầm non</h2>
            <div className="flex gap-2">
              <button onClick={fetchPeriods} className="p-2 text-slate-400 hover:text-pink-500 hover:bg-pink-50 rounded-xl transition-all"><RefreshCw className="w-4 h-4" /></button>
              <button onClick={openAddPeriod} className="flex items-center gap-2 px-5 py-2.5 bg-pink-500 text-white text-[13px] font-black rounded-xl hover:bg-pink-600 transition-all shadow-lg shadow-pink-100"><Plus className="w-4 h-4" /> Tạo Kỳ mới</button>
            </div>
          </div>
          {pLoading ? <Spin /> : periods.length === 0 ? <Empty text="Chưa có Kỳ khảo sát nào" sub="Bấm Tạo Kỳ mới để bắt đầu" /> : (
            <div className="space-y-3">
              {periods.map(p => (
                <div key={p.id} className="bg-white rounded-2xl border border-pink-100 shadow-sm overflow-hidden hover:border-pink-200 transition-all">
                  <div className="px-4 py-3.5 flex flex-wrap items-center justify-between gap-3 cursor-pointer" onClick={() => setExpandedId(expandedId === p.id ? null : p.id)}>
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-pink-50 flex items-center justify-center flex-shrink-0"><Baby className="w-6 h-6 text-pink-400" /></div>
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1 text-[11px] text-slate-400 font-bold">
                          ...
                          <span className="text-pink-400">{p.batches?.length || 0} đợt</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5">
                      ...
                      <button onClick={e => { e.stopPropagation(); openEditPeriod(p); }} className="p-2 text-slate-400 hover:text-pink-500 hover:bg-pink-50 rounded-xl transition-all"><Edit2 className="w-4 h-4" /></button>
                  ...
                  {expandedId === p.id && (
                    <div className="border-t border-pink-50 p-5 bg-pink-50/30">
                      {!p.batches || p.batches.length === 0 ? (
                        <div className="text-center py-8 text-slate-400 font-bold text-xs uppercase tracking-wider bg-white rounded-2xl border-2 border-dashed border-pink-200">Chưa có Đợt nào</div>
                      ) : (
                        <div className="overflow-x-auto bg-white border border-pink-100 rounded-2xl shadow-sm">
                              ...
                              {p.batches.map(b => {
                                ...
                                return (
                                  <tr key={b.id} className="hover:bg-pink-50/30 transition-colors">
                                    <td className="p-4"><span className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-pink-50 font-black text-pink-500 text-xs">#{b.batchNumber}</span></td>
      ...
      {tab === "categories" && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-black text-slate-600 uppercase tracking-widest flex items-center gap-2"><Settings className="w-4 h-4 text-pink-400" /> Danh mục cấu hình</h2>
            <button onClick={() => openAddCfg("target")} className="flex items-center gap-2 px-5 py-2.5 bg-pink-500 text-white text-[13px] font-black rounded-xl hover:bg-pink-600 transition-all shadow-lg shadow-pink-100"><Plus className="w-4 h-4" /> Thêm mục</button>
          </div>
          {cfgLoading ? <Spin /> : (
                    ...
                    <tbody className="divide-y divide-pink-50">
                      {configs.map((c, i) => (
                        <tr key={c.id} className="hover:bg-pink-50/30 transition-colors">
                          ...
                          <td className="p-4 font-mono text-xs font-bold text-pink-600">{c.code}</td>
                          <td className="p-4 font-semibold text-slate-700 text-sm">{c.name}</td>
                          <td className="p-4 text-right"><div className="flex justify-end gap-1"><button onClick={() => openEditCfg(c)} className="p-2 text-slate-300 hover:text-pink-500 hover:bg-pink-50 rounded-lg"><Edit2 className="w-4 h-4" /></button><button onClick={() => setConfirm({ msg: `Xóa "${c.name}"?`, fn: () => doDeleteCfg(c.id) })} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-lg"><Trash2 className="w-4 h-4" /></button></div></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {tab === "children" && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="bg-white rounded-2xl border border-pink-100 shadow-sm p-4 flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              <label className="text-xs font-black text-slate-500 uppercase tracking-wider">Kỳ KS:</label>
              <select value={cPeriodId} onChange={e => { setCPeriodId(e.target.value); setCBatchId(""); }} className="border border-pink-100 rounded-xl p-2 text-sm font-medium text-slate-700 outline-none focus:ring-2 focus:ring-pink-300 min-w-[160px]">
            ...
            <div className="flex items-center gap-2">
              ...
              <select value={cBatchId} onChange={e => setCBatchId(e.target.value)} className="border border-pink-100 rounded-xl p-2 text-sm font-medium text-slate-700 outline-none focus:ring-2 focus:ring-pink-300 min-w-[140px]" disabled={!cPeriodId}>
                <option value="">Tất cả đợt</option>
                {selPeriod?.batches?.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            </div>
            <button onClick={fetchChildren} className="flex items-center gap-1.5 px-4 py-2 text-sm font-bold text-pink-600 bg-pink-50 hover:bg-pink-100 rounded-xl border border-pink-100"><Search className="w-4 h-4" /> Tìm</button>
            <div className="ml-auto relative"><Search className="w-4 h-4 text-slate-300 absolute left-3 top-1/2 -translate-y-1/2" /><input value={cSearch} onChange={e => setCSearch(e.target.value)} placeholder="Tìm bé..." className="pl-9 pr-4 py-2 border border-pink-100 rounded-xl text-sm outline-none focus:ring-2 focus:ring-pink-300 min-w-[200px]" /></div>
          </div>

          <div className="bg-white rounded-2xl border border-pink-100 shadow-sm p-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={openAddChild} disabled={!cPeriodId} className="flex items-center gap-1.5 px-4 py-2 text-xs font-black text-white bg-pink-500 hover:bg-pink-600 rounded-xl shadow-md shadow-pink-100 transition-all disabled:opacity-50"><Plus className="w-4 h-4" /> Thêm bé</button>
            </div>
          </div>
            ...
            {cLoading ? <Spin /> : filtChildren.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left whitespace-nowrap">
                  <thead className="bg-pink-50 border-b border-pink-100">
                    <tr>
                      <th className="p-4 w-12"><input type="checkbox" className="w-4 h-4 rounded accent-pink-500" checked={filtChildren.length > 0 && cSelected.length === filtChildren.length} onChange={e => setCSelected(e.target.checked ? filtChildren.map(c => c.id) : [])} /></th>
                  ...
                  <tbody className="divide-y divide-pink-50">
                    {filtChildren.map((child, i) => (
                      <tr key={child.id} className={`hover:bg-pink-50/30 transition-colors ${cSelected.includes(child.id) ? "bg-pink-50/50" : ""}`}>
                        <td className="p-4"><input type="checkbox" className="w-4 h-4 rounded accent-pink-500" checked={cSelected.includes(child.id)} onChange={e => setCSelected(e.target.checked ? [...cSelected, child.id] : cSelected.filter(id => id !== child.id))} /></td>
                        <td className="p-4 text-slate-400 text-sm">{i+1}</td>
                        <td className="p-4"><span className="font-mono text-xs font-black text-pink-600 bg-pink-50 px-2 py-1 rounded-lg">{child.studentCode}</span></td>
                        <td className="p-4"><div className="flex items-center gap-2.5"><div className="w-8 h-8 rounded-full bg-gradient-to-br from-pink-400 to-rose-300 flex items-center justify-center text-white font-black text-xs shadow-sm">{child.fullName?.charAt(0)}</div><span className="font-bold text-slate-800 text-sm">{child.fullName}</span></div></td>
                        <td className="p-4 text-sm text-slate-500">{child.dateOfBirth ? new Date(child.dateOfBirth).toLocaleDateString("vi-VN") : "—"}</td>
                        <td className="p-4">{child.gender ? <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${child.gender === "Nữ" || child.gender === "F" ? "bg-pink-100 text-pink-600" : "bg-blue-100 text-blue-600"}`}>{child.gender === "M" ? "Nam" : child.gender === "F" ? "Nữ" : child.gender}</span> : <span className="text-slate-300">—</span>}</td>
      ...
      {tab === "reports" && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="bg-white rounded-2xl border border-pink-100 shadow-sm p-4 flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              <label className="text-xs font-black text-slate-500 uppercase tracking-wider">Kỳ KS:</label>
              <select value={rptPeriodId} onChange={e => { setRptPeriodId(e.target.value); setCPeriodId(e.target.value); setRptBatchId("all"); }} className="border border-pink-100 rounded-xl p-2 text-sm outline-none focus:ring-2 focus:ring-pink-300 min-w-[160px]">
                <option value="">-- Chọn Kỳ --</option>
                {periods.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
              </select>
            </div>
            <div className="flex items-center gap-2">
              <label className="text-xs font-black text-slate-500 uppercase tracking-wider">Đợt:</label>
              <select value={rptBatchId} onChange={e => setRptBatchId(e.target.value)} className="border border-pink-100 rounded-xl p-2 text-sm outline-none focus:ring-2 focus:ring-pink-300 min-w-[140px]" disabled={!rptPeriodId}>
                <option value="all">Tất cả đợt</option>
                {periods.find(p => p.id === rptPeriodId)?.batches?.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            </div>
            <button onClick={() => { setCPeriodId(rptPeriodId); fetchChildren(); }} className="flex items-center gap-1.5 px-4 py-2 text-sm font-bold text-pink-600 bg-pink-50 hover:bg-pink-100 rounded-xl border border-pink-100"><RefreshCw className="w-4 h-4" /> Cập nhật</button>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[{ label: "Tổng trẻ KS", value: rptStats.total, color: "from-pink-500 to-rose-400", icon: Baby }, { label: "Đạt yêu cầu", value: rptStats.passed, color: "from-emerald-500 to-teal-400", icon: CheckCircle }, { label: "Chưa duyệt", value: rptStats.pending, color: "from-amber-400 to-orange-400", icon: AlertCircle }, { label: "Không đạt", value: rptStats.failed, color: "from-rose-500 to-red-400", icon: X }].map((stat, i) => (
          ...
          <div className="bg-white rounded-2xl border border-pink-100 shadow-sm p-6">
            <h3 className="font-black text-slate-800 text-sm uppercase tracking-widest mb-5 flex items-center gap-2"><Heart className="w-4 h-4 text-pink-400" /> Phân bố theo Độ tuổi / Lớp</h3>
                ...
                {rptStats.gradeStats.map(g => {
                  ...
                  return (
                    <div key={g.grade} className="space-y-1.5">
                      <div className="flex items-center justify-between text-sm"><span className="font-bold text-slate-700">{g.grade}</span><span className="font-black text-pink-500">{g.count} trẻ ({pct}%)</span></div>
                      <div className="w-full bg-pink-50 h-3 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-pink-400 to-rose-400 rounded-full transition-all duration-700" style={{ width: `${pct}%` }} /></div>
                    </div>
                  );
                })}
              </div>
      ...
      {/* Modal: Period */}
      <Modal open={pModal} onClose={() => setPModal(false)} title={editP ? "Sửa Kỳ" : "Tạo Kỳ mới"} footer={<><button onClick={() => setPModal(false)} className="flex-1 text-xs font-black uppercase text-slate-400 hover:text-slate-600">Hủy</button><button onClick={savePeriod} className="flex-1 py-3.5 bg-pink-500 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg shadow-pink-100">Lưu</button></>}>
        <div className="space-y-4">
      ...
      {/* Modal: Batch */}
      <Modal open={bModal} onClose={() => setBModal(false)} title={editB ? "Sửa Đợt" : "Thêm Đợt mới"} footer={<><button onClick={() => setBModal(false)} className="flex-1 text-xs font-black uppercase text-slate-400 hover:text-slate-600">Hủy</button><button onClick={saveBatch} className="flex-1 py-3.5 bg-pink-500 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg shadow-pink-100">Lưu</button></>}>
        <div className="space-y-4">
      ...
      {/* Modal: Child */}
      <Modal open={cModal} onClose={() => setCModal(false)} title={editC ? "Sửa thông tin Bé" : "Thêm Bé Mầm non"} size="lg" footer={<><button onClick={() => setCModal(false)} className="flex-1 text-xs font-black uppercase text-slate-400">Đóng</button><button onClick={saveChild} className="flex-1 py-3.5 bg-pink-500 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg shadow-pink-100">Lưu</button></>}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Mã bé" required><input value={cForm.studentCode} onChange={e => setCForm({...cForm, studentCode: e.target.value})} disabled={!!editC} className={inp + (editC ? " bg-pink-50/50" : "")} placeholder="MN001" /></Field>
