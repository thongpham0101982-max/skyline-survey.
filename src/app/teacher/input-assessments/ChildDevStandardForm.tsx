...
export default function ChildDevStandardForm({ student, onSave, isLocked, onClose }: any) {
    ...
    return (
        <div className="bg-slate-50 min-h-screen font-sans text-slate-900 border-x flex flex-col">
            ...
            <div className="flex-1 p-3 md:p-6 max-w-5xl mx-auto w-full space-y-6 pb-24">
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 flex flex-col sm:flex-row gap-5 items-center relative overflow-hidden">
                    ...
                    <div className="flex-1 text-center sm:text-left">
                        ...
                        <div className="mt-2 flex gap-3 justify-center sm:justify-start">
                            <span className="text-xs font-medium bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-md border border-emerald-100">
                                Đạt: <strong>{numDat}</strong>
                            </span>
                            <span className="text-xs font-medium bg-red-50 text-red-700 px-2.5 py-1 rounded-md border border-red-100">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left border-collapse">
                            <thead className="bg-slate-50 border-b border-slate-200">
                                <tr>
                                    <th className="px-4 py-3 w-12 text-center font-bold text-slate-600 uppercase text-xs">STT</th>
                                    <th className="px-4 py-3 font-bold text-slate-600 uppercase text-xs min-w-[250px]">Chỉ số đánh giá</th>
                                    <th className="px-4 py-3 w-64 font-bold text-center text-slate-600 uppercase text-xs">Kết quả</th>
                                    <th className="px-4 py-3 w-48 font-bold text-slate-600 uppercase text-xs">Ghi chú</th>
                                </tr>
                            </thead>
                                ...
                                {indicators.map((indicator, idx) => (
                                    <React.Fragment key={idx}>
                                        <tr className="hover:bg-sky-50/30 transition-colors">
                                            <td className="px-4 py-4 text-center font-bold text-slate-400 align-top pt-5">{idx + 1}</td>
                                            <td className="px-4 py-4 align-top">
                                                ...
                                                {idx === 12 && (
                                                    <div className="mt-3 bg-sky-50 p-3 rounded-xl border border-sky-100 w-full lg:w-3/4">
                                                        <table className="w-full text-sm bg-white border border-sky-200 rounded-lg overflow-hidden">
                                                            <thead>
                                                                ...
                                                                <tr>
                                                                    <td className="p-0 border-r border-sky-200">
                                                                        <textarea 
                                                                            ...
                                                                            disabled={isLocked}
                                                                            className="w-full p-2 h-16 resize-none outline-none focus:bg-sky-50 transition-colors text-slate-700 bg-transparent"
                                                ...
                                                {
                                                idx === 15 && (
                                                    <div className="mt-3 flex flex-col gap-2 bg-slate-50 p-3 rounded-xl border border-slate-200 w-full lg:w-2/3">
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200">
                    ...
                    <textarea
                        ...
                        disabled={isLocked}
                        className="w-full p-4 h-32 text-sm border border-slate-200 rounded-xl outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100 bg-slate-50 focus:bg-white transition-all resize-none placeholder-slate-400"
