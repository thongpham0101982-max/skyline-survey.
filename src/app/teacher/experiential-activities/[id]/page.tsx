...
export default function ActivityResultInput() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (actRes.ok) {
        ...
        // Academic year check bypassed for teacher friendliness
  ...
  if (loading) return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-teal-50/30 flex justify-center items-center">
      <div className="text-center">
        <Loader2 className="w-10 h-10 text-[#48BFE3] animate-spin mx-auto mb-3" />
  ...
  const getSelectClass = (val: string) => {
    const base = "w-full appearance-none text-xs font-bold rounded-xl focus:ring-2 focus:ring-[#48BFE3]/20 focus:border-[#48BFE3] pl-3 pr-8 py-2.5 transition-all outline-none border ";
    if (val) {
      ...
      <div className="max-w-[1400px] mx-auto space-y-5">
          ...
          <div className="p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <button
                ...
                className="text-xs font-semibold text-slate-400 hover:text-[#48BFE3] flex items-center gap-1 mb-2 transition-colors"
              >
              ...
              <div className="flex items-center gap-3 flex-wrap">
                {activityCode && (
                  <span className="text-xs font-black text-white bg-[#48BFE3] px-2.5 py-1 rounded-lg tracking-wide">
                    {activityCode}
                  </span>
                )}
              ...
              <div className="flex items-center gap-4 mt-2 text-sm text-slate-500 font-medium flex-wrap">
                {activity?.catalog?.name && activity.catalog.name !== activityName && (
                  <span className="flex items-center gap-1.5">
                    <Tag className="w-3.5 h-3.5 text-[#48BFE3]" />
                ...
                <span className="flex items-center gap-1 bg-teal-50/80 text-[#48BFE3] border border-teal-200/60 px-2 py-0.5 rounded-lg text-xs font-black">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                ...
                disabled={saving}
                className="px-5 py-2 bg-gradient-to-r from-[#48BFE3] to-[#20C997] hover:shadow-lg hover:shadow-[#48BFE3]/25 text-white rounded-xl text-sm font-bold transition-all flex items-center gap-2 disabled:opacity-70"
              >
        ...
        <div className="relative max-w-sm">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-[#48BFE3]/20 focus:border-[#48BFE3] outline-none transition-all shadow-sm"
          />
        </div>

        {/* BULK TOOLBAR */}
        {selectedIds.size > 0 && (
          <div className="bg-gradient-to-r from-[#48BFE3]/10 to-teal-50 border border-[#48BFE3]/25 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4 shadow-sm">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 bg-[#48BFE3] rounded-lg flex items-center justify-center">
                <CheckCheck className="w-4 h-4 text-white" />
              </div>
              <span className="font-bold text-[#48BFE3] text-sm">Đã chọn <span className="text-base">{selectedIds.size}</span> học sinh</span>
              <button
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button onClick={handleApplyBulk} className="px-4 py-2 bg-[#48BFE3] hover:bg-[#009085] text-white text-sm font-bold rounded-lg transition-all flex items-center gap-1.5 shadow-sm">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/60">
            ...
            <button onClick={toggleAll} className="text-xs font-semibold text-slate-500 hover:text-[#48BFE3] flex items-center gap-1.5 transition-colors">
              {allSelected
                ? <><CheckSquare className="w-4 h-4 text-[#48BFE3]" /> Bỏ chọn tất cả</>
                ...
                <tr className="bg-slate-50 border-b border-slate-200 text-xs font-bold uppercase tracking-wider text-slate-500">
                  <th className="pl-5 pr-2 py-3.5 w-10">
                    <input
                      ...
                      onChange={toggleAll}
                      className="w-4 h-4 rounded border-slate-300 text-[#48BFE3] cursor-pointer"
                    />
                  </th>
                  ...
                  {config.customColumns.map(col => (
                    <th key={col.id} className="px-3 py-3.5 min-w-[140px] text-[#48BFE3]">
                  ...
                  return (
                    <tr key={student.id} className={`border-b border-slate-100 transition-colors ${isSelected ? 'bg-teal-50/60' : idx % 2 === 0 ? 'bg-white hover:bg-slate-50/70' : 'bg-slate-50/30 hover:bg-slate-50/70'}`}>
                      <td className="pl-5 pr-2 py-3">
                        <input type="checkbox" checked={isSelected} onChange={() => toggleStudent(student.id)} className="w-4 h-4 rounded border-slate-300 text-[#48BFE3] cursor-pointer" />
                      </td>
                      <td className="px-3 py-3 text-sm font-bold text-slate-400">{idx + 1}</td>
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-2.5">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black text-white flex-shrink-0 ${isSelected ? 'bg-[#48BFE3]' : 'bg-gradient-to-br from-slate-400 to-slate-500'}`}>
                      ...
                      {config.visibleStandardColumns.includes('absenceReasonId') && (
                        <td className="px-3 py-3">
                          <input type="text" placeholder="Lý do..." value={student.absenceReasonId || ''} onChange={e => handleChange(student.id, 'absenceReasonId', e.target.value)}
                            className="w-full bg-white border border-slate-200 text-slate-700 text-sm rounded-lg focus:ring-2 focus:ring-[#48BFE3]/20 focus:border-[#48BFE3] px-3 py-2 transition-all outline-none placeholder:text-slate-400" />
                        </td>
                      )}
                      ...
                      {config.customColumns.map(col => (
                        <td key={col.id} className="px-3 py-3">
                          <input type={col.type === 'number' ? 'number' : 'text'} value={getCustomValue(student.note, col.id)} onChange={e => handleChange(student.id, col.id, e.target.value, true)}
                            className="w-full bg-teal-50/50 border border-teal-100 text-slate-700 text-sm rounded-lg focus:ring-2 focus:ring-[#48BFE3]/30 focus:border-[#48BFE3] px-3 py-2 transition-all outline-none" />
                        </td>
                      ))}
                    </tr>
                  );
          ...
          {students.length > 0 && (
            <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between text-xs text-slate-500 font-medium">
              ...
              {selectedIds.size > 0 && <span className="text-[#48BFE3] font-bold">Đang chọn: {selectedIds.size} học sinh</span>}
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
              <div>
                <h3 className="text-sm font-bold text-slate-700 mb-3 flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-[#48BFE3]" /> Cột chuẩn</h3>
                <div className="grid grid-cols-2 gap-2">
                  {standardColumnsMeta.map(col => (
                    <label key={col.id} className="flex items-center gap-2.5 p-3 rounded-xl border border-slate-200 hover:border-[#48BFE3]/40 cursor-pointer bg-white">
                      <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-[#48BFE3]"
              ...
              <div>
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-sm font-bold text-slate-700 flex items-center gap-1.5"><Plus className="w-4 h-4 text-[#48BFE3]" /> Cột tùy chỉnh</h3>
                  <button onClick={() => setEditConfig({ ...editConfig, customColumns: [...editConfig.customColumns, { id: `col_${Date.now()}`, name: 'Cột mới', type: 'text' }] })}
                    className="text-xs font-bold text-[#48BFE3] bg-[#48BFE3]/10 hover:bg-[#48BFE3]/20 px-3 py-1.5 rounded-lg flex items-center gap-1"><Plus className="w-3 h-3" /> Thêm cột</button>
                </div>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-2">
              ...
              <button onClick={() => { setConfig(editConfig); setShowConfigModal(false); }} className="px-5 py-2 bg-[#48BFE3] text-white font-bold rounded-xl text-sm shadow-sm">Lưu cấu hình</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" value={editData.name} onChange={e => setEditData({ ...editData, name: e.target.value })}
                  className="w-full bg-slate-50 ring-1 ring-slate-200 text-slate-800 text-sm font-semibold rounded-xl focus:ring-2 focus:ring-[#48BFE3] p-3.5 outline-none" placeholder="Tên hoạt động..." />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1.5">Ngày tổ chức</label>
                <input type="date" value={editData.date} onChange={e => setEditData({ ...editData, date: e.target.value })}
                  className="w-full bg-slate-50 ring-1 ring-slate-200 text-slate-800 text-sm font-semibold rounded-xl focus:ring-2 focus:ring-[#48BFE3] p-3.5 outline-none" />
              </div>
            </div>
            <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-2">
              ...
              <button onClick={handleUpdateActivity} disabled={saving} className="px-5 py-2 bg-[#48BFE3] text-white font-bold rounded-xl text-sm shadow-sm flex items-center gap-2 disabled:opacity-70">
