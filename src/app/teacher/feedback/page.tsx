  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      <div>
        ...
        <p className="text-slate-500 mt-1">Các ý kiến đóng góp thuộc năm học mặc định: <span className="font-bold text-[#48BFE3]">{activeYear?.name || 'N/A'}</span></p>
      </div>
            ...
            {feedbacks.map(fb => (
               <div key={fb.id} className="flex items-start gap-4 hover:bg-slate-50 transition-colors shadow-sm text-xs font-semibold">
                  <div className="min-w-[120px] pt-1 shrink-0">
                     <span className="text-[#48BFE3] text-[10px] font-black tracking-wider uppercase text-xs font-semibold">
                        {fb.className}
                     </span>
