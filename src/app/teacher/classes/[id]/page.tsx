...
export default async function TeacherClassDetailPage({ params }: any) {
  ...
  try {
    ...
    console.error("Auth error in TeacherClassDetailPage:", err)
  }
  ...
  try {
    ...
    const safeJson = (d: any) => JSON.parse(JSON.stringify(d || null))
    ...
    console.error("Error in TeacherClassDetailPage:", err)
    return (
      <div className="bg-white rounded-2xl shadow-sm p-12 text-center border-2 border-red-100 max-w-2xl mx-auto my-8">
        ...
        <h3 className="text-lg font-bold text-slate-800">Không thể tải thông tin chi tiết Lớp học</h3>
        <p className="text-slate-500 text-xs mt-2 leading-relaxed">
          Đã xảy ra sự cố trong quá trình truy vấn chi tiết lớp học. Vui lòng thử lại sau.
        </p>
      </div>
    )
  }
