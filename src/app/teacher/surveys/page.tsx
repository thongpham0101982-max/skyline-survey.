  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      <div>
        ...
        <p className="text-slate-500 mt-1">Các đợt khảo sát thuộc năm học mặc định: <span className="font-bold text-[#48BFE3]">{activeYear?.name || 'N/A'}</span></p>
      </div>
      ...
      <div className="bg-white border-2 border-teal-100 rounded-2xl p-5 shadow-xs flex flex-col gap-4">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-lg bg-teal-50 text-[#48BFE3] flex items-center justify-center">
            <Users className="w-5 h-5" />
          </div>
          <div>
            <h3 className="font-bold text-slate-800 text-sm">Thống kê Học sinh Khảo sát trong năm</h3>
            <p className="text-xs text-slate-500 mt-0.5">Tổng số học sinh được phân công khảo sát: <span className="font-black text-[#48BFE3] text-sm">${totalStudents}</span> học sinh</p>
          </div>
        </div>
        ...
        {sortedGrades.length > 0 ? (
          <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100">
            {sortedGrades.map(([grade, count]) => (
              <span key={grade} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-black bg-teal-50 text-[#48BFE3] border border-teal-100 hover:bg-[#48BFE3]/5 transition-colors">
            ...
            return (
              <div key={survey.id} className="bg-white rounded-xl shadow-sm border-2 border-teal-100 overflow-hidden hover:shadow-md transition-shadow group">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h2 className="text-xl font-bold text-slate-900 group-hover:text-[#48BFE3] transition-colors">
                          {survey.name}
                        </h2>
                        {isActive ? (
                    ...
                    <div className="flex items-center gap-8 p-4 text-xs font-semibold">
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-[#48BFE3]">{completionRate}%</div>
                ...
                <div className="flex justify-end text-xs font-semibold">
                  <Link href="/teacher/classes" className="text-sm font-bold text-[#48BFE3] hover:text-[#009085] flex items-center gap-1">
