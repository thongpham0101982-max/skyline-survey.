...
export function FeatureCard({ index, title, description, icon: Icon, gradientFrom, gradientTo }: FeatureCardProps) {
  return (
    <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-4 transition-all duration-300 hover:-translate-y-1 hover:border-white/20 hover:bg-white/10 group select-none">
      ...
      <p className="text-[10px] text-teal-100/50 leading-relaxed mt-1.5">{description}</p>
    </div>
  )
}
...
export function StudentProfileHighlight() {
  ...
  return (
    <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-5 mt-5 select-none">
      <div className="flex flex-col lg:flex-row lg:items-center gap-4">
        <div className="flex-shrink-0 w-[88px] h-[72px] bg-teal-500/10 border border-teal-300/20 rounded-xl flex items-center justify-center group">
          <div className="w-12 h-9 bg-[#0CB3AD]/20 border border-[#0CB3AD]/40 rounded-lg relative flex items-center justify-center transition-all duration-300 group-hover:scale-105 pointer-events-none">
            <FileText className="w-5 h-5 text-[#0CB3AD]" />
            <div className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-emerald-400 border-2 border-[#004F4B]">
              <div className="w-full h-full rounded-full bg-emerald-300 animate-ping opacity-60" />
            </div>
          </div>
        </div>
        <div className="flex-1">
          <h3 className="text-xs font-extrabold text-white tracking-wide uppercase">Hồ sơ học tập điện tử học sinh</h3>
          ...
          <div className="grid grid-cols-3 gap-1.5 mt-3">
            {badges.map((badge, idx) => {
              ...
              return (
                <div key={idx} className="flex items-center gap-1 bg-white/5 border border-white/5 rounded-md px-1.5 py-1 text-[9px] font-bold text-teal-100/80">
                  <BIcon className="w-2.5 h-2.5 text-[#0CB3AD] shrink-0 pointer-events-none" />
...
export function FeatureOverview() {
  ...
  const modules = [
    { index: '01', title: 'Khảo sát năng lực đầu vào', description: 'Tổ chức khảo sát, nhập kết quả, phân tích năng lực, hỗ trợ tuyển sinh và xếp lớp.', icon: Microscope, gradientFrom: '#059669', gradientTo: '#10B981' },
    { index: '02', title: 'Quản lý dự giờ giáo viên', description: 'Đăng ký tiết dạy, phân công dự giờ, đánh giá, phê duyệt và theo dõi năng lực chuyên môn.', icon: Clock, gradientFrom: '#0369A1', gradientTo: '#0EA5E9' },
    { index: '03', title: 'Thành tích và kỳ thi học sinh', description: 'Quản lý kỳ thi, cuộc thi, giải thưởng, huy chương, xếp hạng và lịch sử thành tích.', icon: Medal, gradientFrom: '#B45309', gradientTo: '#F59E0B' },
    ...
    { index: '06', title: 'Kết quả học tập & dự án', description: 'Tổng hợp kết quả môn học, hoạt động trải nghiệm, câu lạc bộ và mức độ tham gia.', icon: Lightbulb, gradientFrom: '#0F766E', gradientTo: '#14B8A6' }
  ]
  return (
    <div className="relative z-10 w-full">
      <div className="md:hidden flex items-center justify-between border-t border-b border-white/5 py-3 mb-4 select-none">
        ...
        <button type="button" onClick={() => setExpanded(!expanded)} className="flex items-center gap-1.5 text-xs font-bold text-[#0CB3AD] bg-white/5 px-3 py-1.5 rounded-lg border border-white/10 hover:bg-white/10 active:scale-95 transition-all">
        ...
        {roles.map((r) => {
          ...
          return (
            <button
              ...
              className={`flex items-center justify-center gap-1.5 py-2 px-2 rounded-xl border text-[11px] font-bold transition-all duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0CB3AD]/40 ${
                isActive
                  ? 'bg-[#E6F6F5] border-[#0CB3AD] text-[#0CB3AD]'
                  : 'bg-[#F5F8F8] border-[#E2ECF0] text-[#8FA5AE] hover:border-[#ACCFCF] hover:text-[#17383D]'
              }`}
            >
              <Icon className={`w-3.5 h-3.5 shrink-0 pointer-events-none ${isActive ? 'text-[#0CB3AD]' : 'text-[#9DB8C0]'}`} aria-hidden="true" />
...
export function PageFooter() {
  return (
    <footer className="w-full pt-4 text-center text-[10px] font-semibold text-slate-300 select-none">
...
export function LoginForm({
  ...
  const inputBase = "w-full h-[46px] rounded-xl border bg-white text-sm font-medium text-[#17383D] placeholder-[#B0C4CB] outline-none transition-all duration-200 hover:border-[#ACCFCF] focus:border-[#0CB3AD] focus:ring-2 focus:ring-[#0CB3AD]/15"
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-3.5">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="relative">
          ...
          <button
            ...
            className="absolute inset-y-0 right-0 w-11 flex items-center justify-center text-[#9DB8C0] hover:text-[#0CB3AD] transition-colors focus-visible:outline-none rounded-r-xl"
      ...
      <div className="flex items-center justify-between select-none">
        <label className="flex items-center gap-2 cursor-pointer text-xs text-slate-500">
          <input
            ...
            className="w-3.5 h-3.5 rounded border-[#D7E2E5] text-[#0CB3AD] focus:ring-[#0CB3AD]/20 cursor-pointer"
          />
        ...
        <button
          ...
          className="text-[11px] font-semibold text-[#0CB3AD] hover:text-[#099c97] hover:underline cursor-pointer transition-colors"
        >
          Quên mật khẩu?
        </button>
      </div>
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full h-[48px] bg-[#0CB3AD] hover:bg-[#099c97] active:bg-[#088b86] text-white rounded-xl text-sm font-bold shadow-sm hover:shadow-md flex items-center justify-center gap-2 hover:-translate-y-px active:translate-y-0 transition-all duration-200 disabled:opacity-60 disabled:pointer-events-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0CB3AD]/40"
      >
        {loading ? (
          <>
